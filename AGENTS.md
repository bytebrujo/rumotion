...
The current Picus version can be found in `packages/core/src/version.ts`. The next version should increment the patch version by 1.

Pull request titles should be in the format `\`[package-name]\``: [commit-message]`. For example, "`@picus/player`: Add new feature.
...
- **Picus Studio** (dev testbed): `cd packages/example && bun run dev` — starts at `http://localhost:3000`. This is the main dev UI for previewing video compositions.
- **Player testbed**: `cd packages/player-example && bun run dev` — for testing `@picus/player` changes.
- **Docs site**: `cd packages/docs && bun run start` — Docusaurus dev server.

### Rendering test videos

From `packages/example`:
- `bunx picus compositions` — list available compositions.
- `bunx picus render <comp-id> --output ../../out/video.mp4` — render a video.
- `bunx picus still <comp-id> --output ../../out/still.png` — render a still image.

### Known caveats

- `@picus/lambda-go` lint requires Go >= 1.23.0. The VM ships Go 1.22.2, so that package's lint will fail. This is optional and doesn't affect core development.
- `@picus/openai-whisper` tests require `OPENAI_API_KEY` env var. Without it, 1 test fails. This is expected and non-blocking.
- The Picus Studio (`bun run dev` in `packages/example`) sometimes reports "Already running on port 3000" if a previous instance is still bound. Check with `curl http://localhost:3000` before assuming it failed.
