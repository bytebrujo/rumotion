...
const innerStitchFramesToVideo = async (
	...
	if (renderAudioEvaluation === 'maybe') {
		throw new Error(
			'Picus is not sure whether to render audio. Please report this in the Picus repo.',
		);
	}
	...
	if (mediaSupport.audio && !mediaSupport.video) {
		if (!resolvedAudioCodec) {
			throw new TypeError(
				'exporting audio but has no audio codec name. Report this in the Picus repo.',
			);
		}

		if (!audio) {
			throw new TypeError(
				'exporting audio but has no audio file. Report this in the Picus repo.',
			);
		}
	...
	if (separateAudioTo) {
		...
		const finalDestination = path.resolve(picusRoot, separateAudioTo);
...
export const internalStitchFramesToVideo = (
	...
	const picusRoot = findPicusRoot();
	const task = innerStitchFramesToVideo(options, picusRoot);
 ...
 * @see [Documentation](https://www.picus.dev/docs/renderer/stitch-frames-to-video)
 */
