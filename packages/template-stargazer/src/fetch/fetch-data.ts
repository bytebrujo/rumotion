...
export async function fetchStargazers({
  ...
  if (!process.env.PICUS_GITHUB_TOKEN) {
    console.error(
      "No PICUS_GITHUB_TOKEN environment variable found. Using the GitHub REST API instead of GraphQL, which has a lower rate-limit and does not show the star dates.",
    );
