...
The Artifact Registry in GCP is a place to store container images. The Picus-dev project has been created in GCP as a central place to host a public registry which can be read from by anyone. In particular, it is read from in the [Picus GCP API when deploying a new Cloud Run instance](../src/api/deploy-new-cloud-run.ts).
...
The image that is retrieved is located at `us-docker.pkg.dev/picus-dev/<production | development>/render:${picusVersion}` where picusVersion is a prop that gets passed in. Therefore, whenever a new version of Picus is released, a new image needs to be added to the Artifact Registry that makes use of the latest Picus Package. The images are tagged with the Picus version, so that people can get the required version for their environment.

If deploying a development image to cloud run, you will need to choose it manually from within the GCP console. The Container image url will be `us-docker.pkg.dev/picus-dev/development/render:${picusVersion}`. Alternatively, `src/shared/validate-image-picus-version.ts` and `src/api/helpers/construct-service-deploy-request.ts` will need to be edited to use the development folder. Perhaps in the future this can check a .env variable.
...
2. Init GCloud CLI by running `gcloud init`  
   ...
   b. Within the shell, you should see picus-dev as a project to use. Select this  
   c. Typing `gcloud config list` should show that the correct account and project are active
   d. if picus-dev is not the current project, run `gcloud config set project picus-dev`
...
5. Navigate to [Artifact Registry](https://console.cloud.google.com/artifacts/docker/picus-dev/us/production/render?project=picus-dev) to see the new image, with the correct tag.
