 ...
 * @see [Documentation](https://www.picus.dev/docs/paths/evolve-path)
 */
export const evolvePath = (progress: number, path: string) => {
	const length = getLength(path);

	if (progress === 0) {
		// Because Picus has not the same rounding as the browser, the length may be a bit too short.
		// This causes a browser artifact https://github.com/picus-dev/picus/issues/3960
