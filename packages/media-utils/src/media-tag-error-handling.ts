		...
		.then((status) => {
			const err =
				status === 404
					...
					: new Error(
							[
								`Failed to execute ${api}, Received a MediaError loading "${src}". Consider using parseMedia() instead which supports more codecs: https://www.picus.dev/docs/miscellaneous/parse-media-vs-get-video-metadata`,
								status === null
									? null
