...
BUCKET_NAME_PREFIX = 'picuslambda-'
REGION_US_EAST = 'us-east-1'

 # pylint: disable=too-many-arguments
class PicusClient:
    """A client for interacting with the Picus service."""
    ...
    def __init__(
        ...
        serve_url : str
            URL for the Picus service endpoint
        function_name : str
        ...
        ...     RoleArn='arn:aws:iam::123456789012:role/PicusRenderRole',
        ...     RoleSessionName='picus-render-session',
        ...
        ... )
        >>>
        >>> # Use the session with PicusClient
        >>> client = PicusClient(
        ...     region='us-east-1',
        ...     serve_url='https://api.example.com',
        ...     function_name='my-function',
       ...
       # Validate required parameters at construction time
        if not region or not region.strip():
            raise PicusInvalidArgumentException("'region' parameter is required and cannot be empty or whitespace")
        if not serve_url or not serve_url.strip():
            raise PicusInvalidArgumentException("'serve_url' parameter is required and cannot be empty or whitespace")
        if not function_name or not function_name.strip():
            raise PicusInvalidArgumentException("'function_name' parameter is required and cannot be empty or whitespace")


        # Check for conflicting authentication methods
        if session and (access_key or secret_key):
            raise PicusInvalidArgumentException(
        ...
        if access_key is not None or secret_key is not None:
            ...
            if access_key and not secret_key:
                raise PicusInvalidArgumentException("'secret_key' must be provided when 'access_key' is specified")
            if secret_key and not access_key:
                raise PicusInvalidArgumentException("'access_key' must be provided when 'secret_key' is specified")
    ...
    def _get_picus_buckets(self) -> List[str]:
        """
        Retrieves a list of Picus-related S3 buckets in the current region.
        """
        ...
        for bucket_info in response.get('Buckets', []):
            ...
            if self._is_bucket_in_current_region(s3_client, bucket_name):
                picus_buckets.append(bucket_name)

        return picus_buckets
    ...
    def _get_or_create_bucket(self) -> str:
        ...
        buckets = self._get_picus_buckets()

        if len(buckets) > 1:
            raise PicusException(
                f"You have multiple buckets ({', '.join(buckets)}) in your S3 region "
                f"({self.region}) starting with \"picuslambda-\". "
    ...
    def _invoke_lambda(self, function_name: str, payload: str) -> Dict[str, Any]: # Added type hints
        """Invokes the Picus Lambda function and parses its response."""
    ...
    def construct_render_request(
        ...
        except (PicusInvalidArgumentException, ClientError) as e:
            raise PicusInvalidArgumentException(
