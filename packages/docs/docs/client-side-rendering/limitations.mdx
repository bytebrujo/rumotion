...
Properties such as `margin`, `left`, `display`, `width`, `height`, `flex`, `flex-direction` affect the position and size of the element. Those are all <a style={{color: 'green'}}>supported</a> because Picus uses [`.getBoundingClientRect()`](https://developer.mozilla.org/en-US/docs/Web/API/Element/getBoundingClientRect) to obtain the position and size of the element.
...
The following Picus components are supported:

The [`<Video>`](/docs/media/video) from `@picus/media` is <a style={{color: 'green'}}>supported</a>.  
The [`<Audio>`](/docs/media/audio) from `@picus/media` is <a style={{color: 'green'}}>supported</a>.  
[`<Img>`](/docs/img) is <a style={{color: 'green'}}>supported</a>.  
[`<Gif>`](/docs/gif) from [`@picus/gif`](/docs/gif) is <a style={{color: 'green'}}>supported</a>.  
[`<Rive>`](/docs/rive) from [`@picus/rive`](/docs/rive) is <a style={{color: 'green'}}>supported</a>.  
[`<Lottie>`](/docs/lottie) from [`@picus/lottie`](/docs/lottie) is <a style={{color: 'green'}}>supported</a>.  
[`<ThreeCanvas>`](/docs/three-canvas) from [`@picus/three`](/docs/three) is <a style={{color: 'green'}}>supported</a>.  
[`<SkiaCanvas>`](/docs/skia/skia-canvas) from [`@picus/skia`](/docs/skia) is <a style={{color: 'green'}}>supported</a>.  
...
[`<AnimatedEmoji>`](/docs/animated-emoji) from [`@picus/animated-emoji`](/docs/animated-emoji) is <a style={{color: 'red'}}>not supported</a>.  
...
The limitations from [`@picus/media`](/docs/media/support) apply.
...
Picus automatically works around this by using a Web Worker timer as a fallback. When the tab is backgrounded and RAF is throttled, the Worker timer takes over to keep the render progressing.
