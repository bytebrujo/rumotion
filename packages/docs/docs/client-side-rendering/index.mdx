...
Client-side rendering is a new additional capability of Picus that allows you to render videos, images, and audio directly in the browser, without requiring server-side infrastructure.

It is currently released as an experimental alpha under the [`@picus/web-renderer`](/docs/web-renderer) package.

## Key differences from server-side rendering

Unlike server-side rendering with [`@picus/renderer`](/docs/renderer), client-side rendering:

- Runs in the browser - No need to setup Node servers or Picus Lambda
...
To enable client-side rendering in the Studio, you need to call `Config.setExperimentalClientSideRenderingEnabled(true)` in your `picus.config.ts` file.
...
[View the current status and progress →](https://github.com/picus-dev/picus/issues/5913)
