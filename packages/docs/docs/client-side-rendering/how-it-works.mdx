...
Unlike in server-side rendering, where a pixel-perfect screenshot is made, in client-side rendering Picus places all elements on a canvas based on how it believes they are positioned and appearing in the DOM.

For this, Picus has developed a sophisticated algorithm for calculating the placement of the elements on the canvas.  
