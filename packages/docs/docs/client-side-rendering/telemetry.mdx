...
[Client-side rendering](/docs/client-side-rendering) has telemetry that sends an event for every render performed using the [`@picus/licensing`](/docs/licensing) package.

Picus has a [license](/docs/license) that makes it free for individuals and small companies but requires a paid license for bigger companies, whose price is determined by the amount of renders.
...
- IP Address
- Domain name (e.g. `https://picus.dev`)
...
If you have a Picus Company License or Enterprise License, go to [picus.pro](https://picus.pro) and get your license key from the "Usage" tab.
