...
## Use `usePicusEnvironment()` instead of `getPicusEnvironment()`

[`getPicusEnvironment()`](/docs/get-picus-environment) is a global API that may conflict with other Picus instances in your project, such as when you have a [`<Player>`](/docs/player) mounted on the same page.

Start using the [`usePicusEnvironment()`](/docs/use-picus-environment) hook instead.

## Use `useDelayRender()` instead of `delayRender()`, `continueRender()` and `cancelRender()`

If you have multiple renders at the same time or another instance of Picus on the same page, such as a `<Player>`, the [`delayRender()`](/docs/delay-render), [`continueRender()`](/docs/continue-render) and [`cancelRender()`](/docs/cancel-render) functions may conflict.
...
The only video and audio tags that are supported are [`<Video>`](/docs/media/video) and [`<Audio>`](/docs/media/audio) from `@picus/media`.  
