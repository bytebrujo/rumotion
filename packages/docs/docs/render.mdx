...
### Picus Studio deployment

It’s possible to deploy the Picus Studio onto a long-running server in the cloud, which can then be accessed by your non-technical team members using just a URL. Check out the [Deploy the Picus Studio](/docs/studio/deploy-server) guide to learn how to do this.
...
Picus has a full-featured server-side rendering API. Read more about it on the [server-side rendering API](/docs/ssr).

## AWS Lambda

Check out [Picus Lambda](/docs/lambda).
...
An alpha version of Picus for Cloud Run is [available](/docs/cloudrun).  
We plan to change it in the future so it shares a runtime with Picus Lambda.
