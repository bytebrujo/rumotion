...
The following thing is an anti-pattern in Picus:
...
While this will work during preview, it will break while rendering. The reason is that Picus is spinning up multiple instances of the webpage to render frames in parallel, and the random values will be different on every instance.

## Fixing the problem

Use the [`random()`](/docs/random) API from Picus to get deterministic pseudorandom values. Pass in a seed (number or string) and as long as the seed is the same, the return value will be the same.
