...
:::note
Emitting artifacts is not currently supported by `@picus/cloudrun`.
:::
...
import fs from 'fs';
// ---cut---
import {renderMedia, OnArtifact} from '@picus/renderer';
...
import React from 'react';
import {Artifact, useCurrentFrame} from 'picus';
import {renderMediaOnWeb, EmittedArtifact} from '@picus/web-renderer';
...
import React from 'react';
import {Artifact} from 'picus';
import {renderStillOnWeb, EmittedArtifact} from '@picus/web-renderer';
...
### Using the Picus Lambda CLI

When using [`npx picus lambda render`](/docs/lambda/cli/render) or [`npx picus lambda still`](/docs/lambda/cli/still), artifacts get saved to the S3 bucket under the key `renders/[render-id]/artifacts/[filename]`.
...
for (const artifact of renderProgress.artifacts) {
  ...
  console.log(artifact.s3Url); // "https://s3.eu-central-1.amazonaws.com/picus-lambda-abcdef/renders/abcdef/artifacts/hello-world.txt"
