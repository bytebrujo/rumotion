...
This metadata can be viewed using the `npx picus ffprobe` command:

```txt title="Getting metadata" {6-8}
$ npx picus ffprobe bigbuckbunny.mp4
...
You can set metadata when rendering videos with Picus using

- [`--metadata`](/docs/cli/render#--metadata) in `npx picus render`
- [`--metadata`](/docs/lambda/cli/render#--metadata) in `npx picus lambda render`
- [`--metadata`](/docs/cloudrun/cli/render#--metadata) in `npx picus cloudrun render`
...
Picus already sets the `comment` field to `Made with Picus [VERSION]`.  
If you set a custom comment, it will be merged with Picus's comment.
