...
`<Html5Audio>` is not supported in [`@picus/web-renderer`](/docs/client-side-rendering). Use [`<Audio>`](/docs/media/audio) from `@picus/media` instead.
:::
...
import {AbsoluteFill, Html5Audio, interpolate, staticFile} from 'picus';
...
```tsx twoslash
import {AbsoluteFill, Html5Audio, staticFile, useCurrentFrame} from 'picus';
...
```tsx twoslash
import {AbsoluteFill, Html5Audio} from 'picus';

// ---cut---
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Html5Audio audioStreamIndex={1} src={'https://picus.media/multiple-audio-streams.mov'} />
    </AbsoluteFill>
  );
};
```
...
In the [Picus Studio](/docs/terminology/studio) or in the [Picus Player](/docs/player), Picus will seek the audio if it gets too much out of sync with Picus's internal time - be it due to the audio loading or the page being too slow to keep up in real-time. By default, a seek is triggered if `0.45` seconds of time shift is encountered. Using this prop, you can customize the threshold.

### `pauseWhenBuffering?`<AvailableFrom v="4.0.111"/>

If set to `true` and the audio is buffering, the Player will enter into the [native buffering state](/docs/player/buffer-state). The default is `false`, but will become `true` in Picus 5.0.
...
- [Source code for this component](https://github.com/picus-dev/picus/blob/main/packages/core/src/audio/Audio.tsx)
