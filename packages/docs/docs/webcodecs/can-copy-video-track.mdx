...
import {canCopyVideoTrack} from '@picus/webcodecs';

const {tracks, container} = await parseMedia({
  src: 'https://picus.media/BigBuckBunny.webm',
  fields: {
    tracks: true,
    container: true,
  },
...
import {convertMedia, canCopyVideoTrack} from '@picus/webcodecs';
...
_string_ <TsType type="VideoTrack" source="@picus/media-parser" />
...
_string_ <TsType type="MediaParserContainer" source="@picus/media-parser" />
...
_string_ <TsType type="ResizeOperation" source="@picus/webcodecs" />

The [resize operation](/docs/webcodecs/resize-a-video) to apply to the video track.

### `outputVideoCodec`

_string | null_ <TsType type="ConvertMediaVideoCodec" source="@picus/webcodecs" />
...
- [Source code for this function on GitHub](https://github.com/picus-dev/picus/blob/main/packages/webcodecs/src/can-copy-video-track.ts)
