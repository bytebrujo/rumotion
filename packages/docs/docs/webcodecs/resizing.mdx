...
You can resize a video in the browser using [`convertMedia()`](/docs/webcodecs/convert-media) function from the [`@picus/webcodecs`](/docs/webcodecs) package.
...
```tsx twoslash title="Scale a video down to 480p"
import {ResizeOperation} from '@picus/webcodecs';
