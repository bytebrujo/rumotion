...
import {getPartialAudioData} from '@picus/webcodecs';

const audioData = await getPartialAudioData({
  src: 'https://picus.media/audio.wav',
  fromSeconds: 10,
  toSeconds: 20,
...
import {getPartialAudioData} from '@picus/webcodecs';
import {hasBeenAborted} from '@picus/media-parser';
...
- The function leverages [`@picus/media-parser`](/docs/media-parser) for parsing and WebCodecs for efficient audio decoding
- Sample rate and other audio characteristics depend on the source media file

## See also

- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/webcodecs/src/get-partial-audio-data.ts)
