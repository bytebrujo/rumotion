...
You can rotate a video in the browser using the [`@picus/webcodecs`](/docs/webcodecs) package to fix a video that has a bad orientation.
...
import {convertMedia, defaultOnVideoTrackHandler} from '@picus/webcodecs';
...
Use [picus.dev/rotate](https://picus.dev/rotate) to rotate a video online using WebCodecs.  
