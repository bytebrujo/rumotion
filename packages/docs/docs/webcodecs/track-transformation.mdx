...
[`@picus/webcodecs`](/docs/webcodecs) allows you to decide for each track what to do with it.
...
The enums `ConvertMediaVideoCodec` and `ConvertMediaAudioCodec` can be imported from `@picus/webcodecs`.
...
import {convertMedia, canReencodeVideoTrack} from '@picus/webcodecs';
...
import {convertMedia, defaultOnAudioTrackHandler} from '@picus/webcodecs';
...
Visit the [source code](https://github.com/picus-dev/picus/tree/main/packages/convert) for [convert.picus.dev](https://convert.picus.dev) to see a reference implementation for an online video converter that displays a user interface for all possible options.
