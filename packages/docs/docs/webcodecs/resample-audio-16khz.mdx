...
For using Whisper, Whisper.cpp, [`@picus/install-whisper-cpp`](/docs/install-whisper-cpp) and Whisper.wasm, you need to provide a 16-bit, 16KHz, WAVE audio file.
...
<details>
  <summary>💼 Important `@picus/webcodecs` License Disclaimer</summary>
  <LicenseDisclaimer />
</details>

```tsx twoslash title="Resample audio to 16kHz"
import {convertMedia, canReencodeAudioTrack} from '@picus/webcodecs';
...
If you don't want to install FFmpeg, you can also use the [smaller FFmpeg binary](/docs/cli/ffmpeg) provided by Picus.

```bash
npx picus ffmpeg -i input.mp4 -ar 16000 output.wav -y
```
