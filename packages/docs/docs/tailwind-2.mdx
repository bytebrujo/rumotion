...
2. Add the following to your [`picus.config.ts`](/docs/config) file:
...
:::warning
Due to a caching bug, the config file might not be picked up until you remove the `node_modules/.cache` folder - watch this issue: https://github.com/picus-dev/picus/issues/315
:::
