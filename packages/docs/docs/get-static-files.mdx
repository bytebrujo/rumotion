...
:::note
This API is being moved to the `@picus/studio` package. Prefer importing the API from [`@picus/studio`](/docs/studio/get-static-files) instead of `picus`.
:::
...
:::warning
This feature _only_ works in Picus Studio and during rendering, otherwise it returns an empty array.  
:::

:::note
On Linux, watching for changes in subdirectories is only supported from Node.js v19.1.0. If you use a version earlier than that, you need to refresh the Picus Studio browser tab manually.
:::

```tsx twoslash title="example.ts"
import {getStaticFiles, StaticFile} from 'picus';
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/core/src/get-static-files.ts)
