...
In Picus, it is not that easy because the `<div>` element in which the video is rendered into has a `scale()` transform applied to it, which affects the value that you get from `getBoundingClientRect()`.
...
- [Source code](https://github.com/picus-dev/measure-item)
- [Preview](https://measure-item.vercel.app)

## Versions prior to v4.0.103

In previous versions of Picus, `getBoundingClientRect()` could return dimensions with all values being `0` in the first `useEffect()` due to mounting your component but not showing it.
