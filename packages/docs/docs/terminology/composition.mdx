...
It can be registered in the [Picus Studio](/docs/terminology/studio) by rendering a [`<Composition>`](/docs/composition) component.  
A composition with a duration of 1 frame is also called a [`<Still>`](/docs/still).

In the [Picus Player](/docs/terminology/player), you don't use the `<Composition>` component, rather you pass the component and metadata directly to the [`<Player>`](/docs/player/player).
...
You need the composition ID to reference what you would like to render, for example: `npx picus render src/index <composition-id>`.
