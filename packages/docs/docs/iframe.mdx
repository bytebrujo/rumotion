...
Picus automatically wraps the `<iframe>` in a [`delayRender()`](/docs/delay-render) call
and ensures that the iframe is loaded before rendering the frame.

Ideally, the website should not have any animations, since only animations using [`useCurrentFrame()`](/docs/use-current-frame) are supported by Picus. See [Flickering](/docs/flickering) for an explanation.

## Example

```tsx twoslash
import {IFrame} from 'picus';

export const MyComp: React.FC = () => {
  return <IFrame src="https://picus.dev" />;
};
```
...
- [Source code for this component](https://github.com/picus-dev/picus/blob/main/packages/core/src/IFrame.tsx)
