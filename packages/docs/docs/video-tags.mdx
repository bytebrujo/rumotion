...
We offer three components for embedding videos into your Picus composition:

- [`<OffthreadVideo />`](/docs/offthreadvideo) - **our recommendation**, based on a Rust-based frame extractor
- [`<Html5Video />`](/docs/html5-video) from [`picus`](/docs/picus) (previously named `<Video>`) - based on the HTML5 `<video>` element
- [`<Video />`](/docs/media/video) from [`@picus/media`](/docs/media) - an experimental WebCodecs-based component, set to become the default

This page offers a comparison to help you decide which tag to use.

|                                                          | [`<OffthreadVideo />`](/docs/offthreadvideo)                                                     | [`<Html5Video />`](/docs/html5-video) <br/> [`<Html5Audio />`](/docs/html5-audio) <br/> (`picus`) | [`<Video />`](/docs/media/video) <br/> [`<Audio />`](/docs/media/audio) (`@picus/media`)                                                   |
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
...
Use the [`usePicusEnvironment()`](/docs/use-picus-environment) hook to render a different component in preview and rendering.

```tsx twoslash title="OffthreadVideo during preview, @picus/media during rendering"
import {usePicusEnvironment, OffthreadVideo, PicusOffthreadVideoProps} from 'picus';
import {Video, VideoProps} from '@picus/media';

const OffthreadWhileRenderingRefForwardingFunction: React.FC<{
  offthreadVideoProps: PicusOffthreadVideoProps;
...
```tsx twoslash title="Html5Audio during preview, @picus/media during rendering"
import {usePicusEnvironment, Html5Audio, PicusAudioProps} from 'picus';
import {Audio, AudioProps} from '@picus/media';

const Html5AudioWhilePreviewingComponent: React.FC<{
  html5AudioProps: PicusAudioProps;
