...
:::note
This guide has been contributed by the community and has not yet been tested by the Picus team.
:::

This guide provides a walkthrough for deploying Picus rendering service with basic render queuing on [Azure Container Apps](https://azure.microsoft.com/en-us/products/container-apps).

## Prerequisites

- A working Picus project (e.g., from `npm create video`) with provision to use [Docker](https://www.docker.com/).
...
For simplicity, we'll use [picus-gpu](https://github.com/alexfernandez803/picus-azure) which is configured to leverage GPU acceleration. This project is based with [Picus render server template](https://github.com/picus-dev/template-render-server) and uses the [scene](https://github.com/picus-dev/gpu-scene/blob/main/src/Root.tsx#L23) composition from [gpu-scene](https://github.com/picus-dev/gpu-scene) project.

## Step 2: Build and tag the docker image

From the project directory, build the Docker image and push it to Docker Hub with your account, specifying an image name and version tag. The `image-name` is `picus-docker-gpu` and `version` is `1.0.0`.

```bash
docker build -t your-dockerhub-username/picus-docker-gpu:1.0.0 .
```

## Step 3: Push the image to docker hub

```bash
docker push your-dockerhub-username/picus-docker-gpu:1.0.0
```
...
CONTAINER_IMAGE="docker.io/your-dockerhub-username/picus-docker-gpu:1.0.0"
...
Container app created. Access your app at https://picus-gpu.grayocean-24741fc9.australiaeast.azurecontainerapps.io/
"picus-gpu.grayocean-24741fc9.australiaeast.azurecontainerapps.io"
```
...
Send a POST request to the service's /renders endpoint. Replace `YOUR_SERVICE_URL` with the URL from the deployment output, `https://picus-gpu.grayocean-24741fc9.australiaeast.azurecontainerapps.io/`.
...
If `status` is `completed`, combine `YOUR_SERVICE_URL` and the `videoUrl` response to generate the download link (`https://picus-gpu.grayocean-24741fc9.australiaeast.azurecontainerapps.io/renders/955338f6-2607-48bd-bedb-6d4c98f7b4dc.mp4`).
...
Performance: Generated videos may appear blurry. GPU acceleration is configured in the project [config](https://github.com/alexfernandez803/picus-azure/blob/main/picus.config.ts#L6), but actual GPU utilization has not been confirmed.
:::
