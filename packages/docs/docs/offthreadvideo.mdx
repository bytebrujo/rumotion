...
`<OffthreadVideo>` is not supported in [`@picus/web-renderer`](/docs/client-side-rendering). Use [`<Video>`](/docs/media/video) from `@picus/media` instead.
:::

## Example

```tsx twoslash
import {AbsoluteFill, OffthreadVideo, staticFile} from 'picus';
...
```tsx twoslash
import {AbsoluteFill, OffthreadVideo} from 'picus';
// ---cut---
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <OffthreadVideo src="https://picus.media/BigBuckBunny.mp4" />
    </AbsoluteFill>
  );
};
```
...
import {AbsoluteFill, interpolate, staticFile, OffthreadVideo} from 'picus';
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <OffthreadVideo muted src="https://picus.media/BigBuckBunny.mp4" />
    </AbsoluteFill>
  );
};
```
...
Since Picus 4.0.117, Picus will adjust the colors of videos in different color spaces (such as HDR) when converting to RGB, to counteract color shifts.  
...
Prior to Picus 4.0.117, tone mapping was always disabled, and the `toneMapped` prop was not available.
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <OffthreadVideo audioStreamIndex={1} src={'https://picus.media/multiple-audio-streams.mov'} />
    </AbsoluteFill>
  );
};
```
...
import {cancelRender, continueRender, delayRender, Loop, OffthreadVideo, PicusOffthreadVideoProps, usePicusEnvironment, useVideoConfig, Html5Video} from 'picus';
...
- [Source code for this component](https://github.com/picus-dev/picus/blob/main/packages/core/src/video/OffthreadVideo.tsx)
