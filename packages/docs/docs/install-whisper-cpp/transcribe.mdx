...
import {transcribe} from '@picus/install-whisper-cpp';
...
Note: `large-v3-turbo` is only working properly from Whisper.cpp versions built from November 2024 or later and Picus v4.0.229 or greater.
...
[Adds the `--split-on-word` flag to Whisper.cpp](https://github.com/picus-dev/picus/pull/4257) for cleaner word-for-word output.
...
```tsx twoslash
import type {TranscribeOnProgress} from '@picus/install-whisper-cpp';
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/install-whisper-cpp/src/transcribe.ts)
