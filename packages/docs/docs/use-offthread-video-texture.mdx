...
:::warning
Deprecated - we recommend using [`@picus/media`](/docs/videos/as-threejs-texture) instead.
:::

Allows you to use a video in React Three Fiber that is synchronized with Picus's `useCurrentFrame()`, similar to [`useVideoTexture()`](/docs/use-video-texture), but uses [`<OffthreadVideo>`](/docs/offthreadvideo) instead.

This hook only works during rendering. In the Player and the Picus Studio, use [`useVideoTexture()`](/docs/use-video-texture) instead. See below for an example.
...
import {ThreeCanvas, useOffthreadVideoTexture} from '@picus/three';
...
import {ThreeCanvas, useOffthreadVideoTexture, useVideoTexture} from '@picus/three';
import {useRef} from 'react';
import {staticFile, usePicusEnvironment, useVideoConfig, Html5Video} from 'picus';
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/three/src/use-offthread-video-texture.ts)
