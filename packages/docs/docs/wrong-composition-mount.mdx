...
:::note
There is no reason to nest compositions in Picus.
...
The cause for the error is that in the component you passed in to the `component` prop of Picus Player, you are returning a `<Composition>`.
...
:::note
There is no use for compositions in `<Player>`. Only use them during rendering and when using the Picus Studio.
:::
