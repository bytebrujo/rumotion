...
:::info
The `public/` folder should always be in the same folder as your `package.json` that contains the `picus` dependency, even if your Picus code lives in a subdirectory.
:::
...
If you are a Create React App or Next.JS user, you might be used to just to be able to reference the asset from a string: `<img src="/my-image.png"/>`. Picus chooses to be different in that you need to use the `staticFile()` API because:
...
- It allows us to make paths framework-agnostic, so your code can work across Picus, Create React App, Next.JS and potentially other frameworks.
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/core/src/static-file.ts)
