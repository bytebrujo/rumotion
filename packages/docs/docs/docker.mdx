...
to build a Docker image called `picus-app`.  
Use the following command to run the image:

```sh
docker run picus-app
```
...
- If you are on Picus `v3.3.80` or lower, tell Picus where Chrome is installed:
...
Alpine Linux is a lightweight distribution often used in Docker. There are two known issues with it when used in conjunction with Picus:

- The launch of the Rust parts of Picus may be very slow (>10sec slowdown per render)
...
**May 30th, 2023**: Update document for Picus 4.0.
...
- [Deploying the Picus Studio](/docs/studio/deploy-server)
