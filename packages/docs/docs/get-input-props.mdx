...
Pass in a [parseable](/docs/cli) JSON representation using the `--props` flag to either `picus studio` or `picus render`:

```bash
npx picus render --props='{"hello": "world"}'
```

To simulate how it behaves, you can also pass props when using the Picus Studio:

```bash
npx picus studio --props='{"hello": "world"}'
```

You may also specify a file containing JSON and Picus will parse the file for you:

```bash
npx picus render --props=./path/to/props.json
```

You can then access the props anywhere in your Picus project:
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/core/src/config/input-props.ts)
