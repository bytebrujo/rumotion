...
In the following example, Picus is instructed to wait for the [`getMediaMetadata()`](/docs/mediabunny/metadata) promise to resolve before evaluating the composition.

By calling [`delayRender()`](/docs/delay-render), Picus will be blocked from proceeding until [`continueRender()`](/docs/continue-render) is called.
...
import {Composition, continueRender, delayRender} from 'picus';
import React from 'react';
...
export const Index: React.FC = () => {
  ...
  useEffect(() => {
    getMediaMetadata('https://picus.media/video.mp4')
...
This technique is not recommended anymore since v4.0, because the `useEffect()` does not only get executed when Picus is initially calculating the metadata of the video, but also when spawning a render worker.
...
<p>
  <Step>1</Step> Perform data fetching using <code>useEffect()</code>:
</p>
...
<p>
  <Step>2</Step> Call your own <code>calculateMetadata()</code> function reused from your Picus project:
</p>
