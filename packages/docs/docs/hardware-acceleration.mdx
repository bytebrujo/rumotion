...
From Picus v4.0.228, Picus supports hardware-accelerated encoding in some cases.  
...
Use the [`--hardware-acceleration`](/docs/cli/render#--hardware-acceleration) option in the `npx picus studio` command.

```bash
npx picus render MyComp --codec prores --hardware-acceleration if-possible
```

### In the Studio

In the [Picus Studio](/docs/studio), you can set the hardware acceleration option in the "Advanced" tab when rendering a video.
...
### In Picus Lambda and Cloud Run

These options are not supported in Picus Lambda and Cloud Run, because those cloud services do not support hardware acceleration.
...
The [`crf` option](/docs/encoding#controlling-quality-using-the-crf-setting) is not compatible with hardware-accelerated encoders. Instead, use the [`--video-bitrate`](/docs/cli/render#--video-bitrate) flag to control the quality and file size of the output video.
