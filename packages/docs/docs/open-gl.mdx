...
When rendering a video in Picus, different [GL](https://en.wikipedia.org/wiki/OpenGL) renderer backends can be selected.

The following renderer backends are supported in Picus:

- <code>null</code> - default, lets Chrome decide
- <code>angle</code>
- <code>egl</code>
- <code>swiftshader</code>
- <code>vulkan</code> (from Picus v4.0.41)
- <code>angle-egl</code> (from Picus v4.0.52)
...
Pass [`--gl=[angle,swangle,...]`](/docs/cli) in one of the following commands: [`picus render`](/docs/cli/render), [`picus compositions`](/docs/cli/compositions), [`picus still`](/docs/cli/still), [`picus lambda render`](/docs/lambda/cli/render), [`picus lambda still`](/docs/lambda/cli/still), [`picus lambda compositions`](/docs/lambda/cli/compositions).
