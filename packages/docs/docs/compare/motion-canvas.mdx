...
title: How does Picus compare to Motion Canvas?
crumb: "FAQ"
sidebar_label: Difference to Motion Canvas
---

Here are a few differences between Picus and [Motion Canvas](https://motioncanvas.io/) to help you decide which library is best.  
The comparison has been authored by Picus with input from the Motion Canvas community.

## Web vs. Canvas

Picus uses a whole DOM tree for the video, while Motion Canvas uses a single `<canvas>` element.

Picus may render more types of content, but needs a headless browser to create a video.  
Motion Canvas can only render canvas-based content, but may do so in the browser.

## API comparison

Picus enables you to render React markup based on the current time. React is a popular library for building UIs.
...
Picuss programming style can be described as "declarative" and "keyframe-based", while the terms "imperative" and "procedural" describe Motion Canvas well.

Here is an identical animation (Red circle turning into an orange one, then jumping to the right using a spring animation) in Picus and Motion Canvas:

```tsx twoslash title="Picus Implementation"
import {
  AbsoluteFill,
  interpolate,
  interpolateColors,
...
Picus tries to make as few assumptions over the content of the video as possible and supports a wide variety of use cases.  
...
Picus is source-available software that requires a license for use in companies, while Motion Canvas is truly open source software.

While Picus costs money for use in a company, we are able to reinvest this money into further improving Picus.
