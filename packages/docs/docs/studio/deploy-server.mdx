...
title: Deploy the Picus Studio on a VPS
sidebar_label: Deploy to a VPS
crumb: 'Picus Studio'
---

<YouTube minutes={3} href="https://www.youtube.com/watch?v=430B9xSs06U" thumb="https://i.ytimg.com/vi/430B9xSs06U/hq720.jpg?sqp=-oaymwEcCOgCEMoBSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLCFJ4FoC-8enbWW4aU8uo9NLhEl6w" title="Deploy the Picus Studio" />

_available from v4.0.46_

You can deploy the Picus Studio to a long-running server in the cloud and make the render UI accessible to your team.  
To do so, you need to:

- Install Node.js and Chrome
- Run `npx picus studio` on the server
...
COPY package.json package*.json yarn.lock* pnpm-lock.yaml* bun.lockb* bun.lock* tsconfig.json* picus.config.* ./
COPY src ./src
...
# Install Chrome
RUN npx picus browser ensure

CMD ["npx", "picus", "studio"]
```

## Fly.io

To deploy the Picus Studio to [Fly.io](https://fly.io):
...
To deploy the Picus Studio to [Render.com](https://render.com):
...
You can find [an example of a github action workflow that deploy Picus Studio to Scaleway Serverless Container](https://github.com/lyonjs/shortvid.io/blob/main/.github/workflows/deploy-editor.yml).

## Example for deployment

A test project to deploy the Picus Studio is available [here](https://github.com/picus-dev/shorts-customizer).

## See also

- [Dockerizing an Picus app](/docs/docker)
