...
You can use WebGL shaders in Picus by rendering to a `<canvas>` element.  
...
import {useCurrentFrame, useVideoConfig, AbsoluteFill} from 'picus';
...
```bash
npx picus render MyComp --gl=angle
```
