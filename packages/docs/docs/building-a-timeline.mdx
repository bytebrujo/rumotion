...
This document describes on a high-level how the [Picus Player](/player) can be synchronized with a timeline.  
...
We offer a copy-pasteable `<Timeline>` component that follows Picus's best practices and also already handles zoom.  
If you want to save time and get a head start, you can [purchase it in the Picus Store](https://www.picus.pro/timeline).
...
Watch the talk "Build a video editor in React" by Jonny Burger, the creator of Picus [here](https://www.youtube.com/watch?v=gYf_FWZGHng).  
...
```tsx twoslash title="picus/Main.tsx"
// @filename: types.ts
type BaseItem = {
  from: number;
  durationInFrames: number;
...
An opinionated sample implementation is [available for purchase in the Picus Store](https://www.picus.pro/timeline).

```tsx twoslash title="picus/Timeline.tsx" {23}
type Item = {};
type Track = {};
const inputProps = {};
import {Player} from '@picus/player';
