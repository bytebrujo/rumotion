...
id: still
sidebar_label: still
title: 'npx picus lambda still'
slug: /lambda/cli/still
crumb: 'Lambda CLI Reference'
---

Using the `npx picus lambda still` command, you can render a still frame in the cloud.

The command has the following structure:

```
npx picus lambda still <serve-url>? [<composition-id>] [<output-location>]
```
...
```
npx picus lambda still https://picuslambda-abcdef.s3.eu-central-1.amazonaws.com/sites/testbed/index.html my-comp
```

Rendering using the serve URL shorthand:

```
npx picus lambda still testbed my-comp
```
...
```
npx picus lambda still --frame=10 testbed my-comp
```

Downloading the result to a `out.png` file:

```
npx picus lambda still testbed my-comp out.png
```
