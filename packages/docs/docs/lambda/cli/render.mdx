...
Using the `npx picus lambda render` command, you can render a video in the cloud.

The structure of a command is as follows:

```
npx picus lambda render <serve-url> [<composition-id>] [<output-location>]
```
...
```
npx picus lambda render https://picuslambda-abcdef.s3.eu-central-1.amazonaws.com/sites/testbed/index.html my-comp
```

Rendering a video and saving it to `out/video.mp4`:

```
npx picus lambda render https://picuslambda-abcdef.s3.eu-central-1.amazonaws.com/sites/testbed/index.html my-comp out/video.mp4
```

Using the shorthand serve URL:

```
npx picus lambda render testbed my-comp
```
...
```
npx picus lambda render --props='{"hi": "there"}' testbed my-comp
```

Printing debug information including a CloudWatch link:

```
npx picus lambda render --log=verbose testbed my-comp
```

Keeping the output video private:

```
npx picus lambda render --privacy=private testbed my-comp
```

Rendering only the audio:

```
npx picus lambda render --codec=mp3 testbed my-comp
```
