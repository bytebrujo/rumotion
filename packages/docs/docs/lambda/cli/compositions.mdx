...
title: npx picus lambda compositions
...
```bash
npx picus lambda compositions <serve-url>
```
...
## `picus lambda compositions` vs. `picus compositions`

You can also get the compositions of a site that is hosted on S3 locally using [`npx picus compositions`](/docs/cli/compositions).  
Vice versa, you can also get the compositions from a [Serve URL](/docs/terminology/serve-url) that is not hosted on AWS Lambda using `npx picus lambda compositions`.

You should use `npx picus lambda compositions` if you cannot use [`npx picus compositions`](/docs/cli/compositions) because the machine cannot run Chrome.
...
:::info
If you don't feel like passing command line flags every time, consider creating a `picus.config.ts` [config file](/docs/config).
:::
...
- [`npx picus compositions`](/docs/cli/compositions)
