...
From 3.0.0 onwards, the changelog is located in the main [Picus changelog on GitHub](https://github.com/picus-dev/picus/releases). This document contains the changelog for prereleases and will not be updated anymore!
...
- Fixes an issue where `PICUS_AWS_ACCESS_KEY` and `PICUS_AWS_SECRET_ACCESS_KEY` environment variables would not be recognized inside Vercel serverless functions.
...
- Added a new [`npx picus lambda quotas`](/docs/lambda/cli/quotas) command which allows you to see your current AWS Lambda concurrency limit
- Added a new [`npx picus lambdas quotas increase`](/docs/lambda/cli/quotas) command which allows you to bump your AWS Lambda concurrency limit
...
- Bug fixed: Would not recognize a deployed Picus function if there are a lot of functions in one AWS account.
...
- Importing `@picus/lambda/client` no longer triggers a puppeteer error inside a Lambda function.
...
- Fixes `@picus/lambda/client` not being available
...
- all changes from 2.6.5 through 2.6.7: including new scale option also for lambda! https://www.picus.dev/docs/scaling
- chromium flags can now be set on lambda as well: https://www.picus.dev/docs/chromium-flags
...
- New algorithm for determining default concurrency: https://picus.dev/docs/lambda/concurrency
...
- Added changes from Picus 2.6.0 and 2.6.1
...
- Added [`npx picus lambda regions`](/docs/lambda/cli/regions) command
...
- The function name of a deployed function is not random anymore. Instead it has the format of `picus-render-2021-12-16-2048mb-120sec`
- More CLI commands support the `-q` (quiet) flag.
- Calling `npx picus lambda functions deploy` if a suitable function already exists will not throw an error anymore but log the existing function.
...
- Improved compatibility with Vercel: Since `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` are reserved environment variables on Vercel and are already occupied, you can now set `PICUS_AWS_ACCESS_KEY_ID` and `PICUS_AWS_SECRET_ACCESS_KEY` instead for calling any Picus Lambda Node.JS API.
- Multi-account load balancing: The environment variables `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `PICUS_AWS_ACCESS_KEY_ID` and `PICUS_AWS_SECRET_ACCESS_KEY` can now be switched before making a call to Lambda. (Previously there was a bug that would cache the clients.)
...
- Fixes the policy generated by `npx picus lambda policies user` so AWS accepts it. Update the user policy of your AWS user if you'd like the slimmest set of permissions needed.
- Fixes an issue where an error occuring during `npx picus lambda policies validate` would not be caught.
...
- Retry if the AWS rate limit is exceeded for `npx picus lambda permissions validate` command and for `validatePermissions()` function.
...
Picus 3.0 Rendering pipeline refactor merged into this branch!
...
- Fixes an issue where multiple versions of Picus could appear in a project even though they were pinned down in your `package.json`.
...
- The default number of max retries is now 1 (previously 3). A new CLI flag `--max-retries` was introduced which can be used for `npx picus lambda render` and `npx picus lambda still`
...
- Fixes an issue where a Picus version mismatch could happen.
...
1. Update to the newest version of Picus Lambda.
1. Read [Step 2](/docs/lambda/setup#2-create-role-policy) of the setup guide and update the role with the newest policy (`npx picus lambda policies role`).
1. Read [Step 5](/docs/lambda/setup#5-create-an-access-key-for-the-user) of the setup guide and update the user with the newest policy (`npx picus lambda policies user`).

- **Breaking change**: If your application throws an error or exception, the render will now fail. This will be the default behavior of Picus 3.0. See: [3.0 Migration](/docs/3-0-migration)
- Added CloudWatch support, now you can read the logs inside the Lambda function. When you execute `npx picus lambda render`, add the `--log=verbose` flag to print out an URL to CloudWatch.
...
- Improved CLI output of `npx picus lambda render`
...
- Command `npx picus lambda sites ls` now supports `-q` flag
...
- Merged changes from Picus 2.5.1
...
- Pinned exact Picus versions to avoid a version mismatch with Yarn
...
- Improved progress display for `npx picus lambda` command
- Now showing estimated cost for `npx picus lambda` command
...
- If you add a filename to the end of the render command `npx picus lambda render [url] [comp-name] out.mp4`, the video will be downloaded to your computer!
- `npx picus lambda render` has a progress bar now. To continue to see all details, use the `--log=verbose` flag!
...
- Added `--frames-per-lambda` option to `npx picus lambda render` command.
...
- Fixes a bug where `npx picus lambda policies user` could not be executed without AWS credentials which is a paradox.
...
- Fix `picus lambda policies validate` wrongly indicating that the `iam:GetUser` permission was not given
...
- New command [`npx picus lambda still`](/docs/lambda/cli) for rendering a still image
...
- Includes all the changes from Picus 2.2
...
- You can now use `npx picus lambda` instead of `npx picus-lambda`.
