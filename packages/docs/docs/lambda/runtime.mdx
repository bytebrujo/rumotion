...
| Picus Version | Node.js Release Line | Locked Runtime ARN                                                                                  |
| ---------------- | -------------------- | --------------------------------------------------------------------------------------------------- |
...
Currently Picus Lambda continues to use the 20.x runtime since instability was experienced with higher versions.

If your [user policy](/docs/lambda/permissions#user-policies) includes `lambda:PutRuntimeManagementConfig` (included by default if you set up Picus Lambda after November 2023), the Lambda runtime will be locked to the ARN shown for your Picus version (recommended).
...
The FFmpeg which is built into `@picus/renderer` is being used on Lambda.
...
Since December 2021 the following fonts are also available **only on the `arm64` version of Picus Lambda:**
