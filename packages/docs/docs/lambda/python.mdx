...
To trigger a Lambda render using Python, install the `picus-lambda` package using `pip`, by executing `pip install picus-lambda` from your python project. Use the same version as the `picus` version you are using from NPM, e.g. `pip install picus-lambda==4.0.15` (see [newest version](https://picus.dev/changelog)).
...
from picus_lambda import RenderMediaParams, Privacy, ValidStillImageFormats
...
When large props are detected, they are automatically uploaded to S3 and referenced by the Lambda function. The SDK will find an existing Picus bucket or create a new one automatically, following the same logic as the JavaScript SDK. This process is transparent to the user.
...
- Added `config` and `session` parameters to `PicusClient` constructor.
