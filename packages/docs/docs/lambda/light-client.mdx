...
The following methods and types can be imported from `@picus/lambda/client`.

`@picus/lambda/client` re-exports everything from the `@picus/lambda-client` NPM package, which you can install if you only need these functions in your project, saving you some dependencies.
...
Instead, this light client is meant to reduce the bundle size and avoid problems if you are calling Picus Lambda APIs from another Lambda function and therefore need to bundle your function code.

Commonly, Next.JS serverless endpoints or similar use AWS Lambda functions under the hood, for which `@picus/lambda/client` can be used.
