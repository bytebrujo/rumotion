...
This guide will show you how to use [Amazon SQS](https://aws.amazon.com/sqs/) (Simple Queue Service) and Picus's [`renderMediaOnLambda()`](/docs/lambda/rendermediaonlambda) API.
...
- The [picus-app](https://github.com/alexfernandez803/picus-serverless/tree/main/picus-app) contains a Picus composition and utility scripts for deploying and deleting Picus Lambda infrastructure in AWS. Note that this is the same application as from the [Serverless Framework guide](/docs/lambda/serverless-framework-integration).
- The [apigw-sqs-app](https://github.com/alexfernandez803/picus-serverless/tree/main/apigw-sqs-app) (API Gateway SQS) contains a [CDK](https://docs.aws.amazon.com/cdk/v2/guide/getting_started.html) project that deploys two Lambda functions. The [enqueue-function](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/src/enqueue-function/index.ts) function, when invoked, queues JSON data sent by the user; this can be an input payload for the Picus Lambda function. The [render-lambda-function](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/src/render-lambda-function/index.ts) function listens to enqueued message from SQS, processes it and renders videos.

The `enqueue-function` is configured to be invoked through [API Gateway](https://aws.amazon.com/api-gateway/) and is secured by [Cognito](https://aws.amazon.com/cognito/). The API Gateway and Cognito setup is automatically created by the [CDK Stack](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/lib/picus-cdk-starter-stack.ts) deployment upon execution of `cdk deploy`.
...
- Follow the same setup instruction from [picus-app guide](/docs/lambda/serverless-framework-integration#picus-app) as we will just re-use the application.

## apigw-sqs-app

In the following step, the Lambda functions [`enqueue-function`](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/src/enqueue-function/index.ts) and [`render-lambda-function`](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/src/render-lambda-function/index.ts) will be deployed to your AWS account. This guide is designed to be executed on your local machine.

The project will create all the resources defined by the CDK stack, including setting up [Cognito](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/lib/picus-cdk-starter-stack.ts#L19) for the project's authentication and authorization system, uploading Lambda code, generating and associating IAM roles to your AWS account.

### Prerequisites

- AWS deployment profile on your local machine, to configure an AWS deployment profile on your local machine.
- A AWS policy created named `picus-executionrole-policy` which is created from this [guide](/docs/lambda/without-iam/#1-create-role-policy).
...
The project can be found at [`picus-serverless project`](https://github.com/alexfernandez803/picus-serverless). If not done in the previous step, clone the project using:

```bash
git clone https://github.com/alexfernandez803/picus-serverless
```

#### 2. Go to `picus-serverless` and traverse to `apigw-sqs-app` directory

```bash
cd picus-serverless && cd apigw-sqs-app
```
...
#### 4. Create the Picus policy

- The `picus-executionrole-policy` should have been created, if not, follow this [guide](/docs/lambda/without-iam/#1-create-role-policy) in setting this up.

The `picus-executionrole-policy` is referenced from [here](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/lib/picus-cdk-starter-stack.ts#L64). The `CDK` stack creates a role for `render-lambda-function` with the `picus-executionrole-policy`.

```ts
// 👇 Create a role with custom name
const renderFunctionLambdaRole = new Role(this, 'picusSQSLambdaRole', {
  roleName: 'picusSQSLambdaRole',
...
This creates a role in AWS with the name of `picusSQSLambdaRole` with 2 policies attached:
...
- `picus-executionrole-policy` policy grants permission to the Lambda function to interact to AWS services that Picus Lambda needs access to in order to render a video. This policy is exactly the same policy from this [guide](/docs/lambda/without-iam/#1-create-role-policy).
...
│   │                             │        │                             │                             │ }                              │
├───┼─────────────────────────────┼────────┼─────────────────────────────┼─────────────────────────────┼────────────────────────────────┤
│ + │ ${queue.Arn}                │ Allow  │ sqs:ChangeMessageVisibility │ AWS:${picusSQSLambdaRole │                                │
│   │                             │        │ sqs:DeleteMessage           │ }                           │                                │
...
│   │                             │        │ sqs:SendMessage             │                             │                                │
├───┼─────────────────────────────┼────────┼─────────────────────────────┼─────────────────────────────┼────────────────────────────────┤
│ + │ ${picusSQSLambdaRole.Arn │ Allow  │ sts:AssumeRole              │ Service:lambda.amazonaws.co │                                │
│   │ }                           │        │                             │ m                           │                                │
└───┴─────────────────────────────┴────────┴─────────────────────────────┴─────────────────────────────┴────────────────────────────────┘
IAM Policy Changes
┌───┬──────────────────────────┬────────────────────────────────────────────────────────────────────────────────┐
...
├───┼──────────────────────────┼────────────────────────────────────────────────────────────────────────────────┤
│ + │ ${picusSQSLambdaRole} │ arn:${AWS::Partition}:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole │
│ + │ ${picusSQSLambdaRole} │ arn:${AWS::Partition}:iam::XXXXXXXXXX:policy/picus-executionrole-policy   │
└───┴──────────────────────────┴────────────────────────────────────────────────────────────────────────────────┘
...
apigw-sqs-app-stack.queuearn = arn:aws:sqs:ap-southeast-2:XXXXXXXXXX:picus_queue
apigw-sqs-app-stack.queuename = picus_queue
apigw-sqs-app-stack.queueurl = https://sqs.ap-southeast-2.amazonaws.com/XXXXXXXXXX/picus_queue
...
### Combining API Gateway, Picus Lambda and SQS together.

These are important information on how IAM roles are used by the function and instructions to enable the [render-lambda-function](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/lib/picus-cdk-starter-stack.ts#L97) to consume SQS messages.

From [CDK stack code](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/lib/picus-cdk-starter-stack.ts#L64), here are the important parts to take note of.

- The stack creates a queue named `picus_queue`.

```js title="create queue"
// 👇 create the queue
const picusQueue = new sqs.Queue(this, 'queue', {
  encryption: sqs.QueueEncryption.KMS_MANAGED,
  queueName: 'picus_queue',
});
```

- `picus_queue` grants access to 2 Lambda functions to interact with it. Each individual role are assigned to their respective Lambda function.
...
- This role is assigned to [`enqueue-function`](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/lib/picus-cdk-starter-stack.ts#L86), to allow it to write CloudWatch logs.

  ```js title="picusSQSLambdaRole"
  // 👇 create a role with custom name
  const renderFunctionLambdaRole = new Role(this, 'picusSQSLambdaRole', {
    roleName: 'picusSQSLambdaRole',
    assumedBy: new ServicePrincipal('lambda.amazonaws.com'),
...
- The [`render-lambda-function`](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/lib/picus-cdk-starter-stack.ts#L97) has been assigned this role, which includes access to interact with Cloudwatch and permission to access other AWS services as specified in the `picus-executionrole-policy`.

  ```js title="Grant access to the queue"
  // 👇 Grant permission to publish to the queue
  picusQueue.grantSendMessages(apiIntegrationRole);
  // 👇 grant permission to consume messages from the queue
  picusQueue.grantConsumeMessages(renderFunctionLambdaRole);
  ```

- Allow the render function to listen to the queue.

  ```js title="Listen to queue"
  picusRenderFunction.addEventSource(
    new SqsEventSource(picusQueue, {
      batchSize: 1,
...
On the other end, [`render-lambda-function`](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/src/render-lambda-function/index.ts) will be notified by SQS that is a video render request, it will consume the data from the render request and generate the video as per instruction, the function executes [`renderMediaOnLambda()`](/docs/lambda/rendermediaonlambda) as part of the process.

## Notes

- The deployment of Picus Lambda is configured to be deployed only to `ap-southeast-2` region to simplify the project, adjust this in the code at [`region.ts`](https://github.com/alexfernandez803/picus-serverless/blob/main/picus-app/src/infra/regions.ts).
- The deployment of `apigw-sqs-app` is configured to be deployed at `ap-southeast-2` region to simplify the project, adjust this in the code at [`picus-cdk-starter.ts`](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/bin/picus-cdk-starter.ts).
- Picus packages should be bundled inside the function when deployed, you can this in `nodeModules` property from `bundling` object, the code for this located in [here](https://github.com/alexfernandez803/picus-serverless/blob/main/apigw-sqs-app/lib/picus-cdk-starter-stack.ts#L103).

## See also

- [Using Lambda without IAM user](/docs/lambda/without-iam)
- [Using the Serverless Framework with Picus Lambda](/docs/lambda/serverless-framework-integration)
