...
Also, each render executed with Picus Lambda is completely isolated from another, in terms of compute and storage.
...
We recommend to scope the site to a specific environment using [`--site-name`](/docs/lambda/cli/sites/create#--site-name) when creating a site. For example, use `--site-name=picus-production` and `--site-name=picus-staging`.

A site contains your React / Picus code and might change frequently.  
Make sure to update your site whenever you make changes to your code or upgrade Picus, and to not use the site for other environments if they differ.

## Buckets

Just one bucket per region can be used for Picus Lambda, but the resources in it can be scoped.
...
- The [`npx picus lambda render`](/docs/lambda/cli/render) command looks for functions that match this convention.
- With the default user policy, Picus Lambda restricts itself from accessing functions that don't match this convention.
...
Every Picus Lambda user runs the exact same code in their function.
...
Functions also don't contain any code that you write, they are binary and every Picus Lambda user runs the exact same code.
...
You can pass [`--function-name`](/docs/lambda/cli/render#--function-name) to [`npx picus lambda render`](/docs/lambda/cli/render)
...
- [Upgrading Picus Lambda](/docs/lambda/upgrading)
