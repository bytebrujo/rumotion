...
EC2 instances have credentials that can interact with the AWS SDK. If you would like to use it with Picus Lambda, you need to assume the role using STS to generate an access token that can be used by Picus.

This guide will demonstrate how to securely interact with Picus's [`renderMediaOnLambda()`](/docs/lambda/rendermediaonlambda) operations from an AWS EC2 instance using [Node.js](https://nodejs.org/) and [TypeScript](https://www.typescriptlang.org/).

To supplement this guide, two projects have been created:

- The [picus-app](https://github.com/alexfernandez803/picus-serverless/tree/main/picus-app) includes a Picus composition and utility scripts for deploying and deleting Picus Lambda infrastructure in AWS. It should be noted that this is the same application as the one featured in the [Serverless Framework guide](/docs/lambda/serverless-framework-integration). Follow the setup [guide](/docs/lambda/serverless-framework-integration#picus-app), if the Picus lambda is not yet deployed to your AWS account.

- The [ec2-picus-lambda](https://github.com/alexfernandez803/picus-serverless/tree/main/ec2-picus-lambda) is a TypeScript Node.js application that initiates a video rendering process via a [REST](https://en.wikipedia.org/wiki/Representational_state_transfer) endpoint.
...
## Setup for `ec2-picus-lambda` application

### 1. Create the Picus policy
...
### 2. Create role for picus render execution
...
<Step>4</Step>Under "Attach permissions policies", search for "picus-executionrole-policy" and click the checkbox to assign this policy. If the policy has not been created, refer to step 1.<br/>
...
<Step>7</Step>On the "Review" page, name the role "picus-ec2-executionrole" exactly. Leave the other fields as they are.<br/>
...
<Step>6</Step>On the "Review" page, name the role as "ec2-picus-role". Leave the other fields as they are.<br/>
<Step>7</Step>Click "Create role" to confirm.<br/>
<Step>8</Step>Make a note of the "ARN" for the role. It should be in this format "arn:aws:iam::XXXXXXXX:role/ec2-picus-role"<br/>

### 4. Trust the EC2 role from "picus-ec2-executionrole"

#### Steps

<Step>1</Step>From the IAM Roles section, find the role created in step 2, or filter roles by name using "picus-ec2-executionrole".<br/>
...
<Step>4</Step>Edit the policy statement to add the ARN of the EC2 role (ec2-picus-role) created in step 3. Add it as one of the principals and as a AWS principal.<br/>
<Step>5</Step>Save the changes to the trust policy.<br/><br/>

```json title="picus-ec2-executionrole"
{
  "Version": "2012-10-17",
  "Statement": [
    {
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::XXXXXXXX:role/ec2-picus-role"
      },
      "Action": "sts:AssumeRole"
    }
  ]
...
:::info
This configuration grants authority to `ec2-picus-role` to assume the role of `picus-ec2-executionrole` and provides the necessary permissions to access AWS services and resources required by Picus for video rendering.
:::
...
<Step>9</Step>From "Advance details", on "IAM instance profile" find the role you specifically created for EC2, this is "ec2-picus-role".<br/>
...
Upload the application [code](https://github.com/alexfernandez803/picus-serverless/tree/main/ec2-picus-lambda) to the EC2 instance by any means you are comfortable with. For this instance, the code was uploaded using an SFTP client named [Cyberduck](https://cyberduck.io/). Upload the application code to the home directory. When logging in from Cyberduck, the default directory is `/home/ubuntu.`
...
```
 ssh -i "picus.pem" ubuntu@example.com
```
...
<Step>2</Step> Assign values for the environment keys such as <code>PORT</code>, <code>PICUS_ROLE_ARN</code>, <code>PICUS_ROLE_SESSION_NAME</code>, <code>API_USERNAME</code>, <code>API_PASSWORD</code><br/><br/>

```bash title=".env"
PORT=8080
PICUS_ROLE_ARN=arn:aws:iam::XXXXXXXXXX:role/picus-ec2-executionrole
PICUS_ROLE_SESSION_NAME=render-sessions
API_USERNAME=admin
API_PASSWORD=password
```

- `PORT` represents the which port should the application can run from.
- `PICUS_ROLE_ARN` represents the `ARN` of the role which the application `assume` to render the video, for this instance it is `picus-ec2-executionrole` ARN from `step 2`.
- `PICUS_ROLE_SESSION_NAME` a name to uniquely identify the role session when the same role is assumed by different principals.
...
- Execute the CURL command
  ...
  From the `/render` API resource, the application will execute this piece of [code](https://github.com/alexfernandez803/picus-serverless/blob/main/ec2-picus-lambda/src/services/render-services.ts#L11) This codes assume the role of `ec2-picus-role`, then provided with temporary access tokens ie `AccessKeyId`, `SecretAccessKey` and `SessionToken`. These credentials will then need to be set as environment variables on the server so that in can be used by the [`renderMediaOnLambda()`](/docs/lambda/rendermediaonlambda) process. Setting the environment parameters route the render process in this (code)[https://github.com/alexfernandez803/picus-serverless/blob/main/ec2-app/render_handler.ts#L14].

  ```bash title="API Response"
  {"message":"Video rendered.","renderId":"px60ct13fy","bucketName":"picuslambda-apsoutheast2-qv16gcf02l"}
  ```
...
<Step>3</Step>From the EC2 Dashboard, select the instance that you want to destroy. For this instance it's "picus-server".<br/>
...
:::note
This is a simple demonstration of using Picus's Lambda and EC2. To productionize this approach, other steps may be required based on the use case. Implement an enterprise-grade security mechanism, run the application as a service, and have it sit behind a reverse proxy like [Nginx](https://www.nginx.com/).
:::
