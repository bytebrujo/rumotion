...
The Picus packages are also bundled into the stack, these ensures that [`renderMediaOnLambda()`](/docs/lambda/rendermediaonlambda) can be executed by the [Lambda function](https://github.com/alexfernandez803/example-lambda/blob/main/src/render-function/index.ts).
...
The CDK creates an IAM role named `picusLambdaServerlessRole` which needs the Picus policy [setup](/docs/lambda/without-iam).
...
```bash
{"message":"SUCCESS","bucketName":"picuslambda-apsoutheast2-5essis84y1","renderId":"1pwhfhh11z"}
```
...
- Try assigning the Picus [role](/docs/lambda/without-iam#1-create-role-policy) via CDK [`code`](https://github.com/alexfernandez803/example-lambda/blob/main/lib/picus-cdk-starter-stack.ts).
