...
As documented on the [`Permissions page`](/docs/lambda/permissions), the default way of using Picus Lambda involves creating a Picus user and assign it policies.
...
- Copy the JSON policy template below:
  <details>
    <summary>Show full role permissions JSON file for latest Picus Lambda version</summary>
    <UserPolicy />
  </details>
- Click next. On the tags page, you don't need to fill in anything. Click next again.
- Give the policy the name `picus-executionrole-policy`. The other fields can be left as they are.
...
- Go to the [AWS Management Console](https://console.aws.amazon.com/console/home)
  ...
  - Attach policy
  - Find the policy, i.e. `picus-executionrole-policy`
  - Select the policy
...
In the background, when the Lambda function is executed, it is provided with environment variables such as `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` and `AWS_SESSION_TOKEN` that has temporary permission to AWS resources that `renderMediaOnLambda()` requires to render the video. The elevated powers come from the policy statements in `picus-executionrole-policy`.
