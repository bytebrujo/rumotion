...
To trigger a Lambda render using PHP, install the `picus/lambda` package using `composer`. Use the same version as the `picus` version you are using from NPM and pin the version by removing the `^` character in your `composer.json`.
