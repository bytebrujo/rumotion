...
```ts twoslash
import {getFunctionInfo} from '@picus/lambda';

const info = await getFunctionInfo({
  functionName: 'picus-render-d7nd2a9f',
  region: 'eu-central-1',
});
console.log(info.functionName); // picus-render-d7nd2a9f
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/lambda/src/api/get-function-info.ts)
