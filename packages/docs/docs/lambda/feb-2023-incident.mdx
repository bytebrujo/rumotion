...
:::info
In Picus 4.0, this problem is fixed.
:::

On February 24th 2023, a new version of the AWS Lambda Node.JS runtime started rolling out that breaks Picus Lambda.  
Picus Lambda users are advised to read this document to avoid any downtime.
...
Since at a later point, AWS will roll out the runtime to any existing functions, we recommend to upgrade to Picus `3.3.54` or later in the near future and redeploy your functions.

## Resolutions

### Option 1: `v3.3.54` update (recommended)

In Picus `v3.3.54`, Picus will lock the AWS Lambda Node.JS runtime to `v14.28` to prevent this issue from happening.  
...
Since it recently became possible to lock the AWS Lambda runtime version, we will be using this option going forward to make Picus more resilient to future runtime updates.
