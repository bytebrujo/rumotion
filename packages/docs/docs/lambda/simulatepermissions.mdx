...
The CLI equivalent is `npx picus lambda policies validate`.
...
```ts twoslash
import {simulatePermissions} from '@picus/lambda';
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/lambda/src/api/iam-validation/simulate.ts)
