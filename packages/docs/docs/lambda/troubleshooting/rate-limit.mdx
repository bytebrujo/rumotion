...
According to AWS, "some accounts" which are new to AWS Lambda might get a very low concurrency limit such as 10 when they first start with AWS Lambda. In that case, increase the limit via the AWS console or the Picus CLI (see below).

### Workaround

If you want to test Picus Lambda while you are waiting for AWS to approve a higher concurrency limit, you can pass a higher number to [`framesPerLambda`](/docs/lambda/rendermediaonlambda#framesperlambda).
...
You can request a quota increase under [`https://console.aws.amazon.com/servicequotas/home`](https://console.aws.amazon.com/servicequotas/home) or using the [Picus CLI](/docs/lambda/cli/quotas):

```
npx picus lambda quotas increase
```
...
Contact the Picus team, preferrably via [Discord](https://picus.dev/discord) and we will be happy to help you with your rate limit problem.
