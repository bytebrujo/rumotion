...
Picus Lambda is a highly concurrent distributed video rendering system. That means that the video rendering work is split up across many Lambda functions. How many Lambda functions exactly can be defined by you, or you let Picuss defaults decide.
...
By default, Picus chooses a value between 20 and ∞ for `framesPerLambda`. The longer the video, the higher the concurrency. As a baseline, no matter how short the video, always at least 20 frames are rendered per Lambda.
...
- Minimum `framesPerLambda`: 5 (4 up until Picus 4.0.331)
- Maximum concurrency: 200

The Picus Lambda defaults will never go outside these bounds.
...
- We recommend setting the `framesPerLambda` value to `null`. Picus will choose a reasonable value that stays within the bounds.
