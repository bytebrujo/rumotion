...
This document describes the necessary permissions for Picus Lambda and explains to those interested why the permissions are necessary.

For a step by step guide on how to set up permissions, [follow the setup guide](/docs/lambda/setup).

## User permissions

This policy should be assigned to the **AWS user**. To do so, go to the [AWS console](https://console.aws.amazon.com/console/home) ➞ [IAM](https://console.aws.amazon.com/iam/home) ➞ [Users](https://console.aws.amazon.com/iamv2/home#/users) ➞ Your created Picus user ➞ Permissions tab ➞ Add inline policy ➞ JSON.
...
:::info
You can always get the suitable permission file for your Picus Lambda version by typing `npx picus lambda policies user`.
:::

## Role permissions

This policy should be assigned to the **role `picus-lambda-role`** in your AWS account. The permissions below are given to the Lambda function itself.

To assign, go to [AWS console](https://console.aws.amazon.com/console/home) ➞ [IAM](https://console.aws.amazon.com/iam/home) ➞ [Roles](https://console.aws.amazon.com/iamv2/home#/roles) ➞ [`picus-lambda-role`](https://console.aws.amazon.com/iam/home#/roles/picus-lambda-role) ➞ Permissions tab ➞ [Add inline policy](https://console.aws.amazon.com/iam/home#/roles/picus-lambda-role$createPolicy?step=edit).

<details>
  <summary>
    Show full role permissions JSON file for latest Picus Lambda version
  </summary>
  <RolePolicy />
</details>

:::info
You can always get the suitable permission file for your Picus Lambda version by typing `npx picus lambda policies role`.
:::
...
The following table is a breakdown of why Picus Lambda requires the permissions it does.
...
<table>
  ...
  <tr>
    ...
    <td><code>*</code></td>
    <td>
      Allows for <code>npx picus lambda permissions validate</code>.
    </td>
  </tr>
  <tr>
    <td>
    ...
    <td><code>arn:aws:iam::*:role/picus-lambda-role</code></td>
    <td>
  ...
  <tr>
    <td>
      ...
      <code>s3:PutBucketPolicy</code> <br />
    </td>
    <td>
      <code>{'arn:aws:s3:::picuslambda-*'}</code>
    </td>
    <td>
      Allows to create and delete buckets and objects in your account, make objects public and configure them as websites. Only buckets that start with <code>picuslambda-</code> can be accessed. <code>s3:PutBucketPolicy</code> is used to apply a bucket policy that restricts public access to <code>s3:GetObject</code> only (see <a href="/docs/lambda/bucket-security">Bucket Security</a>).
    </td>
  </tr>
  <tr>
    <td>
    ...
    <td>
      Allows listing the names of all buckets in your account, in order to detect an already existing Picus bucket.
    </td>
  </tr>
  ...
  <tr>
    ...
    <td>
      <code>{"arn:aws:lambda:*:678892195805:layer:picus-binaries-*"}</code>
    </td>
    <td>
    Allows to read Chromium and FFMPEG binaries. These binaries are hosted in an account hosted by Picus specifically dedicated to hosting those layers in all supported regions.
    </td>
  </tr>
  <tr>
    <td>
  ...
  <tr>
    ...
    <td>
      <code>{"arn:aws:lambda:*:*:function:picus-render-*"}</code>
    </td>
    <td>
  ...
  <tr>
    ...
    <td>
      <code>{"arn:aws:logs:*:*:log-group:/aws/lambda/picus-render-*"}</code>
    </td>
    <td>
...
<table>
  ...
  <tr>
    ...
    <td>
    Get a list of Picus buckets in order to find existing buckets that start with <code>picuslambda-</code>.
    </td>
  </tr>
  <tr>
    <td>
    ...
    <td>
  Create and delete buckets and items, make them public or private and fetch their location. Since Picus stores the videos in an S3 bucket, it needs basic CRUD capabilities over those buckets. The permission only applies to buckets that start with <code>picuslambda-</code>
    </td>
  </tr>
  <tr>
    <td>
