...
Gets an array of Picus projects in your S3 account.

The projects are located in the `sites/` subdirectory of your S3 bucket. Remember - you should only have one bucket for Picus Lambda per region, therefore you do not need to specify the name of the bucket for this function.
...
for (const site of sites) {
  ...
  console.log(site.version); // The Picus version of the site, or null
}
...
:::note
Preferrably import this function from `@picus/lambda/client` (available from v3.3.42) to avoid problems [inside serverless functions](/docs/lambda/light-client).
:::
...
If `true`, only sites whose Picus version matches the installed version of `@picus/lambda` are returned. If `false` or not provided, all sites are returned.
...
An array of deployed Picus projects that you can use for rendering.
...
The Picus version of the site, or `null` if the version could not be determined.

### `buckets`

An array of all buckets in the selected region in your account that start with `picuslambda-`.

:::info
You should only have [1 bucket](/docs/lambda/multiple-buckets) per region for all your Picus projects. Nonetheless `buckets` is an array, since we cannot prevent you from manually creating additional buckets with the `picuslambda-` prefix.
:::
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/lambda/src/api/get-sites.ts)
