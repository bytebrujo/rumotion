...
Some commonly asked questions about Picus Lambda.

### Is Lambda self-hosted?

Yes, you host Picus Lambda in your own AWS account. Picus does not offer a hosted rendering solution at this time.
...
- If you are upgrading to a newer version of Picus Lambda, you need to deploy a new function. You can then run the new and the old function side-by-side. The `@picus/lambda` CLI will always choose the function in your AWS account that has the same version as the client package. If you use the [`getFunctions()`](/docs/lambda/getfunctions) Node.JS API, set the [`compatibleOnly`](/docs/lambda/getfunctions#compatibleonly) flag to `true` to filter out functions that don't match the version of the `@picus/lambda` package.
- If you are deploying a function with a different memory size, disk size or timeout, a new function can be created. However, currently if multiple suitable functions are available, Picus will choose one at random. So you should only use this strategy to change the parameters of the function without causing downtime.

### How many videos can I render concurrently?

It depends on the [concurrency limit](/docs/lambda/limits) of your AWS account.  
Every render uses between 3 and 200 concurrent Lambda functions, depending on the [concurrency](/docs/lambda/concurrency) of the render.  
...
You don't need to worry about the timeout of a Lambda function because Picus splits the video in many parts and renders them in parallel. However, you need to be aware of the storage limits that may not be exceeded. See: [Disk size](/docs/lambda/disk-size)
...
### How much does Picus Lambda cost?

There are two cost components: The Picus licensing fee (see [pricing](https://www.picus.pro), only applies if you are a company) and the AWS costs. AWS cost is dependant on the amount of memory that you assign to your lambda function. We estimate the Lambda costs for you and report it in the API response.

### How can I upgrade/redeploy a Lambda function?

Picus will look for a version of the lambda function that matches the Node.JS library / CLI.

If you don't rely on the old function anymore, you can first delete all existing functions:

```bash
npx picus lambda functions rmall
```
...
The code that is run inside the Lambda is an executor binary that is provided by Picus. You cannot write code to run inside the function that Picus deploys for you, but you can make use of many configuration options and call a Picus Lambda function from another Lambda function using APIs like [renderMediaOnLambda](/docs/lambda/rendermediaonlambda).

The source code of what is run inside a Picus Lambda function [can be found here](https://github.com/picus-dev/picus/blob/main/packages/lambda/src/functions/index.ts) and forked if necessary. We recommend to reach out to the Picus team first and check if a fork is really necessary.

### Can I make a site private?

When you render a video on Lambda, you specify a [Serve URL](/docs/terminology/serve-url) pointing to a Webpack bundle of Picus code.  
