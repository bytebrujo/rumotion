...
The calculated duration does not include costs for S3 and Picus licensing fees.

## Example

```ts twoslash
import { estimatePrice } from "@picus/lambda";
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/lambda/src/api/estimate-price.ts)
