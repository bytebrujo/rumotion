...
Creates a bucket for Picus Lambda in your S3 account. If one already exists, it will get returned instead.

**Only 1 bucket per region** is necessary for Picus Lambda to function.

```ts twoslash
import {getOrCreateBucket} from '@picus/lambda';

const {bucketName} = await getOrCreateBucket({region: 'us-east-1'});

console.log(bucketName); // "picuslambda-32df3p"
```
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/lambda/src/api/get-or-create-bucket.ts)
