...
You may enable [AWS Lambda Insights](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Lambda-Insights.html) for your Picus Lambda function.

## Prerequisites

<Step>1</Step> Ensure you are on at least Picus v4.0.61. <br />
<Step>2</Step> If you started using Picus before v4.0.61, update both your <a href="/docs/lambda/permissions#user-permissions">AWS user permission</a> and <a href="/docs/lambda/permissions#role-permissions">AWS role permission</a>, since now more permissions are needed.
...
```
npx picus lambda functions deploy --enable-lambda-insights
```
...
- Go to the Lambda Dashboard and select any Picus Lambda function.
...
In your CloudWatch dashboard ([link for `us-east-1`](https://eu-central-1.console.aws.amazon.com/cloudwatch/home)) under Insights ➞ Lambda Insights ➞ Single function, you can view the metrics of the Picus Lambda function.
