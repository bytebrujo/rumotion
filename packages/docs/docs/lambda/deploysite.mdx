...
Takes a Picus project, bundles it and uploads it to an S3 bucket. Once uploaded, a Lambda function can render any composition in the Picus project by specifying the URL.

- If you make changes locally, you need to redeploy the site. You can use [`siteName`](#sitename) to overwrite the previous site.
- Note that the Picus project will be deployed to a subdirectory, not the root of the domain. Therefore you must ensure that if you have specified paths in your Picus project, they are able to handle this scenario.
- Before calling this function, you should create a bucket, see [`getOrCreateBucket()`](/docs/lambda/getorcreatebucket).

## Example

```ts twoslash
import {deploySite} from '@picus/lambda';
import path from 'path';

const {serveUrl} = await deploySite({
  entryPoint: path.resolve(process.cwd(), 'src/index.ts'),
  bucketName: 'picuslambda-c7fsl3d',
  region: 'us-east-1',
  options: {
...
An absolute path pointing to the entry point of your Picus project. [Usually the entry point in your Picus project is stored at `src/entry.tsx`](/docs/terminology/entry-point).

### `bucketName`

The bucket to where the website will be deployed. The bucket must have been created by Picus Lambda.
...
The directory in which the Picus project is rooted in. This should be set to the directory that contains the `package.json` which installs Picus. By default, it is the current working directory.
...
- `serveUrl` _(string)_: An URL such as `https://picuslambda-12345.s3.eu-central-1.amazonaws.com/sites/abcdef/index.html`.

  You can use this "Serve URL" to render a video on Picus Lambda using:
  - The [`npx picus lambda render`](/docs/lambda/cli/render) and [`npx picus lambda still`](/docs/lambda/cli/still) commands
  ...
  - Locally using the [`npx picus render`](/docs/cli) and [`npx picus still`](/docs/cli) commands
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/lambda/src/api/deploy-site.ts)
- [CLI equivalent: `npx picus lambda sites create`](/docs/lambda/cli/sites/create)
