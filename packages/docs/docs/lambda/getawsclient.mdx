...
This API exposes full access to the AWS SDK that Picus uses under the hood. You can use it to interact with your AWS infrastructure in ways that Picus doesn't provide a function for.

## Example: Getting a buffer for a render

```tsx {19-28}
// Import from "@picus/lambda" instead before Picus v4.0.60
import {getAwsClient, getRenderProgress} from '@picus/lambda/client';
import {Readable} from 'stream';

const bucketName = 'picuslambda-d9mafgx';

const getFileAsBuffer = async () => {
  const progress = await getRenderProgress({
    renderId: 'd7nlc2y',
    bucketName: 'picuslambda-d9mafgx',
...
```tsx
// Import from "@picus/lambda" instead before Picus v4.0.60
import {getAwsClient} from '@picus/lambda/client';
...
One of the [supported regions](/docs/lambda/region-selection) of Picus Lambda, for which the client should be instantiated.
...
:::note
You don't need to create a new client from the SDK and should instead reuse the `client` that is also returned and being used by Picus, in order to save memory.
:::

## See also

- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/lambda/src/api/get-aws-client.ts)
