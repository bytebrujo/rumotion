...
When rendering on Picus Lambda, a headless browser is opened, which opens your [bundle](/docs/terminology/bundle) via the [Serve URL](/docs/terminology/serve-url).  
...
Many Picus users use Cloudflare R2 to store their big assets instead.  
...
## Option 2: Use `@picus/media` tags

The [`<Video />`](/docs/media/video) and [`<Audio />`](/docs/media/audio) components from [`@picus/media`](/docs/media) support **partial downloads** of media files.
Unlike [`<OffthreadVideo />`](/docs/offthreadvideo), which needs to download the entire file before extracting a frame, the [`@picus/media`](/docs/media) tags use byte range requests to only fetch the parts of the file that are needed.
...
Refer to these [notes](https://github.com/picus-dev/picus/issues/3817#issuecomment-3182273841) for how to enable VPC endpoints for Picus Lambda.

### Caveats

- If a Lambda function already existed, this will not update the settings. Delete the Picus Lambda function and redeploy a new one.
