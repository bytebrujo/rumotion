...
Before going live with Picus Lambda, you need to think about into which AWS region you are deploying your function and bucket.
...
You can call [`getRegions()`](/docs/lambda/getregions) or type [`npx picus lambda regions`](/docs/lambda/cli/regions) to get this list programmatically.
...
- When using the CLI, you can set the region using the `PICUS_AWS_REGION` environment variable. It's best to put it in a `.env` file so you don't forget it.

:::note
The variable is called `PICUS_AWS_REGION` because in Cloud providers like Vercel, `AWS_REGION` is a reserved environment variable name. However, Picus does also accept the latter if you use it locally.
:::
...
:::note
The PICUS_AWS_REGION environment variable and `--region` flag do not have an effect when using the Node.JS APIs. You need to pass a region explicitly.
:::

If you don't set a region, Picus will use the default region.
...
Some regions that are supported by Picus are not enabled by default in an AWS account. If you get a message:
