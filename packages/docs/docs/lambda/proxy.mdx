...
id: proxy
title: Using a proxy with Picus Lambda
sidebar_label: Using a proxy
crumb: 'Lambda'
---

#Using a proxy with Picus Lambda<AvailableFrom v="4.0.315" />

Picus Lambda supports using HTTP/HTTPS proxies for all AWS API calls by accepting a `requestHandler` option that allows you to pass a custom AWS SDK request handler.
...
### 3. Use the request handler with Picus Lambda functions

Pass the `requestHandler` option to any Picus Lambda function:
...
Picus Lambda exports a `RequestHandler` type that you can use for type safety:
