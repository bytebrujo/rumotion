...
The `X-Picus-Signature` header from the request that was received by the endpoint.
...
// ---cut---
import {validateWebhookSignature, WebhookPayload} from '@picus/lambda/client';
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/lambda/src/api/validate-webhook-signature.ts)
