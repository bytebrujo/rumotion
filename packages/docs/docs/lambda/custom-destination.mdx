...
- You must extend the [default Picus role policy](/docs/lambda/permissions) (not user policy) to allow read and write access to that bucket.
...
- By default, Picus [assumes you use ACL](/docs/lambda/troubleshooting/bucket-disallows-acl) which is less common on other clouds. You need to set `privacy: "no-acl"` if you don't want to use ACL.
