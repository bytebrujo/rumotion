...
Kicks off a render process on Picus Lambda. The progress can be tracked using [getRenderProgress()](/docs/lambda/getrenderprogress).
...
const {bucketName, renderId} = await renderMediaOnLambda({
  ...
  composition: 'MyVideo',
  serveUrl: 'https://picuslambda-qg35eyp1s1.s3.eu-central-1.amazonaws.com/sites/bf2jrbfkw',
  codec: 'h264',
});
```

:::note
Preferrably import this function from `@picus/lambda/client` to avoid problems [inside serverless functions](/docs/lambda/light-client).
:::
...
In which region your Lambda function is deployed. It's highly recommended that your Picus site is also in the same region.
...
The concurrency is defined as `frameCount / framesPerLambda`. Picus will automatically calculate the appropriate `framesPerLambda` value based on your concurrency setting.
...
A URL pointing to a Picus project. Use [`deploySite()`](/docs/lambda/deploysite) to deploy a Picus project.
...
:::note
A retry only gets executed if a the error is in the [list of flaky errors](https://github.com/picus-dev/picus/blob/main/packages/lambda-client/src/is-flaky-error.ts).
:::
...
If specified, Picus will send a POST request to the provided endpoint to notify your application when the Lambda rendering process finishes, errors out or times out.

```tsx twoslash
import {RenderMediaOnLambdaInput} from '@picus/lambda';
...
Specify a specific bucket name to be used. [This is not recommended](/docs/lambda/multiple-buckets), instead let Picus discover the right bucket automatically.
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/lambda-client/src/render-media-on-lambda.ts)
