...
title: Using the Serverless Framework with Picus Lambda
...
This guide will show you how to use Picus with [Serverless Framework (serverless.com)](https://www.serverless.com/).

To supplement this guide, two projects have been created. The [picus-app](https://github.com/alexfernandez803/picus-serverless/tree/main/picus-app) contains a Picus composition and utility scripts for deploying and deleting Picus Lambda infrastructure in AWS.

The [serverless-app](https://github.com/alexfernandez803/picus-serverless/tree/main/serverless-app) contains a Serverless project that deploys two Lambda functions. The [render_handler](https://github.com/alexfernandez803/picus-serverless/blob/main/serverless-app/render_handler.ts) function, when invoked, will call the deployed Picus Lambda function to render a video. The [progress_handler](https://github.com/alexfernandez803/picus-serverless/blob/main/serverless-app/progress_handler.ts) function tracks the progress of the render.
...
This contains instructions for setting up and installing the `picus` Lambda to your AWS account. This deployment is designed to be executed on your local machine.
...
The project can be found at [`picus-serverless project`](https://github.com/alexfernandez803/picus-serverless).

```bash
git clone https://github.com/alexfernandez803/picus-serverless
```

#### 2. Go to `picus-serverless` and traverse to the `picus-app` directory

```bash
cd picus-serverless && cd picus-app
```
...
The `AWS_KEY_*` and `AWS_SECRET_*` represent the AWS account credentials that are allowed to deploy the Picus Lambda function.

#### 5. Deploy the Lambda function

The project has the deployment script configured in [package.json](https://github.com/alexfernandez803/picus-serverless/blob/main/picus-app/package.json).
...
This will execute the [deploy function](https://github.com/alexfernandez803/picus-serverless/blob/main/picus-app/src/infra/deploy-lambda-fn.ts) that will deploy the Picus Lambda to your AWS account.

```bash title="deployment logs"

Found 1 accounts. Deploying...
Ensured function="picus-render-3-3-78-mem2048mb-disk2048mb-240sec" to region="ap-southeast-2" in account 1
entryPoint /xxxx/code/picus-serverless/picus-app/src/index.tsx
Deployed site to region="ap-southeast-2" in account 1 with bucket="picuslambda-apsoutheast2-xxxxx" under serveUrl="https://picuslambda-apsoutheast2-xxxxx.s3.ap-southeast-2.amazonaws.com/sites/picus-render-app-3.3.78/index.html"
...
This will execute the [delete function](https://github.com/alexfernandez803/picus-serverless/blob/main/picus-app/src/infra/delete-lambda-fn.ts) which removes the Picus Lambda function from your AWS account.

## serverless-app

This contains instructions for setting up and installing Lambda services [render_handler](https://github.com/alexfernandez803/picus-serverless/blob/main/serverless-app/render_handler.ts) and [progress_handler](https://github.com/alexfernandez803/picus-serverless/blob/main/serverless-app/progress_handler.ts) to your AWS account. This guide is designed to be executed on your local machine.
...
#### 2. Go to `picus-serverless` and traverse to `serverless-app` directory

```bash
cd picus-serverless && cd serverless-app
```
...
Each of the function is assigned with a role named `picusLambdaServerlessRole` as both function has the same access patterns to the Picus Lambda.
...
- Under "Permissions policies", filter for `picus-executionrole-policy` and click the checkbox to assign this policy. This `policy` should have been created, if not, follow this [guide](/docs/lambda/without-iam/#1-create-role-policy) in setting this up.
...
- In the final step, name the role `picusLambdaServerlessRole` exactly. You can leave the other fields as is.
- Click "Create role" to confirm.

This steps creates a role named `picusLambdaServerlessRole` with permissions from `picus-executionrole-policy` and `AWSLambdaBasicExecutionRole` which allows the lambda function to create Cloudwatch logs. The role is referenced by the two functions in the [serverless.yml](https://github.com/alexfernandez803/picus-serverless/blob/main/serverless-app/serverless.yml#L78) file, which enables the two Lambda functions to render video and check the progress of the render.
...
dashboard: https://app.serverless.com/changeme/apps/aws-picus-serverless/api-render-video/dev/ap-southeast-2
endpoints:
...
```bash title="response"
{
    ...
    "bucketName": "picuslambda-apsoutheast2-xxxxxxxx"
}
```

This will initiate the render of the video and provide output with the `renderId` and `bucketName`. The code for the Lambda function is located [here](https://github.com/alexfernandez803/picus-serverless/blob/main/serverless-app/render_handler.ts).

#### 2. Get the progress of the render

```bash title="progress"
curl --location --request GET 'https://xxxxxxxx.execute-api.ap-southeast-2.amazonaws.com/dev/render/i9xnfrgXXXX?bucketName=picuslambda-apsoutheast2-xxxxxxxx' \
...
```bash title="response"
{
    ...
    "finality": {
        "type": "success",
        "url": "https://s3.ap-southeast-2.amazonaws.com/picuslambda-apsoutheast2-xxxxxxxx/renders/i9xnfrgXXXX/out.mp4"
    },
    "mediaUrl": "https://picuslambda-apsoutheast2-xxxxxxxx.s3.ap-southeast-2.amazonaws.com/renders/i9xnfrgXXXX/out.mp4?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Content-Sha256=UNSIGNED-PAYLOAD&X-Amz-Credential=ASIAZ3B4C6O75ZTGPMJ4%2F20230128%2Fap-southeast-2%2Fs3%2Faws4_request&X-Amz-Date=20230128T073123Z&X-Amz-Expires=900&X-Amz-Security-Token=IQXXXXXXXXV%2BWIoTQ5CvZXcljmGUIOkllDRsnmrRGNYvY8IVn8FRQmt%2Bc8%2BJQdiG0ShI0y82jB2s%2BbkaPf%2FJNDrSjO5tBo8%2FXwtaP2z9PewUIND1yMm4TkOUMXXXXXn6j&X-Amz-Signature=0881241614cd6c778b1XXXXXX42941c&X-Amz-SignedHeaders=host&x-id=GetObject"
}
```

This API will provide the progress details of the render, indicating whether it is a `success` or `failure`. If the video render is completed, it will provide the `mediaUrl`, which is a pre-signed URL that makes the video downloadable. The code for the Lambda function is located [here](https://github.com/alexfernandez803/picus-serverless/blob/main/serverless-app/progress_handler.ts).
...
- Some code is borrowed from [github-unwrapped-2021](https://github.com/picus-dev/github-unwrapped-2021/tree/main/src)
