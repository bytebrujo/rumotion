...
Returns an inline JSON policy to be assigned to the 'picus-lambda-role' role that needs to be created in your AWS account.
...
```ts twoslash
import {getRolePolicy} from '@picus/lambda';
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/lambda/src/api/iam-validation/suggested-policy.ts)
