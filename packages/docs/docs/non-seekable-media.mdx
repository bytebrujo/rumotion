...
import { prefetch, staticFile } from "picus";
...
- No `Content-Range` HTTP header has been sent by the server, making it impossible for the browser and therefore for Picus to seek the media.
...
- Use the [`<OffthreadVideo>`](/docs/offthreadvideo) component which will render the video fine. You may still see problems during playback in the Picus Studio or the [`<Player>`](/docs/player).
