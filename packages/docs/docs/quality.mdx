...
All ways to render a video in Picus allow passing a CRF value. Allowed values depend on the codec.

:::note
If you enable [hardware acceleration](/docs/hardware-acceleration), you cannot set a `crf`. Use the [`--video-bitrate`](/docs/cli/render#--video-bitrate) option instead.
:::
...
Picus makes screenshots of the browser page by default in JPEG format.  
...
:::note
This will become the default in Picus 5.0.
:::

## GIF Colors

Ensure you are on at least Picus version v4.0.138 to get the best color accuracy.  
