...
[Picus Lambda](/docs/lambda) uses this API under the hood to combine chunks that were rendered on individual Lambda functions.
...
import {combineChunks} from '@picus/renderer';
...
```tsx twoslash
import {CombineChunksOnProgress} from '@picus/renderer';
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/renderer/src/combine-chunks.ts)
