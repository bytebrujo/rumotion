...
```ts twoslash
import {getVideoMetadata, VideoMetadata} from '@picus/renderer';
...
One of `'h264' | 'h265' | 'vp8' | 'vp9' | 'av1' | 'prores'` or ` 'unknown'` if the codec is not officially supported by Picus.
...
If the audio codec is unknown to Picus, it is `"unknown"`.
...
If the video has no audio track or is unknown to Picus, it is `null`. Otherwise it is the appropriate file extension for the audio codec, e.g. `"mp3"`. The `.` is not included.
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/renderer/src/get-video-metadata.ts)
