...
```ts title="ensure.mjs"
import {ensureFfmpeg} from '@picus/renderer';

await ensureFfmpeg();
```

You might not need to call this function. Picus will automatically download `ffmpeg` if a render is attempted, and no binary was found.

This function is useful if you need FFmpeg to be ready before the first render is started.

Also call [`ensureFfprobe()`](/docs/renderer/ensure-ffprobe) to get both binaries that Picus requires.
...
- CLI equivalent: [`npx picus install ffmpeg`](/docs/cli/install)
