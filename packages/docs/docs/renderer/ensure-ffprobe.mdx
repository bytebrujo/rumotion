...
```ts title="ensure.mjs"
import {ensureFfprobe} from '@picus/renderer';

await ensureFfprobe();
```

You might not need to call this function. Picus will automatically download `ffprobe` if a render is attempted, and no binary was found.

This function is useful if you need `ffprobe` to be ready before the first render is started.

Also call [`ensureFfmpeg()`](/docs/renderer/ensure-ffmpeg) to get both binaries that Picus requires.
...
- CLI equivalent: [`npx picus install ffprobe`](/docs/cli/install)
