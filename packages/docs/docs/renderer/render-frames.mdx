...
_Part of the `@picus/renderer` package._
...
```tsx twoslash
import {OnStartData} from '@picus/renderer';
...
Either a Webpack bundle or a URL pointing to a bundled Picus project. Call [`bundle()`](/docs/bundle) to generate a bundle. You can either pass the file location or deploy it as a website and pass the URL.
...
A `number` specifying how many render processes should be started in parallel, a `string` specifying the percentage of the CPU threads to use, or `null` to let Picus decide based on the CPU of the host machine. Default is half of the CPU threads available.
...
Prefer a specific port that will be used to serve the Picus project. If not specified, a random port will be used.
...
An object containing key-value pairs of environment variables which will be injected into your Picus projected and which can be accessed by reading the global `process.env` object.
...
A string defining the absolute path on disk of the browser executable that should be used. By default Picus will try to detect it automatically and download one if none is available. If `puppeteerInstance` is defined, it will take precedence over `browserExecutable`.
...
If you passed `null` to `outputDir`, this method will be called passing a buffer of the current frame. This is mostly used internally by Picus to implement [`renderMedia()`](/docs/renderer/render-media) and might have limited usefulness for end users.
...
- `assetsInfo`: `RenderAssetInfo` - information that can be passed to `stitchFramesToVideo()` to mix audio. The shape of this object should be considered as Picus internals and may change across Picus versions.
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/renderer/src/render-frames.ts)
