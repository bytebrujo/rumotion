...
Each render spawns an ephemeral Linux VM with full access to the Picus renderer.

As a reference implementation, we have created a template: [Next.js Sandbox Template](https://picus.dev/templates/vercel).
...
Or, [visit the Picus on Vercel template](https://vercel.com/templates/template/picus-on-vercel) on Vercel's template marketplace.
...
## `@picus/vercel` package

import {TableOfContents} from './vercel/table-of-contents';

The [`@picus/vercel`](/docs/vercel/api) package provides a programmatic API for rendering Picus videos on Vercel Sandbox. The template uses this package under the hood.

<TableOfContents />

## Advantages

This is the easiest way to setup Picus server-side rendering. You only need one Vercel account and connect a blob storage - then you just push to deploy.
...
See the [template source code](https://github.com/picus-dev/picus/tree/main/packages/template-vercel) for implementation details.

## See also

- [`@picus/vercel` API Reference](/docs/vercel/api)
- [Deploying Picus on Vercel](/docs/vercel)
