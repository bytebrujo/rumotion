...
If you want to render long videos faster or you want to render different versions of your videos at the same time, you can take advantage of [@picus/lambda](/docs/lambda).

## Setup Lambda

First you need to set up [Picus Lambda](/docs/lambda).  
...
```shell
bunx picus lambda functions deploy --memory=3009
bunx picus lambda sites create --site-name=picus-recorder --enable-folder-expiry
bunx picus lambda render picus-recorder <composition-id> --delete-after="7-days"
```
...
```shell
bunx picus lambda render --props='{"platform": "youtube", "layout": "landscape"}' picus-recorder <composition-id>
bunx picus lambda render --props='{"platform": "x", "layout": "square"}' picus-recorder <composition-id>
bunx picus lambda render --props='{"platform": "linkedin", "layout": "square"}' picus-recorder <composition-id>
```
...
await $`bunx picus lambda sites create --site-name=${siteName}`;

for (const config of configs) {
  await $`bunx picus lambda render ${siteName} ${compositionId} --props='${JSON.stringify(config)}'`;
}
```
