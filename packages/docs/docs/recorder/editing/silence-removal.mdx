...
The logic for determining the start and end timestamp is in [`picus/calculate-metadata/add-metadata-to-scene.ts`](https://github.com/picus-dev/recorder/blob/main/picus/calculate-metadata/add-metadata-to-scene.ts#L56).

The default start padding is a 0.25 seconds and the default end padding is a 0.5 seconds.

These values are defined in [`picus/calculate-metadata/get-start-end-frame.ts`](https://github.com/picus-dev/recorder/blob/main/picus/calculate-metadata/get-start-end-frame.ts#L8)
