...
:::warning
From v4.0 on, this warning does not appear anymore and video frame extraction should always be fast. The information in this document only applies to older versions of Picus and is preserved for people who are still using them.
:::
...
- If a H.264 video in a MP4 container has it's timestamps corrupted so that it cannot be seeked to the exact frame that Picus wants to extract.
  - **Recommendation**: It might be faster to first re-encode the video using FFMPEG to fix the seeking: `npx picus ffmpeg -i inputvideo.mp4 outputvideo.mp4`
