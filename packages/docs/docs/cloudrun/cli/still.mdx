...
id: still
sidebar_label: still
title: 'npx picus cloudrun still'
...
Using the `npx picus cloudrun still` command, you can render an image on GCP.

The structure of a command is as follows:

```
npx picus cloudrun still <serve-url> [<still-id>] [<output-location>]
```
...
```
npx picus cloudrun still https://storage.googleapis.com/picuscloudrun-123asd321/sites/abcdefgh/index.html tiles --service-name=picus--3-3-82--mem512mi--cpu1-0--t-800
```

Using the site name as opposed to the full serve-url:

```
npx picus cloudrun still test-site tiles --service-name=picus--3-3-82--mem512mi--cpu1-0--t-800
```

Passing in input props:

```
npx picus cloudrun still test-site tiles --service-name=picus--3-3-82--mem512mi--cpu1-0--t-800 --props='{"hi": "there"}'
```
