---
image: /generated/articles-docs-cloudrun-cli-services-rm.png
sidebar_label: rm
title: 'npx picus cloudrun services rm'
slug: /cloudrun/cli/services/rm
crumb: 'Cloud Run CLI Reference'
---

<ExperimentalBadge>
  <p>
    Cloud Run is in <a href="/docs/cloudrun/status">Alpha status and not actively being developed.</a>
  </p>
</ExperimentalBadge>

```
npx picus cloudrun services rm picus--3-3-82--mem512mi--cpu1-0--t-800
```

Removes one or more services from your GCP project. Pass a space-separated list of services you'd like to delete.

<details>
<summary>
Example output
</summary>
<pre>
<br/>
Service name:      picus--3-3-82--mem2gi--cpu1-0--t-800<br/>
Version:           3.3.82<br/>
CPU Limit:         1.0<br/>
Memory Limit:      2Gi<br/>
Timeout:           300sec<br/>
Region:            us-east1<br/>
Service URL:       https://picus--3-3-82--mem2gi--cpu1-0--t-800-1a2b3c4d5e-ue.a.run.app<br/>
GCP Console URL:   https://console.cloud.google.com/run/detail/us-east1/picus--3-3-82--mem2gi--cpu1-0--t-800/logs
Delete? (Y/n):  Y<br/>
Deleted!
<br/>

</pre>
</details>

## `--region`

The [GCP region](/docs/cloudrun/region-selection) to select.

## `--yes`, `-y`

Skips confirmation.

## See also

- [Setup guide](/docs/cloudrun/setup)
- [Cloud Run services](/docs/cloudrun/cli/services)
