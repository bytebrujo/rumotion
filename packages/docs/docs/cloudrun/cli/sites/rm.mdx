---
image: /generated/articles-docs-cloudrun-cli-sites-rm.png
sidebar_label: rm
title: 'npx picus cloudrun sites rm'
slug: /cloudrun/cli/sites/rm
crumb: 'Cloud Run CLI Reference'
---

<ExperimentalBadge>
  <p>
    Cloud Run is in <a href="/docs/cloudrun/status">Alpha status and not actively being developed.</a>
  </p>
</ExperimentalBadge>

```bash
npx picus cloudrun sites rm central-site
npx picus cloudrun sites rm central-site another-site # multiple at once
```

Removes a site (or multiple) from Cloud Storage by it's ID.

<details>
  <summary>Example output</summary>
  <pre>
    Site: central-site
    <br />
    Bucket: picuscloudrun-abcdefgh
    <br />
    Region: us-central1
    <br />
    Serve Url: https://storage.googleapis.com/picuscloudrun-abcdefgh/sites/central-site/index.html
    <br />
    <br />
    Delete? (Y/n) Y<br />
    Deleted site central-site from bucket picuscloudrun-abcdefgh.
    <br />
  </pre>
</details>

## `--region`

The [GCP region](/docs/cloudrun/region-selection) to remove sites from.

:::note
The `rm` command does not support the --all-regions flag, as it is possible to have the same site name in multiple regions. This makes it difficult to remove multiple site-names from multiple regions.
:::

## `--yes`, `-y`

Removes a site (or multiple) without asking for confirmation.

```
npx picus cloudrun sites rm central-site -y
```

## See also

- [Setup guide](/docs/cloudrun/setup)
- [Cloud Run sites](/docs/cloudrun/cli/sites)
