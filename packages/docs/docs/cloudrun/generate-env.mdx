...
It is important to have strict control over the service account keys, as they are used to authenticate the Picus project to GCP. The .env file should not be committed to source control, and not shared with anyone. Any time you no longer require a service account key, it should be deleted.
...
1. Open the [Project Dashboard](https://console.cloud.google.com/home/dashboard) in the GCP console, and select your existing Picus project.
...
3. Within the Cloud Shell, type the following command and follow the prompts.
   ...
   _The first command downloads a tar file from the Picus repo, and extracts it to the current directory. The second command runs the installer script._
   If you want to generate a new .env file, or manage keys already created, you will want to select option 3.

   If this is the first time [initialising Picus in the GCP project](/docs/cloudrun/setup), you will want to select option 1.  
   If you are [updating the version of Picus for this GCP project](/docs/cloudrun/setup), you will want to select option 1.

   <!-- ToDo - host this in the official Picus repo -->
...
```
npx picus cloudrun policies validate
```
