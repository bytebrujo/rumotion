...
Takes a Picus project, bundles it and uploads it to an Cloud Storage bucket. Once uploaded, a Cloud Run service can render any composition in the Picus project by specifying the URL.
...
```ts twoslash
import {deploySite} from '@picus/cloudrun';
import path from 'path';

const {serveUrl} = await deploySite({
  entryPoint: path.resolve(process.cwd(), 'src/index.ts'),
  bucketName: 'picuscloudrun-c7fsl3d',
  options: {
...
The bucket to where the website will be deployed. The bucket must have been created by Picus Cloud Run.
...
An URL such as `https://storage.googleapis.com/picuscloudrun-123asd321/sites/abcdefgh/index.html`.

You can use this "Serve URL" to render a video on Picus Cloud Run using:

- The [`npx picus cloudrun render`](/docs/cloudrun/cli/render) command
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/cloudrun/src/api/deploy-site.ts)
- [CLI equivalent: `npx picus cloudrun sites create`](/docs/cloudrun/cli/sites/create)
