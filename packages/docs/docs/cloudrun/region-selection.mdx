...
Before going live with Picus Cloud Run, you need to think about into which GCP region you are deploying your service and bucket (which will hold your sites and renders).
...
You can call [`getRegions()`](/docs/cloudrun/getregions) or type [`npx picus cloudrun regions`](/docs/cloudrun/cli/regions) to get this list programmatically.
...
- When using the CLI, you can set the region using the `PICUS_GCP_REGION` environment variable. It's best to put it in a `.env` file.
...
:::info
The PICUS_GCP_REGION environment variable and `--region` flag do not have an effect when using the Node.JS APIs. You need to pass a region explicitly.
:::
