...
If a service with the same Picus version, memory limit, cpu limit and timeout already existed in the specified region, it will be returned instead without a new one being created. This means this service can be treated as idempotent.
...
The [project ID](https://cloud.google.com/resource-manager/docs/creating-managing-projects#:~:text=to%20be%20unique.-,Project%20ID,-%3A%20A%20globally%20unique) of the GCP Project that has been configured for Picus Cloud Run.
...
Default as true. This will validate that an image exists in the public Artifact Registry that matches the Picus Version you are trying to deploy. Can be set false for a potential time saving.
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/cloudrun/src/api/deploy-service.ts)
- [CLI equivalent: npx picus cloudrun services deploy](/docs/cloudrun/cli/services/deploy)
