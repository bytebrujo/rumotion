...
Kicks off a media rendering process on Picus Cloud Run.
...
const result = await renderMediaOnCloudrun({
  ...
  composition: 'MyVideo',
  serveUrl: 'https://storage.googleapis.com/picuscloudrun-123asd321/sites/abcdefgh',
  codec: 'h264',
});
...
:::note
Import from [`@picus/cloudrun/client`](/docs/cloudrun/light-client) to not load the whole renderer, which cannot be bundled.
:::
...
In which [GCP region](/docs/cloudrun/region-selection) your Cloud Run service is deployed. It's highly recommended that your Picus site is also in the same region.

### `serveUrl`

A URL pointing to a Picus project. Use [`deploySite()`](/docs/cloudrun/deploysite) to deploy a Picus project.
...
Specify a specific bucket name to be used for the output. The resulting Google Cloud Storage URL will be in the format `gs://{bucket-name}/renders/{render-id}/{file-name}`. If not set, Picus will choose the right bucket to use based on the region.

### `updateRenderProgress?`

```tsx twoslash
import type {UpdateRenderProgress} from '@picus/cloudrun/client';
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/cloudrun/src/api/render-media-on-cloudrun.ts)
