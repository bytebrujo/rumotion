...
## 1. Install `@picus/cloudrun`

<Installation pkg="@picus/cloudrun" />
...
2. Within the Cloud Shell, type the following command and follow the prompts.
   ...
   ```bash
   curl -L https://github.com/picus-dev/picus/raw/main/packages/cloudrun/src/gcpInstaller/gcpInstaller.tar | tar -x -C . && node install.mjs
   ```

- Select <Step>1</Step>to setup this project for Picus Cloud Run, or to run the setup again for an update.
...
From within your code base, run the following command to validate the permissions are setup correctly in GCP. As long as your GCP project was setup with a matching Picus version, this should pass.

```
npx picus cloudrun permissions
```
...
```ts twoslash
import {deployService} from '@picus/cloudrun';

// ---cut---
const deployResult = await deployService({
  projectID: 'my-picus-project',
  region: 'us-east1',
});
```
...
The service consists of necessary binaries and JavaScript code that can take a [serve URL](/docs/terminology/serve-url) and make renders from it. A service is bound to the Picus version, if you upgrade Picus, you [need to deploy a new service](/docs/cloudrun/upgrading). A service does not include your Picus code, it will be deployed in the next step instead.
...
Run the following command to deploy your Picus project to a Cloud Storage bucket. Pass as the last argument the [entry point](/docs/terminology/entry-point) of the project.

```bash
npx picus cloudrun sites create src/index.ts --site-name=my-video
```
...
import {deploySite, getOrCreateBucket} from '@picus/cloudrun';
...
Next, upload your Picus project to a Cloud Storage bucket. Specify the entry point of your Picus project, this is the file where [`registerRoot()`](/docs/register-root) is called.
...
When you update your Picus code in the future, redeploy your site. Pass the same [`siteName`](/docs/cloudrun/deploysite#sitename) to overwrite the previous deploy. If you don't pass [`siteName`](/docs/cloudrun/deploysite#sitename), a unique URL will be generated on every deploy.
...
```bash
npx picus cloudrun render <serve-url | site-name> <composition-id> --cloud-run-url=<cloud-run-url>
```
...
```bash
npx picus cloudrun render <serve-url | site-name> <composition-id> --service-name=<service-name> --region=<region>
```

</TabItem>
</Tabs>

Progress will be printed until the video finished rendering. Congrats! You rendered your first video using Picus Cloudrun 🚀
...
```bash
npx picus cloudrun still <serve-url | site-name> <still-id> --cloud-run-url=<cloud-run-url>
```
...
```bash
npx picus cloudrun still <serve-url | site-name> <still-id> --service-name=<service-name> --region=<region>
```

</TabItem>
</Tabs>

Congrats! You rendered your first still using Picus Cloudrun 🚀
...
```ts twoslash
import {getServices, renderMediaOnCloudrun} from '@picus/cloudrun/client';
...
Congrats! [Check your Cloud Storage Bucket](https://console.cloud.google.com/storage/browser) - you just rendered your first video using Picus Cloud Run 🚀
...
```ts twoslash
import {getServices, renderStillOnCloudrun} from '@picus/cloudrun';
...
```ts twoslash
import {renderStillOnCloudrun} from '@picus/cloudrun/client';
...
Congrats! [Check your Cloud Storage Bucket](https://console.cloud.google.com/storage/browser) - you just rendered your first still using Picus Cloud Run 🚀
...
- Select [which region(s)](/docs/cloudrun/region-selection) you want to run Picus Cloud Run in.
- Familiarize yourself with the CLI and the Node.JS APIs (list in sidebar).
- Learn how to [upgrade Picus Cloud Run](/docs/cloudrun/upgrading).
