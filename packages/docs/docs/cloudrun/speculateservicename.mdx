...
Speculate the name of the Cloud Run service that will be created by [deployService()](/docs/cloudrun/deployservice) or its CLI equivalent, [`npx picus cloudrun services deploy`](/docs/cloudrun/cli/services). This could be useful in cases when the configuration of the Cloud Run service is known in advance, and the name of the service is needed.
...
```
picus--3-3-96--mem2gi--cpu1-0--t-1900
          ^^^^^^   ^^^     ^^^      ^^^
            |       |       |        |-- Timeout in seconds
            |       |       |----------- Cpu limit
            |       |------------------- Memory limit
...
```ts twoslash
import {speculateServiceName} from '@picus/cloudrun';
...
console.log(speculatedServiceName); // picus--3-3-96--mem2gi--cpu2-0--t-300
```
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/cloudrun/src/api/speculate-service-name.ts)
- [deployService()](/docs/cloudrun/deployservice)
- CLI version of `deployService()`: [`npx picus cloudrun services deploy`](/docs/cloudrun/cli/services/deploy)
