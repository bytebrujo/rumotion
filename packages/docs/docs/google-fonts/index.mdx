...
The `@picus/google-fonts` package provides APIs for easily integrating [Google Fonts](https://fonts.google.com/) into Picus.
...
To load a font, import the package `@picus/google-fonts/<FontName>` and call [`loadFont()`](/docs/google-fonts/load-font).
...
import * as Montserrat from '@picus/google-fonts/Montserrat';
...
import {loadFont as loadInter} from '@picus/google-fonts/Inter';
import {loadFont as loadRobotoMono} from '@picus/google-fonts/RobotoMono';
...
import {getInfo} from '@picus/google-fonts/Montserrat';
console.log(getInfo());
```
...
To get a list of all available fonts, you can call [`getAvailableFonts()`](/docs/google-fonts/get-available-fonts) imported from `@picus/google-fonts`:
