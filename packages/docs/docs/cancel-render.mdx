...
By invoking this function, Picus will stop the current render, and not perform any retries.
...
`cancelRender()` can be imported from the [`picus`](/docs/picus) package, but preferrably, you should use the [`useDelayRender()`](/docs/use-delay-render) (from v3.0.374) hook instead, because it future-proofs your code for [browser rendering](/docs/miscellaneous/render-in-browser).
...
import {delayRender, continueRender, cancelRender} from 'picus';
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/core/src/cancel-render.ts)
