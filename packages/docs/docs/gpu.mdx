...
Some types of content in Picus can benefit from a GPU being available on the machine that is used for rendering.  
...
If a GPU is available, it should be enabled by default while in the Picus Studio or Picus Player.  
