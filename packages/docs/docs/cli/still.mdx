...
```bash
npx picus still <serve-url|entry-point>? [<composition-id>] [<output-location>]
```
...
If `composition-id` is also not passed, Picus will let you select a composition.
