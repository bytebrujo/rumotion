...
```bash
npx picus install ffmpeg
```

```bash
npx picus install ffprobe
```

You might not need to call this function. Picus will automatically download `ffmpeg` and `ffprobe` if a render is attempted, and no binary was found.
