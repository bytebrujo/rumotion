...
```bash
npx picus render <entry-point|serve-url>? <composition-id> <output-location>
```
...
If `composition-id` is not passed, Picus will ask you to select a composition.  
