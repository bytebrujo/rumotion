...
title: npx picus studio
sidebar_label: studio
crumb: CLI Reference
---

_Alias: npx picus preview_

Start the [Picus Studio](/docs/studio).

```bash
npx picus studio <entry-point>?
```
...
```sh
npx picus studio --browser-args="--disable-web-security"
```
...
```sh
npx picus studio --beep-on-finish
```
...
```sh
npx picus studio --number-of-shared-audio-tags=5
```
...
```sh
npx picus studio --enable-cross-site-isolation
```
...
```sh
npx picus studio --disable-ask-ai
```
...
```sh
npx picus studio --public-license-key="your-license-key"
```
