...
Picus ships with [it's own Webpack configuration](https://github.com/picus-dev/picus/blob/main/packages/bundler/src/webpack-config.ts).
...
In your [`picus.config.ts`](/docs/config) file, you can call [`Config.overrideWebpackConfig()`](/docs/config#overridewebpackconfig) from `@picus/cli/config`.
...
From Picus v4.0.229, you can also use `Config.overrideWebpackConfig` multiple times, but this only works in the config file:
...
The easiest way is to use the `@picus/enable-scss`.  
    ...
    module: {
      ...
      rules: [
        ...
        ...(currentConfiguration.module?.rules ?? []).map((r) => {
          ...
          return {
            ...r,
            // Remove Picus loading SVGs as a URL
...
3. Add the override function to your [`picus.config.ts`](/docs/config) file:
...
```ts twoslash title="picus.config.ts - synchronous"
import {Config} from '@picus/cli/config';
...
:::note
Since Webpack does not allow synchronous WebAssembly code in the main chunk, you most likely need to declare your composition using [`lazyComponent`](/docs/composition#example-using-lazycomponent) instead of `component`. Check out a [demo project](https://github.com/picus-dev/id3-tags) for an example.
:::

```ts twoslash title="picus.config.ts - asynchronous"
import {Config} from '@picus/cli/config';
...
## Importing ES Modules in picus.config.ts<AvailableFrom v="4.0.117"/>
