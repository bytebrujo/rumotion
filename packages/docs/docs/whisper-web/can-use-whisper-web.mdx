...
Checks if the current browser environment supports running `@picus/whisper-web` with a specified model.  
...
```tsx twoslash
import {canUseWhisperWeb, type WhisperWebModel} from '@picus/whisper-web';
...
- **Cross-Origin Isolation:** For `SharedArrayBuffer` to work, which is required by `@picus/whisper-web`, the page must be served with specific HTTP headers:
...
- [Source code for `canUseWhisperWeb()`](https://github.com/picus-dev/picus/blob/main/packages/whisper-web/src/can-use-whisper-web.ts)
