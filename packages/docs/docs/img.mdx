...
If you use `<Img>`, Picus will ensure that the image is loaded before rendering the frame. That way you can avoid flickers if the image does not load immediately during rendering.
...
Picus inherits the props of the regular `<img>` tag, like for example `style`.

## GIFs

Don't use the `<Img>` tag for GIFs, use [`@picus/gif`](/docs/gif) instead.
...
- The maximum resolution that Chrome can display is `2^29` pixels (539 megapixels) <sup><a href="https://stackoverflow.com/questions/57223559/what-is-the-maximum-image-dimensions-supported-in-desktop-chrome#:~:text=than%202%5E29-,(539MP)">[source]</a></sup>. Picus inherits this restriction.
...
- [Source code for this component](https://github.com/picus-dev/picus/blob/main/packages/core/src/Img.tsx)
