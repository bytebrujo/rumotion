...
This example uses [`@aws-sdk/s3-request-presigner`](https://github.com/aws/aws-sdk-js-v3/tree/main/packages/s3-request-presigner) and [the AWS SDK imported from `@picus/lambda`](/docs/lambda/getawsclient). By calling the function below, two URLs are generated:
...
import {AwsRegion, getAwsClient} from '@picus/lambda/client';
...
export const generatePresignedUrl = async (contentType: string, contentLength: number, expiresIn: number, bucketName: string, region: AwsRegion): Promise<{presignedUrl: string; readUrl: string}> => {
  ...
  const {client, sdk} = getAwsClient({
    region: process.env.PICUS_AWS_REGION as AwsRegion,
    service: 's3',
  });
...
- The AWS SDK gets imported using [getAwsClient()](/docs/lambda/getawsclient). If you don't use Picus Lambda, install the [`@aws-sdk/client-s3`](https://github.com/aws/aws-sdk-js-v3/tree/main/clients/client-s3) package directly.
...
export const POST = async (request: Request) => {
  if (!process.env.PICUS_AWS_BUCKET_NAME) {
    throw new Error('PICUS_AWS_BUCKET_NAME is not set');
  }

  if (!process.env.PICUS_AWS_REGION) {
    throw new Error('PICUS_AWS_REGION is not set');
  }
  ...
  const {presignedUrl, readUrl} = await generatePresignedUrl({
    ...
    bucketName: process.env.PICUS_AWS_BUCKET_NAME as string,
    region: process.env.PICUS_AWS_REGION as AwsRegion,
  });
