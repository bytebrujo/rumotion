...
[Picus Lambda](/docs/lambda) is our recommended solution if you are looking for a way to split up renders into chunks that run on different machines.
...
- Bundling Chrome, Picus binaries, necessary fonts and a custom Emoji font so they fit in a Lambda function.
...
[Picus Lambda](/docs/lambda) is following the same blueprint as well.
