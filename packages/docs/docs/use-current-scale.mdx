...
import {Sequence, useCurrentScale} from 'picus';
...
If you are outside of a Picus context, the hook will throw an error.  
If you want to avoid this error and return a default scale, you can pass an options object with the property `dontThrowIfOutsideOfPicus` set to `true`.  
...
const MyRegularReactComponent = () => {
  const scale = useCurrentScale({dontThrowIfOutsideOfPicus: true});
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/core/src/use-current-scale.ts)
