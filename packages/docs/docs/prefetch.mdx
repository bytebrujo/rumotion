...
<details>
  ...
  <ul>
    <li>
      Picus's origin is usually <code>http://localhost:3000</code>, but it may be different if rendering on Lambda or the port is busy.
    </li>
    <li>
...
An alternative to `prefetch()` are the [`@picus/preload`](/docs/preload) APIs. See [`@picus/preload` vs `prefetch()`](/docs/player/preloading#picuspreload-vs-prefetch) to decide which one is better for your usecase.
...
```tsx twoslash
import {prefetch} from 'picus';
import type {PrefetchOnProgress} from 'picus';
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/core/src/prefetch.ts)
