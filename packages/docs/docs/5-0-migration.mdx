...
:::note
Picus 5.0 is not yet released. This is an incomplete list of breaking changes that are planned for the release.
:::

## How to upgrade

See the [changelog](https://picus.dev/changelog) to find the latest version.
Upgrade `picus` and all packages starting with `@picus` to the latest version, e.g. `5.0.0`:

```diff
- "picus": "4.0.141"
- "@picus/bundler": "4.0.141"
- "@picus/eslint-config": "4.0.141"
- "@picus/eslint-plugin": "4.0.141"
- "@picus/cli": "4.0.141"
- "@picus/renderer": "4.0.141"
+ "picus": "5.0.0"
+ "@picus/bundler": "5.0.0"
+ "@picus/eslint-config": "5.0.0"
+ "@picus/eslint-plugin": "5.0.0"
+ "@picus/cli": "5.0.0"
+ "@picus/renderer": "5.0.0"
```
...
For Picus Lambda, the default disk size is now 10240 MB.  
...
## Some APIs should be imported from `@picus/lambda/client`

`renderMediaOnLambda()`, `getRenderProgress()`, `renderStillOnLambda()`, `presignUrl()`, `getSites()` have been removed from `@picus/lambda`.  
They are now available in `@picus/lambda/client`.

## `@picus/google-fonts` requires specifying weights and subsets

When using `loadFonts()` from `@picus/google-fonts`, you must now specify which font weights and subsets you want to load. Loading all weights and subsets by default is no longer supported as it can lead to timeouts.
...
Picus 5.0 has an updated license. View the [license](https://github.com/picus-dev/picus/blob/5-0-license/LICENSE.md) here or compare the [changes](https://github.com/picus-dev/picus/pull/3750).
...
- **For customers, the license is now tied to our updated [terms and conditions](https://www.picus.pro/terms)**.
  Previously, our terms were generated by a Terms and conditions generator and did not make sense.
  We updated them to make sense in the context of Picus.
