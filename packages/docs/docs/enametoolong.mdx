...
This error occurs if Picus is used on Windows and too many audio layers are in the video so that an FFmpeg command gets generated that is longer than the maximum allowed Windows command length (8192 characters).

Unfortunately, FFmpeg does not have any other alternative way to specify a massive amount of inputs, so Picus can not fix this issue. Here are some recommendations instead:
