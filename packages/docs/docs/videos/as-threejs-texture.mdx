...
## Using `@picus/media`<AvailableFrom v="4.0.387" />
...
- The above example can be found in the [Picus testbed](https://github.com/picus-dev/picus/tree/main/packages/example/src/VideoTexture.tsx).

## Using `<OffthreadVideo>`

_deprecated in favor of using `@picus/media`_

You can use the [`useOffthreadVideoTexture()`](/docs/use-offthread-video-texture) hook from [`@picus/three`](/docs/three-canvas) to get a texture from a video.
...
- It creates a new texture for each frame, which is less efficient than using `@picus/media`
...
You can use the [`useVideoTexture()`](/docs/use-video-texture) hook from [`@picus/three`](/docs/three-canvas) to get a texture from a video.
