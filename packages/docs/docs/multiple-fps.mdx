...
```ts twoslash
import {useVideoConfig, interpolate, useCurrentFrame} from 'picus';
const frame = useCurrentFrame();
// ---cut---
// Animate from second 1 to second 2
...
import {interpolate, useCurrentFrame, Sequence, useVideoConfig} from 'picus';
const frame = useCurrentFrame();
...
```tsx twoslash
import {Composition, useCurrentFrame, useVideoConfig} from 'picus';
