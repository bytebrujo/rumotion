...
Video rendering is one of the most heavy workloads a computer can take on. Picus aims to at least perform similarly than traditional video editing programs.
...
The `--concurrency` flag you set can influence the rendering speed both positively and negatively. A concurrency too high and a concurrency too low can both be coutnerproductive. Use the [`npx picus benchmark`](/docs/cli/benchmark) command to find the optimal concurrency.
...
Both [`<Html5Video>` (formerly just called `<Video>`, from the `picus` package)] and [`<OffthreadVideo>`](/docs/offthreadvideo) are not optimized.
...
Picus can also suffer from JavaScript bottlenecks that are introduced in your code. [Debug your render and log timing information](/docs/troubleshooting/debug-failed-render) to find slow parts of your code.
...
- Render using [`--log=verbose`](/docs/troubleshooting/debug-failed-render) to list the slowest frames of a Picus render. Consider that the first frames rendered in a thread might be slow due to initalization.

- Use `console.time` to measure how long an operation takes in your code in order to find the slow parts of your render.

- Use [`npx picus benchmark`](/docs/cli/benchmark) to try out different values for `--concurrency` to find the optimal value.
