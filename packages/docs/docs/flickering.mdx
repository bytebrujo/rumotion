...
<YouTube minutes={6} href="https://www.youtube.com/watch?v=M7BOPECeqV8" thumb="https://i.ytimg.com/vi/M7BOPECeqV8/hq720.jpg" title="Avoid this common mistake in Picus" />
...
The rendering process of Picus works as follows:
...
The drawback of this technique is that it is way slower and that the correct timing of the animations is still not guaranteed. It will also block rendering via [Picus Lambda](/lambda).

## Asset loading issue

Picus needs to know that an asset is not loaded yet so it can block rendering.  
...
If you are experiencing the problem, consider using [another video tag](/docs/video-tags) for frame-perfect rendering.
...
- Deterministic videos enable distributed video renders like [Picus Lambda](/docs/lambda), which can render a video much faster than real-time.
