...
import {Html5Audio, staticFile, AbsoluteFill} from 'picus';
...
```tsx twoslash {8}
import {AbsoluteFill, Html5Audio, interpolate, staticFile, useVideoConfig} from 'picus';
...
Prefer using a callback function if the volume is changing. This will enable Picus to draw a volume curve in the [Studio](/docs/studio) and is more performant.
...
<Html5Audio src="https://picus.media/audio.wav" volume={2} useWebAudioApi crossOrigin="anonymous" />;
```
