...
<YouTube
  ...
  title="Integrate Picus Lambda into your app"
/>

Render Picus videos on [AWS Lambda](https://aws.amazon.com/lambda/). This is the fastest and most scalable way to render Picus videos.  
...
- A Picus project gets deployed to a S3 bucket as a website.
- The Lambda function gets invoked and opens the Picus project.
...
See in more detail: [How Picus Lambda works](/docs/lambda/how-lambda-works)

## Architecture

- **Lambda function**: Requires a layer with Chromium, currently hosted by Picus. Only one Lambda function is required, but it can execute different actions.
- **S3 bucket**: Stores the projects, the renders, and render metadata.
- **CLI**: Allows to control the overall architecture from the command line. Is installed by adding `@picus/lambda` to a project.
...
The following regions are available for Picus Lambda:
...
Most of our users render multiple minutes of video for just a few pennies. The exact cost is dependent on the region, assigned memory, type of video, parallelization and other parameters. For each render, we estimate a cost and display it to you. You might also need a Picus license (see below).

## AWS permissions

Picus Lambda requires you to create an AWS account and create a user with some permissions attached to it. We require only the minimal amount of permissions required for operating Picus Lambda.

[**Read more about permissions**](/docs/lambda/permissions)

## CLI

You can control Picus Lambda using the `npx picus lambda` command.
...
We make it easy to remove all Picus resources from your AWS account without leaving any traces or causing further costs.

[**How to uninstall Picus Lambda**](/docs/lambda/uninstall)
