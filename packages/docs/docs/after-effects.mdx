...
If you are a After Effects user, you might find it useful to convert your After Effects compositions to Picus compositions. You can use the [`@picus/lottie`](/docs/lottie) package for this.

:::note
Picus compositions got their name because After Effects coined this term!
:::
...
### Import the file into Picus

Copy the file into the Picus project. The recommended way is to put the JSON inside the `public/` folder of Picus (create it if necessary) and then load it using [`staticFile()`](/docs/staticfile):

```tsx twoslash title="Animation.tsx"
import {Lottie, LottieAnimationData} from '@picus/lottie';
import {useEffect, useState} from 'react';
import {cancelRender, continueRender, delayRender, staticFile} from 'picus';
...
It is advised to make your composition [the same size and duration](/docs/lottie/getlottiemetadata) as the original composition in After Effects. Congrats, you're playing an After Effects animation in Picus! 🎉
