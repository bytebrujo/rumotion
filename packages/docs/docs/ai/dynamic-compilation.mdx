...
crumb: 'AI'
title: Just-in-time compilation of Picus code
...
If you have generated a Picus component as a string (for example [using LLMs](/docs/ai/generate)), you can compile it in the browser to display a live preview.
...
Picus components need access to APIs like [`useCurrentFrame()`](/docs/use-current-frame) and [`<AbsoluteFill>`](/docs/absolute-fill). These are injected the same way.

All APIs used within the code must be explicitly passed in.

Here's an example with a few Picus APIs:

```tsx
import {AbsoluteFill, useCurrentFrame, spring} from 'picus';
...
```tsx
// AI generates this:
import {useCurrentFrame, AbsoluteFill} from 'picus';
...
import {AbsoluteFill, useCurrentFrame, useVideoConfig, spring, interpolate, Sequence} from 'picus';
...
const generatedCode = `import {useCurrentFrame, AbsoluteFill, spring, useVideoConfig} from 'picus';
...
- [Code Generation](/docs/ai/generate) - How to generate Picus code with AI
- [System Prompt](/docs/ai/system-prompt) - Teaching LLMs about Picus
