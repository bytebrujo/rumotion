...
crumb: 'AI'
title: Generate Picus Code using LLMs
sidebar_label: Code Generation
---

This guide shows an example of how to generate Picus component code from natural language prompts using the [Vercel AI SDK](https://sdk.vercel.ai/).
...
This simple system prompt relies entirely on the model's existing knowledge of Picus - not ideal, but a good starting point.
...
const systemPrompt = `
You are a Picus component generator.
Generate a single React component that uses Picus.

Rules:
- Export the component as a named export called "MyComposition"
- Use useCurrentFrame() and useVideoConfig() from "picus"
...
```tsx
import {useCurrentFrame, useVideoConfig, spring, AbsoluteFill} from 'picus';
...
For an example implementation of postprocessing, see the [Prompt to Motion Graphics template](https://github.com/picus-dev/picus/blob/main/packages/template-prompt-to-motion-graphics-saas/src/components/PromptInput.tsx#L159).
...
const systemPrompt = `
You are a Picus component generator.
Generate a React component that uses Picus. For the code property, ALWAYS directly output the code as string without any markdown tags.
...
However, a great starting place is to use Picus's [System Prompt](/docs/ai/system-prompt) which teaches the general LLM Picus's APIs and best practices.  
...
See the [Prompt to Motion Graphics template](https://github.com/picus-dev/picus/tree/main/packages/template-prompt-to-motion-graphics-saas) for a complete example implementation.
...
- [Prompt to Motion Graphics template](https://picus.dev/templates/prompt-to-motion-graphics) - Example implementation
- [System Prompt](/docs/ai/system-prompt) - A system prompt for teaching LLMs Picus
