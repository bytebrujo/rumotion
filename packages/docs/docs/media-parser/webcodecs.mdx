...
id: webcodecs
title: Processing video with WebCodecs and @picus/media-parser
...
import {parseMedia, MediaParserOnAudioTrack, MediaParserOnVideoTrack} from '@picus/media-parser';
...
## How Picus helps you work with WebCodecs

Since handling queueing correctly is a challenging task, we offer helpers that help you work with WebCodecs.

Picus's primitives are designed to offer you functionality to implement queueing in a sensible way:
...
// @noErrors
import type {MediaParserOnAudioTrack, MediaParserOnVideoTrack} from '@picus/media-parser';
...
For example, this [video recorded with an iPhone](https://github.com/picus-dev/picus/blob/main/packages/example/public/iphone-hevc.mov) has metadata that it should be displayed at 90 degrees rotation.
...
See here for an example of how a [video frame is turned into a bitmap](https://github.com/picus-dev/picus/blob/1a99dadcd8e700bf94abdfa3f5506329d6c9c182/packages/example/src/Encoder/SrcEncoder.tsx#L98-L102) and rotated.
...
Using Picus Media Parser and WebCodecs is [at least 3 times faster than using WebAssembly solutions](https://github.com/picus-dev/webcodecs-benchmark) in any scenario we tested.
...
## With `@picus/webcodecs`

[`@picus/webcodecs`](/docs/webcodecs) is a package that uses `@picus/media-parser` to provide video processing in the browser.  
...
A testbed with many different codecs and edge cases is available [here](https://github.com/picus-dev/picus/blob/main/packages/example/src/DecoderDemo.tsx).  
Follow [these instructions](https://www.picus.dev/docs/contributing#testing-your-changes) to run the testbed locally.

## License reminder

Like Picus itself, this package is licensed under the [Picus License](https://picus.dev/license).  
TL;DR: Individuals and small teams can use this package, but teams of 4+ people [need a company license](https://picus.pro/license).
