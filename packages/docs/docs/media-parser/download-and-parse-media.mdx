...
- `nodeWriter` from `@picus/media-parser/node-writer`: Writes to disk using Node.js's `fs` module.
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/media-parser/src/download-and-parse-media.ts)
