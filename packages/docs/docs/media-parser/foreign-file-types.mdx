...
import {parseMedia, IsAnImageError, IsAPdfError, IsAnUnsupportedFileTypeError} from '@picus/media-parser';
...
An error if the file is of a type that `@picus/media-parser` does not recognize.
