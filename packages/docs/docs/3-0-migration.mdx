...
When upgrading from Picus 2 to Picus 3, note the following changes and apply them to your project.

## How to upgrade

Upgrade `picus` and all packages starting with `@picus` to `^3.0.0`:

```diff
- "picus": "^2.6.15"
- "@picus/bundler": "^2.6.15"
- "@picus/eslint-config": "^2.6.15"
- "@picus/eslint-plugin": "^2.6.15"
- "@picus/cli": "^2.6.15"
- "@picus/renderer": "^2.6.15"
+ "picus": "^3.0.0"
+ "@picus/bundler": "^3.0.0"
+ "@picus/eslint-config": "^3.0.0"
+ "@picus/eslint-plugin": "^3.0.0"
+ "@picus/cli": "^3.0.0"
...
### Picus is built with React 18 types

The Picus components adhere to [version 18 of `@types/react` and `@types/react-dom`](https://github.com/DefinitelyTyped/DefinitelyTyped/pull/56210). Minor type errors may occur that can be resolved by adjusting your components. We recommend that you upgrade the `@types/react` package yourself.
...
Previously, you could catch errors being thrown in your Picus code using the `onError` property of `getCompositions()`, `renderFrames()` and `renderStill()`.

The new behavior of Picus 3.0 is that if an error occurs, these functions reject instead.
...
**Upgrade path**: Change import in `picus.config.ts`and use `overrideWebpackConfig()`.
