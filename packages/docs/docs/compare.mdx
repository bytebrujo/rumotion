...
We recommend Picus Lambda for most people as it has the best tradeoff for speed, ease of setup, maturity, total cost and scalability.

If you are already a Vercel customer and want the simplest setup, [Vercel Sandbox](/docs/vercel-sandbox) is a great choice.

The Picus customers with the highest amount of videos rendered use the Picus Lambda solution.

## Speed

Picus Lambda is the fastest option, because it implements distributed rendering - the video gets divided into smaller chunks and rendered in parallel across multiple Lambda functions.
...
Compute on Picus Lambda is more expensive than other options.
...
Picus Lambda and Cloud Run are also fast to set up.
Picus Lambda is built into our SaaS templates, so you can get started very quickly.
...
This is a big effort overall and is only recommended for teams who are committed to using Picus and want the most flexibility.

## Feature set

Picus Lambda currently has the following features that Cloud Run does not have:
...
We have guides for configuring [a server with GPU acceleration](/docs/miscellaneous/cloud-gpu), but it is not yet supported by Picus.

Picus Lambda and Vercel Sandbox do not have GPUs. You are limited to CPU-only rendering.
...
We are happy with the architecture of Picus Lambda and Node.js APIs and are committed to it in the long-term.
...
Cloud Run is in Alpha and we are considering moving it to a model which re-uses the code of Picus Lambda.
This would allow us to better match the features of Picus Lambda and enable distributed rendering.
