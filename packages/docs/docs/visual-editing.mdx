...
icon in the top right corner of the Picus Studio to open the right sidebar. Alternatively, press <kbd>Cmd/Ctrl</kbd> + <kbd>J</kbd> to toggle the sidebar. Select the <code>Props</code> tab.
...
- `z.nullable()`
- `z.enum()`
- [`zColor()`](/docs/zod-types/z-color) (from `@picus/zod-types`)
- [`zTextarea()`](/docs/zod-types/z-textarea) (from `@picus/zod-types`)
- [`zMatrix()`](/docs/zod-types/z-matrix) (from `@picus/zod-types`)
...
Picus can statically analyze your root file and allow you to save props back to the code via the 💾 button. For this to work, your default props must be inlined into your [`<Composition>`](/docs/composition):
