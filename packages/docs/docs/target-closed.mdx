...
- In Picus Lambda, increase the memory size.
...
:::note
Add `--log=verbose` during a render to print the browser executable that is being used (from Picus v2.6.7 on).
:::

Did none of these tips help? File an issue in the Picus repository.
