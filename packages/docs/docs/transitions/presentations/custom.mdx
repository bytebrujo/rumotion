...
import type {TransitionPresentationComponentProps} from '@picus/transitions';
...
import type {TransitionPresentation} from '@picus/transitions';
...
<br />
<Step>2</Step> Using <a href="/docs/shapes"><code>@picus/shapes</code></a>, an SVG path is calculated, and grown from zero to the full size of the canvas. <br />
<Step>3</Step> The <code>presentationProgress</code> is used to interpolate the shape
size. <br /> <Step>4</Step> <a href="/docs/paths"><code>@picus/paths</code></a> is used to center the star in the middle of the canvas.
<br />
...
See the source code for [already implemented presentations](https://github.com/picus-dev/picus/blob/main/packages/transitions/src/presentations) for a useful reference.
