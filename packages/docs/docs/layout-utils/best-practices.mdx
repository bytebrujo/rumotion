...
title: Best practices for @picus/layout-utils
sidebar_label: Best practices
crumb: '@picus/layout-utils'
---

Take note of the following points to ensure correct measurements when using the [`@picus/layout-utils`](/docs/layout-utils) package.
...
import {Dimensions, measureText} from '@picus/layout-utils';
...
const MyComp: React.FC = () => {
  ...
  useEffect(() => {
    ...
    waitUntilDone().then(() => {
      ...
      // font loading from @picus/google-fonts is already wrapped in it
...
The following logic is borrowed from the [Picus Recorder](/docs/recorder).

A file is defined which loads some fonts:

```tsx twoslash title="fonts.ts"
import {fontFamily as regularFont, loadFont as loadRegular} from '@picus/google-fonts/Inter';

import {fontFamily as monospaceFont, loadFont as loadMonospace} from '@picus/google-fonts/RobotoMono';
...
import {cancelRender, continueRender, useDelayRender} from 'picus';
...
:::note
In Picus v5, this will become the default.
:::
