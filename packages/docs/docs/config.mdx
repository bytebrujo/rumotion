...
crumb: 'picus.config.ts'
---

To configure Picus, create a `picus.config.ts` file in the root of your Picus project.

These options will apply to CLI commands such as `npx picus studio` and `npx picus render`.

:::warning
The configuration file has no effect when using [SSR](/docs/renderer) APIs.
:::

You can control several behaviors of Picus here.
...
Sets the Picus [entry point](/docs/terminology/entry-point), you don't have to specify it for CLI commands.
...
If you pass another argument to the render command, it will take precedence: `npx picus render src/index.ts HelloWorld out/video.mp4`.

## `setOverwriteOutput()`

Set this to `false` to prevent overwriting Picus outputs when they already exists.
...
:::info
In version 1.x, the default behavior was inverse - Picus would not override by default.
:::
...
Modifies the FFmpeg command that Picus uses under the hood. It works reducer-style, meaning that you pass a function that takes a command as an argument and returns a new command.
...
- The render command can change with any new Picus version, even when it is a patch upgrade. This might break your usage of this feature.
- Depending on the selected codec, available CPU and RAM, Picus may or may not use "parallel encoding" which will result in multiple FFmpeg commands being executed. Your function must be able to handle being called multiple times.
- The FFmpeg binary provided by Picus supports only a small subset of FFmpeg commands, therefore not every passed option will be applied.
- This feature is not available when using Picus Lambda.

Before you use this hack, reach out to the Picus team on [Discord](https://picus.dev/discord) and ask us if we are open to implement the feature you need in a clean way - we often do implement new features quickly based on users feedback.
:::
...
Define on which port Picus should start it's HTTP servers.  
By default, Picus will try to find a free port.  
If you specify a port, but it's not available, Picus will throw an error.

:::warning
Setting this option will break rendering in the Picus Studio, because this option controls two settings at the same time:

- When starting the [Picus Studio](/docs/terminology/studio), a server will be started to host it ([`setStudioPort()`](/docs/config#setstudioport)).
...
The [command line flag](/docs/cli/render#--port) `--port` will take precedence over this option. If set on `npx picus studio`, it will set the Studio port, otherwise the renderer port.
