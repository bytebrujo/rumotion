...
The whole state of the Editor Starter is stored in a single object with the shape defined by the [`EditorState`](https://github.com/search?q=repo%3Apicus-dev%2Feditor-starter%20type%20EditorState&type=code).
...
- `itemSelectedForCrop`: The ID of the item that is currently in [Crop Mode](/docs/editor-starter/cropping#crop-mode), if there is one.
...
In [`src/editor-context-provider.tsx`](https://github.com/picus-dev/editor-starter/blob/main/src/editor/context-provider.tsx), you will see a very deeply nested tree of various context providers.
...
If you only need to access the state upon an interaction, you can use the [`useCurrentStateAsRef()`](https://github.com/search?q=repo%3Apicus-dev%2Feditor-starter%20const%20useCurrentStateAsRef&type=code) hook.  
