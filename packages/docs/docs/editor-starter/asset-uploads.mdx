...
The constant [`MAX_FILE_UPLOAD_SIZE_IN_MB`](https://github.com/search?q=repo%3Apicus-dev%2Feditor-starter%20MAX_FILE_UPLOAD_SIZE_IN_MB&type=code) limits the size of the file that can be uploaded. By default, it is set to 1000MB.
...
Then set the `PICUS_AWS_TRANSFER_ACCELERATION` environment variable to `true` in your `.env` file.
