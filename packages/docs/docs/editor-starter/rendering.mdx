...
Rendering videos to MP4 works using [Picus Lambda](/docs/lambda), a distributed renderer on AWS Lambda.

## Setup

Follow the [Picus Lambda setup instructions](/docs/lambda/setup) step-by-step to set up your AWS account.

:::note
If you have already setup [asset uploads](/docs/editor-starter/asset-uploads), you should skip steps 4 and 5 of the Picus Lambda setup instructions.  
...
- You upgrade to a new Picus version
...
```ts
export const SITE_NAME = process.env.VERCEL_ENV === 'production' ? 'picus-editor-starter' : 'picus-editor-starter-dev';
```

[You don't need to separate function deployments between production and development](/docs/lambda/separate-environments).
