...
The font stack is implemented on [`@picus/google-fonts`](/docs/google-fonts).
...
The controls for text are mostly collected in the [`TextInspector`](https://github.com/search?q=repo%3Apicus-dev%2Feditor-starter%20TextInspector&type=code) component.
...
Change the script [`generate-google-font-info.ts`](https://github.com/picus-dev/editor-starter/blob/main/src/scripts/generate-google-font-info.ts) to include the fonts you want to include.
...
You will have to refactor the Editor Starter to be able to load fonts from other sources. We recommend using [`@picus/fonts`](/docs/fonts-api) for this.
