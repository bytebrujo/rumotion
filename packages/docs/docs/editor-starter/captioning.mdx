...
For implementation details, refer to the source code in [`src/editor/captioning`](https://github.com/picus-dev/editor-starter/tree/main/src/editor/captioning).
...
3. It converts OpenAI's response into Picus's [`Caption`](/docs/captions/caption) type and adds it to the timeline as a [`CaptionsItem`](https://github.com/search?q=repo%3Apicus-dev%2Feditor-starter+type+captionsitem&type=code).
...
Captions are automatically split into "pages" for easier management. Pages are timed groups of words or sentences that fit nicely on screen. This is achieved by using [`createTikTokStyleCaptions`](/docs/captions/create-tiktok-style-captions) from [`@picus/captions`](/docs/captions/api) package.
...
Review the [logic of `MAX_DURATION_ALLOWING_CAPTIONING_IN_SEC`](https://github.com/search?q=repo%3Apicus-dev%2Feditor-starter%20MAX_DURATION_ALLOWING_CAPTIONING_IN_SEC&type=code) to tweak it.

## Alternatives

### `@picus/whisper-web`

You can replace the OpenAI Whisper API with [`@picus/whisper-web`](/docs/whisper-web) for local, in-browser transcription.  
...
You can use [`@picus/install-whisper-cpp`](/docs/install-whisper-cpp) to transcribe audio on a Node.js server.
...
- [`@picus/install-whisper-cpp`](/docs/install-whisper-cpp)
