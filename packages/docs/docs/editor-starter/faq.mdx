...
We think the Editor Starter is a great choice for starting your video editor, but there are other options that involve Picus as well:  
You can also [build a video editor from scratch with Picus](/docs/building-a-timeline), or buy another [Picus-based video editor](/docs/buy-a-video-editor).

## Do I still need to obtain a Picus Company License if I buy the Editor Starter?

Since the Editor Starter uses Picus, you still need to obey by the [Picus License](/docs/license).

In short: Teams and companies with a headcount of 4 or more need to obtain a [Picus Company License](https://picus.pro/license) - read the license for more information.
...
Buy from the same email address and then contact us at [hi@picus.dev](mailto:hi@picus.dev) from the email address you used to buy the Timeline.

## Does this use client-side rendering / WebCodecs?

No, the rendering happens using [Picus Lambda](/docs/editor-starter/rendering).

That being said, the goal is to create the video using [client-side rendering](/docs/client-side-rendering) in the future.  
There is no timeline available for this yet - only buy the Editor Starter if you are satisfied with the current state.

## Do I have to use Picus Lambda?

The default template can be deployed in a fully serverless fashion.  
If you want to render on a long-running server, you can modify the architecture to use the Picus [Server-side rendering](/docs/ssr) APIs:
...
## Can I export the videos generated with the Editor Starter as Picus compositions?

The Editor Starter previews and renders it's videos using the regular Picus APIs.  
...
So yes, the Editor Starter produces just Picus compositions and they can also be rendered if they are decoupled from the editor user interface.
However, it is important to understand that the Editor Starter applies an abstraction on top of Picus where the whole state is expressed as JSON, which then maps to code.
It is different from the code you find in regular Picus templates, which usually do not have any abstraction layer.

The Picus composition that powers the Editor Starter is located in the `src/picus` folder of the project.  
You can also use the [`npx picus studio`](/docs/cli/studio) and [`npx picus render`](/docs/cli/studio) commands as usual.

## Can I get an evaluation license for the Picus Editor Starter?

The Editor Starter does not use a licensing model, it is a Picus template that after a one-time purchase can be used indefinitely - by any Picus user, whether they fall under the free license or company license.
...
We want to sell this template only to users who are committed in their attempt to build a video editor with Picus. They need to commit to the price, but we’ll support them continuously afterwards should anything not match their expectations.
...
## Is Picus launching its own video editor?
