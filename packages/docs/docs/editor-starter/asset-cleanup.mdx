...
If you have a `assetId` from an object in the `deletedAssets` array, you can delete it from IndexedDB by calling the [`deleteCachedAsset`](https://github.com/picus-dev/editor-starter/blob/9fdd35812f8505273edf29f207dd9d021c901491/src/editor/caching/indexeddb.ts#L92) method.
...
const {PICUS_AWS_BUCKET_NAME, PICUS_AWS_REGION} = requireServerEnv();

const {client, sdk} = getAwsClient({
  region: PICUS_AWS_REGION,
  service: 's3',
});

const command = new sdk.DeleteObjectCommand({
  Bucket: PICUS_AWS_BUCKET_NAME,
  Key: remoteFileKey,
});
...
Once an asset has been successfully deleted from S3 and IndexedDB, you can remove it from the state by calling the [`clearDeletedAsset()`](https://github.com/picus-dev/editor-starter/blob/main/src/editor/state/actions/clear-deleted-asset.ts) method.
