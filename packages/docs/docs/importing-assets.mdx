...
To import assets in Picus, create a `public/` folder in your project and use [`staticFile()`](/docs/staticfile) to import it.
...
Use the [`<Img/>`](/docs/img) tag from Picus.
...
```tsx twoslash
import {OffthreadVideo, staticFile} from 'picus';
...
export const MyComp: React.FC = () => {
  return <OffthreadVideo src="https://picus.media/BigBuckBunny.mp4" />;
};
```

See also: [Which video formats does Picus support?](/docs/miscellaneous/video-formats)
...
As an alternative way to import files, Picus allows you to `import` or `require()` several types of files in your project:
...
Picus runs in the browser, so it does not have access to arbitrary files on your computer.  
...
See [why does Picus does not support absolute paths](/docs/miscellaneous/absolute-paths).
...
By using the components from Picus, you ensure that:

<Step>1</Step> The assets are fully loaded before the the frame is rendered
<br />
<Step>2</Step> The images and videos are synchronized with Picus's timeline.
...
- [Why Picus does not support absolute paths](/docs/miscellaneous/absolute-paths)
