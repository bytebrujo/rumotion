...
title: In-memory cache for @picus/media
sidebar_label: Cache
crumb: 'API - @picus/media'
---

[`@picus/media`](/docs/media) keeps a cache of recently decoded video and audio frames.
...
Due to Picus's [multithreaded rendering and each frame being independent](/docs/flickering), it is not guaranteed that video frames are needed in their natural order.
...
- [`npx picus benchmark` → `--media-cache-size-in-bytes`](/docs/cli/benchmark#--media-cache-size-in-bytes)
- [`npx picus compositions` → `--media-cache-size-in-bytes`](/docs/cli/compositions#--media-cache-size-in-bytes)
- [`npx picus render` → `--media-cache-size-in-bytes`](/docs/cli/render#--media-cache-size-in-bytes)
- [`npx picus still` → `--media-cache-size-in-bytes`](/docs/cli/still#--media-cache-size-in-bytes)
- [`npx picus lambda render` → `--media-cache-size-in-bytes`](/docs/lambda/cli/render#--media-cache-size-in-bytes)
- [`npx picus lambda still` → `--media-cache-size-in-bytes`](/docs/lambda/cli/still#--media-cache-size-in-bytes)
- [`npx picus lambda compositions` → `--media-cache-size-in-bytes`](/docs/lambda/cli/compositions#--media-cache-size-in-bytes)
- [`npx picus cloudrun render` → `--media-cache-size-in-bytes`](/docs/cloudrun/cli/render#--media-cache-size-in-bytes)
- [`npx picus cloudrun still` → `--media-cache-size-in-bytes`](/docs/cloudrun/cli/still#--media-cache-size-in-bytes)
- [`npx picus cloudrun render` → `--media-cache-size-in-bytes`](/docs/cloudrun/cli/render#--media-cache-size-in-bytes)
- [Picus Studio](/docs/studio): Set the cache size under the "Advanced" tab
