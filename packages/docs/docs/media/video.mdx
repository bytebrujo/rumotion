...
Not to be confused with the older [`<Html5Video>` / `<Video>`](/docs/html5-video) tag from `picus`.
:::
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Video src="https://picus.media/BigBuckBunny.mp4" />
    </AbsoluteFill>
  );
};
```
...
export const MyVideo = () => {
  return (
    <AbsoluteFill>
      <Video volume={(f) => interpolate(f, [0, 30], [0, 1], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'})} src={'https://picus.media/video.mp4'} />
    </AbsoluteFill>
  );
};
```
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Video playbackRate={2} src={'https://picus.media/video.mp4'} />
    </AbsoluteFill>
  );
};
```
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Video muted src="https://picus.media/BigBuckBunny.mp4" />
    </AbsoluteFill>
  );
};
```
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Video loop src="https://picus.media/BigBuckBunny.mp4" />
    </AbsoluteFill>
  );
};
```
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Video audioStreamIndex={1} src={'https://picus.media/multiple-audio-streams.mov'} />
    </AbsoluteFill>
  );
};
```
...
Only props that are not supported by `<Video>` from `@picus/media` need to be specified here - props that apply for both tags will automatically be forwarded and do not need to be specified here.
...
:::note
When using [`@picus/web-renderer`](/docs/web-renderer) for client-side rendering, fallback is not possible and the render will always fail if the video cannot be embedded. See [Fallback is not possible in client-side rendering](/docs/media/fallback#fallback-is-not-possible-in-client-side-rendering).
:::
...
- [Source code for this component](https://github.com/picus-dev/picus/blob/main/packages/media/src/video/video.tsx)
- [Comparison of video tags](/docs/video-tags)
- [`<Audio>` (from `@picus/media`)](/docs/media/audio)
- [`<Html5Video>` (from `picus`)](/docs/html5-video)
