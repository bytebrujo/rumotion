...
title: 'Fallback from @picus/media to <OffthreadVideo> or <Html5Audio>'
sidebar_label: 'Fallback to <OffthreadVideo>'
crumb: '@picus/media'
---

Sometimes, a media file cannot be embedded using [`@picus/media`](/docs/media)'s [`<Video>`](/docs/media/video) and [`<Audio>`](/docs/media/audio) tags.  
In such cases, a fallback to [`<OffthreadVideo>`](/docs/offthreadvideo) or [`<Html5Audio>`](/docs/html5-audio) from the `picus` package is attempted.

## When a fallback is attempted

Here are some cases where [`@picus/media`](/docs/media) may fall back to [`<OffthreadVideo>`](/docs/offthreadvideo) or [`<Html5Audio>`](/docs/html5-audio) from `picus`:
...
If [`@picus/media`](/docs/media) falls back to another tag, then a warning message will be logged in the render:
...
export const MyComp: React.FC = () => {
  return <Video src="https://picus.media/video.mp4" disallowFallbackToOffthreadVideo />;
};
```

To prevent [`<Audio>`](/docs/media/audio) from falling back to [`<Html5Audio>`](/docs/html5-audio) from [`picus`](/docs/picus), set the [`disallowFallbackToHtml5Audio`](/docs/media/audio#disallowfallbacktohtml5audio) prop:

```tsx twoslash title="No fallback to HTML5 audio tag"
import {Audio} from '@picus/media';

export const MyComp: React.FC = () => {
  return <Audio src="https://picus.media/audio.mp3" disallowFallbackToHtml5Audio />;
};
```
...
**During rendering**, [`<OffthreadVideo>`](/docs/offthreadvideo) does not natively support looping. To work around this, `@picus/media` attempts to determine the duration of the video and automatically wraps [`<OffthreadVideo>`](/docs/offthreadvideo) in a [`<Loop>`](/docs/loop) component.
...
When using [`@picus/web-renderer`](/docs/web-renderer) for client-side rendering, fallback to [`<OffthreadVideo>`](/docs/offthreadvideo) or [`<Html5Audio>`](/docs/html5-audio) is **not possible**.

This is because these fallback components require server-side capabilities that are not available in the browser.

If a media file cannot be rendered using [`@picus/media`](/docs/media) during client-side rendering, the render will fail with an error message describing the issue.
