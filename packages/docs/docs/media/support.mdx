...
The new [`<Audio>`](/docs/media/audio) and [`<Video>`](/docs/media/video) tags from `@picus/media` are based on [Mediabunny](https://mediabunny.dev) and WebCodecs.
...
If a media file cannot be decoded, be it because of an unsupported codec or because it does not support CORS, or it has an alpha channel and the browser does not support WebGL which is required to decode the alpha channel, then `@picus/media` automatically falls back to [`<OffthreadVideo>`](/docs/offthreadvideo) or [`<Html5Audio>`](/docs/html5-audio) from the [`picus`](/docs/picus) package.

See: [Fallback from `@picus/media` to `<OffthreadVideo>` or `<Html5Audio>`](/docs/media/fallback)
