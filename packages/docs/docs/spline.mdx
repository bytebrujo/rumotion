...
[Spline](https://app.spline.design/) allows to design 3D models on the web. It allows exporting these models as React Three Fiber code, meaning that can be directly used and animated in Picus.

This tutorial shows how to design, export and animate a scene with Spline and Picus.
...
## Animating 3D Model using Picus

1. Create a new Picus project from the React Three Fiber template by running `npx create-video@latest`.
...
```tsx twoslash title="src/Torus.tsx"
import {interpolate, spring} from 'picus';
...
11. That's it! See a rotating Torus in the Picus Studio.
...
- [Picus Three Template](https://github.com/picus-dev/template-three)
