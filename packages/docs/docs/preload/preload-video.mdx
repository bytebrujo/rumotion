...
_This function is part of the [`@picus/preload`](/docs/preload) package._
...
An alternative to `preloadVideo()` is the [`prefetch()`](/docs/prefetch) API. See [`@picus/preload` vs `prefetch()`](/docs/player/preloading#picuspreload-vs-prefetch) to decide which one is better for your usecase.

## Usage

```tsx twoslash
import {preloadVideo} from '@picus/preload';

const unpreload = preloadVideo('https://picus.media/BigBuckBunny.mp4');
...
```tsx twoslash
import {preloadVideo, resolveRedirect} from '@picus/preload';
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/preload/src/preload-video.ts)
