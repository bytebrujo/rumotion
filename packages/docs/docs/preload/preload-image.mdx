...
An alternative to `preloadImage()` is the [`prefetch()`](/docs/prefetch) API. See [`@picus/preload` vs `prefetch()`](/docs/player/preloading#picuspreload-vs-prefetch) to decide which one is better for your usecase.

## Usage

```tsx twoslash
import { preloadImage } from "@picus/preload";
...
```tsx twoslash
import { preloadImage, resolveRedirect } from "@picus/preload";
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/preload/src/preload-image.ts)
