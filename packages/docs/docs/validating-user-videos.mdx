...
The `canDecode()` function checks if a video can be played in the browser and loaded into [`<Video>`](/docs/media/video) from `@picus/media`.

:::note
Different Picus video components have different format compatibilities:
...
:::note
Caveat: In `@picus/media`, videos with a H.265 codec are supported in the regular browser, but during rendering, they fall back to [`<OffthreadVideo>`](/docs/offthreadvideo).
:::
