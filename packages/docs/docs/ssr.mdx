...
Picus's rendering engine is built with Node.JS, which makes it easy to render a video in the cloud.
...
The fastest way to render videos in the cloud is to use [`@picus/lambda`](/docs/lambda).
...
jobs:
  render:
    ...
    steps:
      ...
      - run: npm i
      - run: npx picus render MyComp out/video.mp4
...
jobs:
  render:
    ...
    steps:
      ...
      - run: npx picus render MyComp out/video.mp4 --props="./input-props.json"
      - uses: actions/upload-artifact@v4
        with:
          name: out.mp4
          path: out/video.mp4
...
See: [Dockerizing a Picus project](/docs/docker)
