...
The following types are part of the API of `@picus/vercel`:

## `VercelSandbox`

```ts twoslash
import type {VercelSandbox} from '@picus/vercel';
//                ^?
```
...
```ts twoslash
import type {RenderStillOnVercelProgress} from '@picus/vercel';
//                ^?
```
...
```ts twoslash
import type {VercelBlobAccess} from '@picus/vercel';
//                ^?
```
