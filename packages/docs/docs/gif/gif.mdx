...
_Part of the [`@picus/gif`](/docs/gif) package_

Displays a GIF that synchronizes with Picuss [`useCurrentFrame()`](/docs/use-current-frame).
...
<details>
  ...
  <ul>
    <li>
      Picus's origin is usually <code>http://localhost:3000</code>, but it
...
The `playbackRate` prop controls the playback speed of the GIF animation within your Picus video. It enables you to adjust how fast or slow the GIF animation plays, allowing for precise synchronization with your video content.
...
Modifies the timeout of the internal [`delayRender()`](/docs/delay-render) call when loading the GIF. By default, Picus will wait 30 seconds for the GIF to load before timing out. You can increase or decrease this timeout by passing a custom value.
...
- [Source code for this component](https://github.com/picus-dev/picus/blob/main/packages/gif/src/Gif.tsx)
