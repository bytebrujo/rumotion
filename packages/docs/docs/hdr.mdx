...
title: High Dynamic Range and Picus
sidebar_label: HDR
id: hdr
...
Picus uses a headless Chrome browser to render, which does not support HDR.  
...
The Picus team is keen to hear your experiences.

### In `<OffthreadVideo />`

By default, color conversion is applied automatically to HDR videos.  
Picus tries to use FFmpeg and the `zscale` library to preserve the colors as well as possible when converting the video to SDR.
...
Because all frames are rendered in SDR, **it doesn't make sense to output an HDR video with Picus.**
