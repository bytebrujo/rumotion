...
The code for `@picus/web-renderer` is located in the `packages/web-renderer` folder.  
...
To fix a visual bug or add support for a new CSS property, first obtain some markup that does not look correct, then add it to the [`packages/web-renderer/src/test/fixtures`](https://github.com/picus-dev/picus/tree/main/packages/web-renderer/src/test/fixtures) folder.

Also add it to [`packages/web-renderer/src/test/Root.tsx`](https://github.com/picus-dev/picus/blob/main/packages/web-renderer/src/test/Root.tsx) so it is rendered in the Studio (`bun run studio`) - you can easily compare it to a server-side render or see how the browser renders it.

Then add a new test ([Example: `packages/web-renderer/src/test/transforms.test.tsx`](https://github.com/picus-dev/picus/blob/main/packages/web-renderer/src/test/transforms.test.tsx)) that renders the fixture in 3 different browsers and makes a screenshot of it.
...
Update the code in [`packages/web-renderer/src/drawing`](https://github.com/picus-dev/picus/tree/main/packages/web-renderer/src/drawing) to add a new feature to the web renderer and run the tests again.
...
Enable the experimental client-side rendering flag in your `picus.config.ts`:
...
```sh
npx picus studio --enable-experimental-client-side-rendering
```
