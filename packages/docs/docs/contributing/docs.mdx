...
Good documentation makes the difference between frustration and joy! We happily accept contributions to the Picus documentation.
...
<Step>1</Step> Set up the Picus repository <a href="/docs/contributing">according the instructions here</a>. <br/>
...
- **Link to terminology**: When using a Picus-specific term, link to the [terminology](/docs/terminology) page that explains it.
...
Using `<Demo type="[demo-name]" />` you can render a [Picus Player](/docs/terminology/player) and specify props that can be updated.
