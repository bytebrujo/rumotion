...
:::note
As we migrate to the [new media tags](/docs/mediabunny/new-video), we will not add any new Rust code, only maintain the existing code.
:::

To participate in the development of the Rust parts of Picus, you need to do additional steps.  
...
- Picus has platform specific packages like `compositor-darwin-arm64` and a user will only install packages that are specific to their operating system.
...
- Those shared libraries and FFmpeg binaries come from the [`rust-ffmpeg-splitter`](https://github.com/picus-dev/rust-ffmpeg-splitter) repository. They are built in CircleCI as well as on a M1 Macbook and pasted into the [`rust-ffmpeg-sys`](https://github.com/picus-dev/rust-ffmpeg-sys) project. The `rust-ffmpeg-sys` project is a dependency of [`rust-ffmpeg`](https://github.com/picus-dev/rust-ffmpeg) which is a dependency of the main Picus project. Refer to those repositories to learn how to build them.
