...
We are happy to accept contributions to the Picus project that implement new features.

## Setup

See the [CONTRIBUTING.md](https://github.com/picus-dev/picus/blob/main/CONTRIBUTING.md) file for instructions on how to set up the project.
...
- **Backwards-compatibility**: Your feature cannot break existing code if Picus is upgraded, unless the feature lands in a major version.
...
Picus uses [Font Awesome](https://fontawesome.com/) v5.15.4 for the icons in the Picus Studio.

We have a license and can grant access to Pro icons if needed.

## Communicate with us

Use the `#development` channel on [Discord](https://picus.dev/discord) to quickly ask questions and get feedback.
