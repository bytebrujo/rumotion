...
Wraps the native [`<video>`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLVideoElement) element to include video in your component that is synchronized with Picus's time.
...
`<Html5Video>` is not supported in [`@picus/web-renderer`](/docs/client-side-rendering). Use [`<Video>`](/docs/media/video) from `@picus/media` instead.
:::
...
All the props that the native `<video>` element accepts (except `autoplay` and `controls`) will be forwarded (but of course not all are useful for Picus). This means you can use all CSS to style the video.

```tsx twoslash
import {AbsoluteFill, staticFile, Html5Video} from 'picus';
...
```tsx twoslash
import {AbsoluteFill, Html5Video} from 'picus';
// ---cut---
export const MyComposition = () => {
  return (
    <AbsoluteFill>
...
import {AbsoluteFill, interpolate, staticFile, Html5Video} from 'picus';
...
A name and that will be shown as the label of the sequence in the timeline of the Picus Studio. This property is purely for helping you keep track of items in the timeline.
...
While Picus doesn't limit the range of possible playback speeds, in development mode the [`HTMLMediaElement.playbackRate`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLMediaElement/playbackRate) API is used which throws errors on extreme values. At the time of writing, Google Chrome throws an exception if the playback rate is below `0.0625` or above `16`.
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Html5Video muted src="https://picus.media/BigBuckBunny.mp4" />
    </AbsoluteFill>
  );
};
```

This has the benefit that Picus will not have to download the video file during rendering in order to extract the audio from it.
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Html5Video loop src="https://picus.media/BigBuckBunny.mp4" />
    </AbsoluteFill>
  );
};
```
...
This is because Picus mounts a `<video>` tag and sets `ref.currentTime = frame / fps` to synchronize with the timeline. When `currentTime` is set to a value larger than the video's duration, the HTML5 `<video>` element displays the last frame.
:::

### `acceptableTimeShiftInSeconds?`<AvailableFrom v="3.2.42" />

In the [Studio](/docs/terminology/studio) or in the [Picus Player](/docs/player), Picus will seek the video if it gets too much out of sync with Picus's internal time - be it due to the video loading or the page being too slow to keep up in real-time. By default, a seek is triggered if `0.45` seconds of time shift is encountered. Using this prop, you can customize the threshold.
...
export const MyComposition = () => {
  return (
    <AbsoluteFill>
      <Html5Video audioStreamIndex={1} src={'https://picus.media/multiple-audio-streams.mov'} />
    </AbsoluteFill>
  );
};
```
...
If set to `true` and the video is loading, the Player will enter into the [native buffering state](/docs/player/buffer-state). The default is `false`, but will become `true` in Picus 5.0.

### `showInTimeline?`<AvailableFrom v="4.0.122"/>

If set to `false`, no layer will be shown in the timeline of the Picus Studio. The default is `true`.
...
Picus will download the whole video during render in order to mix its audio. If the video contains a silent audio track, you can add the muted property to signal to Picus that it does not need to download the video and make the render more efficient.

## Codec support

See: [Which video formats does Picus support?](/docs/miscellaneous/video-formats)

## Alternatives: `<OffthreadVideo>` and `@picus/media`

[`<OffthreadVideo>`](/docs/offthreadvideo) is a Rust-based alternative to `<Html5Video>`.  
[`@picus/media`](/docs/media/video) is an experimental component that will replace `<Html5Video>` at some point.
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/core/src/video/Video.tsx)
- [`<Html5Audio />`](/docs/html5-audio)
- [`<OffthreadVideo />`](/docs/offthreadvideo)
- [`<Video />` (from `@picus/media`)](/docs/media/video)
