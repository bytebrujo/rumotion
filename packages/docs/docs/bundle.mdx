...
_Part of the `@picus/bundler` package._

Bundles a Picus project using Webpack and prepares it for rendering using [`renderMedia()`](/docs/renderer/render-media). [See a full server-side rendering example.](/docs/ssr-node)
...
A `string` containing an absolute path of the entry point of a Picus project. [In most Picus project created with the template, the entry point is located at `src/index.ts`](/docs/terminology/entry-point).
...
Gets called when a symbolic link is detected in the `public/` directory. Since Picus will forward the symbolic link, it might be useful to display a hint to the user that if the original symbolic link gets deleted, the bundle will also break.
...
Picus versions earlier than v3.2.17 had the following function signature instead:
...
It is still supported in Picus v3, but we encourage to migrate to the new function signature.
...
- [Source code for this function](https://github.com/picus-dev/picus/blob/main/packages/bundler/src/bundle.ts)
