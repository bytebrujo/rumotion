...
You can use Picus to do a batch render to create many videos based on a dataset. In the following example, we are going to turn a JSON dataset into a series of videos.

We'll start by creating a blank Picus project:
...
JSON is the most convenient format to import in Picus. If your dataset is in a different format, you can convert it using one of many available libraries on NPM.
...
This component will animate a title, subtitle and image using Picus. Replace the contents of the `src/Composition.tsx` file with the following:

```tsx title="Composition.tsx"
import React from 'react';
import {AbsoluteFill, Img, interpolate, spring, useCurrentFrame, useVideoConfig} from 'picus';
...
This can be done by using the [`bundle()`](/docs/bundle) function from the [`@picus/bundler`](/docs/bundler) package. Make sure to include the webpack override in the bundle if you have one.
...
// @filename: render.ts
// ---cut---
import {selectComposition, renderMedia} from '@picus/renderer';
import {webpackOverride} from './webpack-override';
import {bundle} from '@picus/bundler';
import {data} from './dataset';
...
- [Example repository using a dataset](https://github.com/alexfernandez803/picus-dataset)
