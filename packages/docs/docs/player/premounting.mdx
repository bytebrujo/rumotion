...
Picus only ever renders the current frame.  
...
because Picus only ever renders the current time.
:::
...
import {Sequence, staticFile, OffthreadVideo} from 'picus';
...
In the [Picus Studio](/docs/studio), a premounted component is indicated with diagonal stripes:
...
Premounting can also be implemented yourself with the public Picus APIs.  
Use the following component like you would `<Sequence>`:

```tsx twoslash title="PremountedSequence.tsx"
import React, {forwardRef, useMemo} from 'react';
import {Freeze, Sequence, SequenceProps, useCurrentFrame, usePicusEnvironment} from 'picus';
...
> = ({premountFor, ...props}, ref) => {
  ...
  const style: React.CSSProperties = useMemo(() => {
    return {
      ...passedStyle,
      opacity: active ? 0 : 1,
      // @ts-ignore Only in the docs - it will not give a type error in a Picus project
