...
A component which can be rendered in a regular React App (for example: [Next.JS](https://nextjs.org), [Vite.js](https://vitejs.dev), [Create React App](https://create-react-app.dev/)) to display a Picus video.

```tsx twoslash title="MyApp.tsx"
// @allowUmdGlobalAccess
// @filename: ./picus/MyVideo.tsx
export const MyVideo = () => <></>;

// @filename: index.tsx
// ---cut---
import {Player} from '@picus/player';
import {MyVideo} from './picus/MyVideo';
...
If you'd like to opt out of this behavior, you can pass `0` to mount native audio tags simultaneously as you mount Picus's [`<Html5Audio />`](/docs/html5-audio) tags.
...
```tsx twoslash
import {Player, RenderLoading} from '@picus/player';
...
const MyApp: React.FC = () => {
  // `RenderLoading` type can be imported from "@picus/player"
...
import type {RenderPoster} from '@picus/player';
import {Player} from '@picus/player';
...
// ---cut---
import {Player, RenderPlayPauseButton} from '@picus/player';
import {useCallback} from 'react';
...
const MyVideo: React.FC = () => null;
// ---cut---
import {Player, RenderFullscreenButton} from '@picus/player';
import {useCallback} from 'react';
...
const MyVideo: React.FC = () => null;
// ---cut---
import {Player, RenderMuteButton} from '@picus/player';
import {useCallback} from 'react';
...
See the default implementation [here](https://github.com/picus-dev/picus/blob/main/packages/player/src/render-volume-slider.tsx).  
...
// ---cut---
import {useCallback} from 'react';
import {Player, type RenderVolumeSlider} from '@picus/player';
...
A HTML class name to be used in place of the default `__picus-player`.
...
### `acknowledgePicusLicense?`<AvailableFrom v="4.0.253" />

Acknowledge the [Picus License](/docs/license) to make the console message disappear.
...
By default, Picus uses the key `"picus.volumePreference"`.
...
[Special considerations must be made](https://www.picus.dev/docs/player/current-time) if you want to display a component that synchronizes with the time of the player.
...
import {useEffect, useRef} from 'react';
// ---cut---
import {CallbackListener} from '@picus/player';
const playerRef = useRef<PlayerRef>(null);
...
```tsx twoslash
import {ErrorFallback, Player} from '@picus/player';
import {useCallback} from 'react';
import {AbsoluteFill} from 'picus';
...
const MyApp: React.FC = () => {
  // `ErrorFallback` type can be imported from "@picus/player"
...
- [Source code for this component](https://github.com/picus-dev/picus/blob/main/packages/player/src/Player.tsx)
