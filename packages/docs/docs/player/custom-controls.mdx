...
The Player provides a slot between the main playback controls and the fullscreen button where you can render custom controls using the [`renderCustomControls`](/docs/player/player#rendercustomcontrols) prop.

Use this when you want to add custom buttons or indicators that blend in with the default controls bar.

```tsx twoslash title="ControlsSlot.tsx"
const MyVideo: React.FC = () => null;
// ---cut---
import {Player, PlayerRef, RenderCustomControls} from '@picus/player';
...
:::note
Use a [`PlayerRef`](/docs/player/player#playerref) to access player state and methods from within your custom controls. The patterns shown in the [Controls outside the Player](#controls-outside-the-player) section below work the same way inside the controls slot.
:::
...
:::note
The [conventional time formatting for video editors](https://github.com/picus-dev/picus/issues/694#issuecomment-968318664) is `hh:mm:ss.ff` where `hh` is hours, `mm` is minutes, `ss` is seconds and `ff` is frames past the second.  
:::
...
Picus also considers a video "muted" if the volume is 0.  
