...
Picus has a native buffer state, which can be used to pause the video when the buffer is empty.
...
For [`<Video>`](/docs/media/video) and [`<Audio>`](/docs/media/audio) from [`@picus/media`](/docs/media), the buffer state is enabled by default.
...
import {useBufferState} from 'picus';
...
import React from 'react';
import {useBufferState, delayRender, continueRender} from 'picus';
...
By default, Picus will display the following UI based on the state of the player:
...
## Upcoming changes in Picus 5.0

In Picus 4.0, the tags `<Html5Audio>`, `<Html5Video>`, `<OffthreadVideo>` will need to opt-in to use the buffer state.

In Picus 5.0, it is planned that `<Html5Audio>`, `<Html5Video>` and `<OffthreadVideo>` will automatically use the buffer state, but they can opt out of it.
