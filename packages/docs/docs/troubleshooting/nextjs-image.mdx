...
Picus has no way of knowing when the image is finished loading because it is impossible to determine so when loading an image through `<Image>`.

This will lead to Picus not waiting for the image to be loaded during rendering and cause visible flickers.
