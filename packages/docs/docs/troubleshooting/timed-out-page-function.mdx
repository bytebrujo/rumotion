...
Error: Timed out evaluating page function (f, c) => {
  window.picus_setFrame(f, c);
}
```

generally happens if the Picus renderer is trying to send a JavaScript command to the browser, but the browser does not respond to it within the timeout (before `v4.0.73`: 5 second, from `v4.0.73`: the [`--timeout`](/docs/cli/render#--timeout) flag, by default 30 seconds).
