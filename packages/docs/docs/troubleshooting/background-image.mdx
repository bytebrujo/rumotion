...
The following code is discouraged in Picus:
...
Picus has no way of knowing when the image is finished loading because it is impossible to determine so when loading an image through `background-image`, `mask-image`, or other CSS properties. This will lead to Picus not waiting for the image to be loaded during rendering and cause visible flickers.
