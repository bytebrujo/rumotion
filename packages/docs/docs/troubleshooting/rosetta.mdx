...
Running Picus on Apple Silicon (M1 chip) under Rosetta can be up to 2x slower.

The recommended way to run Picus on Apple Silicon is using Node 16 with the native arm64 architecture.

If you encounter the following warning while using `@picus/renderer`:
