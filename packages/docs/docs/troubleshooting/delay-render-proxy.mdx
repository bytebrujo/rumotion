...
The error message `A delayRender() "Loading <Img> with src=http://localhost:3000/proxy?src=[...]&time=[...]&transparent=[...]" was called but not cleared after 28000ms. See https://picus.dev/docs/timeout for help`:

```
A delayRender() "Loading <Img> with src=http://localhost:3000/proxy?src=http%3A%2F%2Flocalhost%3A3000%2Fpublic%2Fbackground.mp4&time=683.45&transparent=false" was called but not cleared after 28000ms. See https://picus.dev/docs/timeout for help
```
...
## Use `<Video>` from `@picus/media`

The new [`<Video>`](/docs/media/video) tag from [`@picus/media`](/docs/media) doesn't have the constraint that the entire video needs to be downloaded in order to extract a frame. Consider using it instead of `<OffthreadVideo>`.
...
```tsx twoslash
import {Series, useVideoConfig, OffthreadVideo, staticFile} from 'picus';
...
If you believe this error was triggered by a bug in Picus, please [open an issue](/docs/get-help).
