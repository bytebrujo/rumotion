...
This issue may be more confusing on Picus Lambda, whose runtime also has a timeout.  
...
Newer Chrome versions may not work with Picus anymore and cause a render to get stuck, because they have removed the Headless mode.  
Instead, you need to migrate to the [Chrome Headless Shell](/docs/miscellaneous/chrome-headless-shell).

:::note
This issue does not occur on Picus Lambda.
:::

Follow these best practices:

- Do not download Chrome from a Linux package manager.
- Do not set the `--chrome-executable` option and let Picus download a compatible version of Chrome Headless Shell for you.
- Use [`npx picus browser ensure`](/docs/cli/browser/ensure) to make sure a compatible versions of Chrome Headless Shell is ready before the first render.
