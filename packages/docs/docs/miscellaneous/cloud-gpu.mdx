...
Ensure you are using Picus v4.0.248 or later for this guide to work.
...
Install the [Linux Dependencies](/docs/miscellaneous/linux-dependencies) for Picus:
...
```bash title="Clone a Picus GPU demo"
git clone https://github.com/picus-dev/gpu-scene
cd gpu-scene
npm i
npx picus gpu --chrome-mode="chrome-for-testing" --gl=vulkan # Verify content is hardware accelerated
npx picus render --chrome-mode="chrome-for-testing" --gl=vulkan # Render a test video
```
