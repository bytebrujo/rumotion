...
Picus is automatically installing "Chrome Headless Shell" into your `node_modules` in order to render videos.
...
- [`npx picus browser ensure`](/docs/cli/browser/ensure) on the command line
...
There are two flavors ("modes") that Picus can use to render videos:
...
**In the CLI**: Pass `--chrome-mode="chrome-for-testing"` to `npx picus render`, `npx picus benchmark`, `npx picus compositions`, `npx picus still`, `npx picus gpu` and `npx picus browser ensure`.
...
**In the config file (only applies to the CLI and Studio)**: Set `Config.setChromeMode('chrome-for-testing')` in your `picus.config.ts` file.
...
```
node_modules/.picus/chrome-headless-shell/[platform]/chrome-headless-shell-[platform]
```
...
```
node_modules/.picus/chrome-for-testing/[platform]
```
...
Picus writes a `VERSION` file to the download folder to track which Chrome version is installed:

```
node_modules/.picus/chrome-headless-shell/VERSION
node_modules/.picus/chrome-for-testing/VERSION
```

The file contains the Chrome version string, e.g. `144.0.7559.20`.

If the `VERSION` file is missing (e.g. when upgrading from an older version of Picus) or does not match the expected version, Picus will delete the existing browser and download the correct version.
...
## Why does Picus manage Chrome?

Picus previously used the desktop version of Chrome that many users had already installed.  
This workflow broke at some point, because Chrome removed the headless mode and extracted it to "Chrome Headless Shell".

## Best practices

To ensure your project does not get disrupted by an upcoming Chrome change, you should use the Picus mechanisms which uses and pins the version of Chrome Headless Shell.

- Use Picus v4.0.208 or later to not pick up an externally installed browser.
- Use [`npx picus browser ensure`](/docs/cli/browser/ensure) to ensure Chrome Headless Shell is available.
...
If you use a Picus version below v4.0.208, you are at risk of Chrome automatically being upgraded to a version that does not ship with a headless mode.  
:::

## What is Chrome Headless Shell?

Chrome used to ship with a `--headless` flag, which Picus would use.

As of Chrome 123, the headless mode is split up into:

- `--headless=old`, which is ideal for screenshotting (and therefore Picus)
...
Hence we encourage you to use Chrome Headless Shell to future-proof your Picus application.

## Version

Picus will download a well-tested Chrome version:

| Picus version | Chrome version                    |
| ---------------- | --------------------------------- |
...
If you use the [`server-side rendering APIs`](/docs/ssr), use the callback in [`ensureBrowser()`](/docs/renderer/ensure-browser#onbrowserdownload) to change the Chrome version.

## On Lambda and Cloud Run

If you are using [Picus Lambda](/docs/lambda) or [Cloud Run](/docs/cloudrun), you don't need to worry about installing a browser - it is included in the runtime already.

## Previous changes

### Added option to install Chrome for Testing (v4.0.247)

In order to enable GPU-accelerated workloads on Linux, Picus now allows you to install Chrome for Testing.

### Migrated to Chrome Headless Shell

Because Chrome removed the headless mode from the desktop browser, Picus now uses Chrome Headless Shell.
...
In previous versions, Picus would download the free version of Chromium, which would not include codecs for the proprietary H.264 and H.265 codecs.
This would often lead to problems when using the [`<Html5Video>`](/docs/html5-video) tag.
