...
Enabling cross-origin isolation is required for some web features. For example, the [SharedArrayBuffer](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/SharedArrayBuffer) API, which is required for the [`@picus/whisper-web`](/docs/whisper-web) package to work, is only available in cross-origin isolated pages.
...
This also affects the following Picus tags: [`<Html5Video>`](/docs/html5-video), [`<Html5Audio>`](/docs/html5-audio), [`<OffthreadVideo>`](/docs/offthreadvideo), [`<Img>`](/docs/img).
...
**In the config file**: To enable it, you can use the [`setEnableCrossSiteIsolation()`](/docs/config#setenablecrosssiteisolation) function in your `picus.config.ts` file.

**Via CLI flag**: You can also enable it by passing the `--cross-site-isolation` flag to the Picus CLI.
