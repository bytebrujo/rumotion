...
[`parseMedia()`](/docs/media-parser/parse-media) is a library developed by Picus that parses video files using JavaScript.
...
## `getVideoMetadata()` from `@picus/media-utils`
...
## `getVideoMetadata()` from `@picus/renderer`
...
- [`@picus/media-utils`: `getVideoMetadata()`](/docs/get-video-metadata)
- [`@picus/renderer`: `getVideoMetadata()`](/docs/renderer/get-video-metadata)
