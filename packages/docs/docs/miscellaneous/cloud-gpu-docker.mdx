...
# Clone the repo
RUN git clone https://github.com/picus-dev/gpu-scene.git
WORKDIR /gpu-scene
RUN npm install
...
npx picus render --gl=angle-egl Scene out/video.mp4
```

<Step>6</Step> Build the container and run a sample render: <br />

```bash
sudo docker build . -t picus-docker-gpu
sudo docker run --gpus all --runtime=nvidia -e "NVIDIA_DRIVER_CAPABILITIES=all" picus-docker-gpu
```

## Debugging

Use the [`npx picus gpu`](/docs/cli/gpu) command to get the output of `chrome://gpu` to verify that the GPU is working.
