...
By default, Picus starts the Chromium browser on Linux with the `--single-process` flag. This is because:
...
This can lead to degraded rendering performance, especially when trying to leverage high-core CPUs and high Picus concurrency.
...
- **CLI**: Use the [`--enable-multi-process-on-linux=false`](/docs/cli/render#--enable-multiprocess-on-linux) flag.
- **Config file**: Use the [`Config.setChromiumMultiProcessInLinux()`](/docs/config#setchromiummultiprocessonlinux) option.
- **Picus Studio**: Check the toggle in the advanced settings.
...
The answer depends on the Linux environment. If you are running Picus on a server, you should try to enable multi-process mode and see if it works.
...
- [Dockerizing a Picus app](/docs/docker)
