...
[Install `@picus/media-parser`](/docs/media-parser) if necessary.
...
:::note
If your asset is not CORS-enabled, you can use the [`getVideoMetadata`](/docs/get-video-metadata) function from [`@picus/media-utils`](/docs/media-utils) instead of `parseMedia()`.
:::
...
Follow the same steps, but instead of [`parseMedia()`](/docs/media-parser), use [`getAudioDurationInSeconds()`](/docs/get-audio-duration-in-seconds) from `@picus/media-utils` to calculate the duration of the composition based on the audio file.
