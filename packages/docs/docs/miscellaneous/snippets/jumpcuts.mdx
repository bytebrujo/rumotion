...
<div>
  ...
  <div>
    <Step>3</Step> If the jump is tiny (&lt;0.45sec), Picus might not seek due to the [`acceptableTimeshiftInSeconds`](/docs/offthreadvideo#acceptabletimeshiftinseconds) prop. You might have to decrease it temporarily.
  </div>
</div>
...
import {CalculateMetadataFunction, OffthreadVideo, staticFile, useCurrentFrame} from 'picus';
