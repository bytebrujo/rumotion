...
<details>
  ...
  <p>We load a URL (e.g. `https://picus.media/video.mp4`) and parse it using Mediabunny.</p>
...
const metadata = await getMediaMetadata('https://picus.media/video.mp4');
...
In that case, you are better off using the [`getVideoMetadata()`](/docs/get-video-metadata) function from the [`@picus/media-utils`](/docs/media-utils) package, which uses the `<video>` tag internally.
