...
To use React 19's newest features, you need at least version `4.0.0` of Picus.
...
We have updated all [templates](https://picus.dev/templates) to use React 19 (exception: React Native Skia).  
See the source code of the templates for examples on how to upgrade it to React 19.

## `HTMLRefElement` Type Change

If you have type errors related to React Refs, upgrade to v4.0.236 of Picus, where we aligned the types with React 19.

## Ecosystem support

Some libraries that are used with Picus need upgrading.
