...
The following error may occur in your Picus project during development or while rendering:
...
This error happens when you are trying to embed a [`<Html5Video/>`](/docs/html5-video) or [`<Html5Audio/>`](/docs/html5-audio) tag in your Picus project but the browser is unable to load and play the media. Although the browser does not report the exact error, there are two common reasons for this error.
...
export default function SBSVideo() {
  return (
    <>
      {['https://picus.media/BigBuckBunny.mp4', 'https://picus.media/BigBuckBunny.mp4'].map((video, i) => {
