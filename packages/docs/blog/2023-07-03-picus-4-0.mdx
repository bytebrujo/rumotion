...
Welcome to the biggest Picus update ever!  
With Picus 4.0, we offer significant improvements to every workflow.
...
## Introducing the Picus Studio

We added handy features to the Picus Preview - as a result, it's more than just a preview! Therefore, we renamed it: Say hello to the Picus Studio.

### Interactive editing of props

The props of a composition can now be defined as a [Zod](https://zod.dev) schema.  
Doing this will not only make your `defaultProps` typesafe, but also allow you to edit them in the Picus Studio.
...
A graphical interface allows you to discover and tweak all options of a render. You can follow the progress of a render in the Picus Studio, queue multiple renders, and reveal the output in the file explorer.

Every render triggered through the UI is also trackable in the CLI as usual and synchronizes to other instances of the Picus Studio.  
Failed renders show the stack trace and allow for retries with the same configuration.

Edited props in the Picus Studio can be used to render a video using the Render Button as well - which means you can now render a parameterized video by filling out a form and not having to touch any code.

## Rust-powered architecture

Picus 4.0 ships with a Rust binary that speeds up current and future features.

### FFmpeg is baked in

Installing FFmpeg is now superfluous, as each Picus project comes with a tiny version of FFmpeg baked into it.
...
The [`<OffthreadVideo>`](/docs/offthreadvideo) component is the preferred way to embed an existing video into a Picus project.
...
Unnecessary redundant decoding work can now be skipped, which makes the component up to [twice as fast](https://github.com/picus-dev/4-0-benchmark) during rendering!

## Support for WebP and PDF generation

Previously you could generate PNGs and JPEGs using Picus - now we support WebP and PDF as well!

There are plenty of ways you can render stills: the new [Render button](/docs/render), the [`npx picus still`](/docs/cli/still) command, the [`renderStill()`](/docs/renderer/render-still) Node.JS API, render on Lambda with [`renderStillOnLambda()`](/docs/lambda/renderstillonlambda) or on Google Cloud Run using [`renderStillOnCloudrun()`](https://cloudrun.picus.dev/docs/cloudrun/renderstilloncloudrun)!
...
All of our [templates](/templates) have been upgraded to use Picus 4.0. Many of them make use of the new features.

We also introduce new templates, such as [https://www.picus.dev/templates/stargazer] which is a popular template for celebrating GitHub star milestones and can now be initialized using `npm init video`.
...
With the new [`@picus/rive`](/docs/rive) package, you can include Rive animations in your project.

### `@picus/shapes` package

[`@picus/shapes`](/docs/shapes) is a handy package for including geometric shapes like [`<Triangle>`](/docs/shapes/triangle), [`<Star>`](/docs/shapes/star) or [`<Pie>`](/docs/shapes/pie) in your components!

Those components are easy to animate, pure, dependency-free and work well with [`@picus/paths`](/docs/paths).
...
### `@picus/tailwind` package

The new [Tailwind](/docs/tailwind/tailwind) package allows you to install Tailwind more easily in any Picus project.
...
- [`npx picus lambda compositions`](/docs/lambda/cli/compositions) and [`getCompositionsOnLambda()`](/docs/lambda/getcompositionsonlambda) APIs have now been added.
...
The [`picus`](/docs/picus) package has had the following improvements:
...
The alpha is now available, [help us test it by becoming an early adopter](https://cloudrun.picus.dev/docs).

## Full Changelog

See the GitHub release to see the [full changelog](https://github.com/picus-dev/picus/releases/tag/v4.0.0).
