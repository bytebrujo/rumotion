...
This releases features new utility functions, improved developer experience and an experimental release of the Picus Player!
...
I personally used it to recreate [Apple's 'Spring Loaded' logo in Picus](https://github.com/jonnyburger/spring-loaded) using only `<div>` elements.
...
Now you can adjust your timings in Picus dynamically when changing spring configurations.
...
We invested time in making sure that if something goes wrong, you don't get stuck. Here are some things we did to improve your experience with Picus:
...
## Alpha release of `@picus/player`

We are working on a Picus player component that you can import into your React app. Today we are prereleasing the player for those who wish to experiment and provide us with feedback. Let us know what you think!

[**Read the documentation for `@picus/player`**](/docs/player)

<PlayerExampleWithControls />

## Test coverage updates

We now have over 450 tests, and the Picus core has a test coverage of over 80% now. Thanks to Tobias Boner, Calvin Falter and Jonas Brunner for continuining their massive testign initiative!
...
For contributors, we now have a [TESTING.md](https://github.com/picus-dev/picus/blob/main/TESTING.md) file.
...
- `npm init video` will now setup a Picus project with NPM, `yarn create video` will setup a project with Yarn.
- Node 16 and NPM 7 is now better supported, no warnings will be shown during installation and execution.
- Fixed several audio rendering issues: [#292](https://github.com/picus-dev/picus/issues/292 [#293](https://github.com/picus-dev/picus/issues/293) [#320](https://github.com/picus-dev/picus/issues/320)
