...
Welcome to the release notes of Picus 2.2! It's been a while since v2.1, but in the meanwhile we had a dozen smaller releases inbetween. This post summarizes the highlights of the past 2 months 🎉.
...
import {Freeze, Html5Video} from 'picus';
...
Learn more about [`@picus/three`](/docs/three) and get started using our [template](https://github.com/picus-dev/template-three).
...
## Picus waits for fonts to be loaded

If a Webfont gets loaded via Google Fonts, it would be loaded using `font-display: swap`, which means Picus would sometimes render a frame before the font is loaded. Now we use the `document.fonts.ready` browser API to make loading webfonts completely seamless for you.

## Specify a custom `--port`

By default the Picus Preview starts a server on port 3000. Should you want to have it start on a different port for any reason, you can now pass a CLI flag.
...
- Multiple versions of Picus are imported on a page (for example when using the `<Player />` in your app)
...
[A new page](/docs/third-party) has been created listing some popular libraries and describing how well they work together with Picus - or don't.

### New landing page

Finally, the [homepage of Picus](https://picus.dev) got a makeover! We hope to better communicate what Picus is and why it's awesome.

## Various Bugfixes

- Fixed a memory leak while calling getCompositions() [#480](https://github.com/picus-dev/picus/pull/480)
- Add emoji support to Dockerfile [#476](https://github.com/picus-dev/picus/pull/476)
- The preview now shows the current time within a sequence in a timeline [#468](https://github.com/picus-dev/picus/pull/468)
- TTF fonts can now be loaded locally [#462](https://github.com/picus-dev/picus/pull/462)
- Fix Picus Player assuming it's in a rendering environment [#460](https://github.com/picus-dev/picus/pull/460)
- Fix getAudioMetadata() and getAudioDuration() not returning an error in the `onError` event [#459](https://github.com/picus-dev/picus/pull/459)
- Added documentation on how to use TailwindCSS [`69b079e`](https://github.com/picus-dev/picus/commit/69b079e)
- Fixed documentation about how to use SASS/SCSS [`e92c917`](https://github.com/picus-dev/picus/commit/e92c917)
- If you are using a plain JS entry file, you can also use a picus.config.js config file [#475](https://github.com/picus-dev/picus/pull/475)
- You can customize how many timeline tracks should be shown at most using `Config.Preview.setMaxTimelineTracks()` [#439](https://github.com/picus-dev/picus/pull/439)
- Fixed a bug where environment variables would not be immediately accessible [#432](https://github.com/picus-dev/picus/pull/432)
- Input props flag can now also be passed for preview [#430](https://github.com/picus-dev/picus/pull/430)
- Better input validation of the most common functions
- Improve rendering of very long videos, using less memory and avoiding a maximum callstack exceeded error message. [#398](https://github.com/picus-dev/picus/pull/398)
- Setting `crf` to 0 is not allowed anymore for H264 videos as it will lead to a video that does not play on all platforms. [#379](https://github.com/picus-dev/picus/pull/379)
- The docs for `<Sequence />` now have interactive examples. [#378](https://github.com/picus-dev/picus/pull/378)
- A lot of tests have been added to the core functions. [#376](https://github.com/picus-dev/picus/pull/376) etc.
- The Typescript types have been improved to allow for more types of components to be passed to `component`. [#372](https://github.com/picus-dev/picus/pull/372)
...
- Thanks to [Björn Zeutzheim](https://github.com/olee) for implementing the `@picus/three` package!
...
We start seeing people launching products and startups using Picus, which is awesome. Stay tuned for posts that highlight how people are using Picus for their business case! At the time of writing, five customers are currently in possession of a [Company license](https://companies.picus.dev).
...
We are increasingly putting [bounties on non-urgent issues](https://github.com/picus-dev/picus/issues?q=+label%3A%22%3Adollar%3A+Funded+on+Issuehunt%22+) and have rewarded
$657 to Picus contributors to far, in addition to multiple thousands of dollars being paid out to top contributors that were approached by us.

We are working on multiple fronts to make Picus even better for businesses and Indie Hackers that want to integrate programmatic video into their product - stay tuned!
