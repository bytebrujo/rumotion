...
image: /img/picus-2-6.png
---

The biggest announcement of this release is that the [`@picus/player`](/player) package is now generally available - but not just that, we have some other sweet new features too!
...
With the `<Player/>` component, you can embed a Picus Video inside a React app without rendering the video. The API is modeled after the native HTML `<video>` tag that many developers are already familiar with.
...
In this release, we added a new [`renderLoading`](/docs/player/player#renderloading) prop and wrote docs for how to [scale your player](/docs/player/scaling), [code-sharing between Picus and Create React App/Next.JS](/docs/player/integration) and [preloading assets](/docs/player/preloading).

[See the landing page for `@picus/player`](/player)  
...
<br />
<br />
The Fast Refresh and error overlay is now inlined in our codebase and allows for customization that makes sense for Picus. The overlay now matches the dark theme of the Picus Preview and includes handy links such as opening a file in the editor, looking for the error in our GitHub issues and
our Discord community.

## Support for `/public` folder

You can now put static files inside a `/public` folder in your Picus project and load it using the new [`staticFile()`](/docs/staticfile) API.
If you include the new `<Player />` component in a Create React App or Next.JS project, the folder can be shared across Picus and the framework you are using.

## `<Audio />` or `<Video />` now support `data:` URLs

Data URLs are now valid sources for [`<Html5Audio />`](/docs/html5-audio) and [`<Html5Video />`](/docs/html5-video) tags. This is useful for example for tones that are programmatically generated. To help with development of such projects, a new API was added to the `@picus/media-utils` project: [`audioBufferToDataUrl()`](/docs/audio-buffer-to-data-url). See our festive [Tone.js](https://github.com/picus-dev/tone-js-example) sample project for an example!
...
- Updated `@picus/three` to use React Three Fiber v7.
