...
Announcing the official [`@picus/lottie`](/docs/lottie) package, including a [typesafe component](/docs/lottie/lottie) and extensive [documentation](/docs/lottie).
...
<p align="center">
  <sub>
    Animations from Lottiefiles embedded in Picus: <a href="https://lottiefiles.com/106683-gold">1</a>, <a href="https://lottiefiles.com/112208-smiling-face-with-halo">2</a>, <a href="https://lottiefiles.com/111529-rocket">3</a>
  </sub>
</p>

To get started, install `@picus/lottie` into your Picus project and import the [`<Lottie>`](/docs/lottie) component:
...
Using the [`@picus/skia`](/docs/skia) package, you can now use [React Native Skia](https://shopify.github.io/react-native-skia/) in Picus! Thanks to our collaborators [William Candillon](https://youtube.com/wcandillon) and [Christian Falch](https://twitter.com/chrfalch), Picus is now a first-class target for React Native Skia.
...
Renders that are audio only are now faster because Picus will not wait for the video tags to seek.
...
- Chrome 104 is now available on Picus Lambda which means you can use the [handy transform shorthands](https://twitter.com/JNYBGR/status/1555055938008223744)!
- You can now render ProRes on Picus Lambda.
- Picus Lambda now has a [`privacy: "no-acl"`](/docs/lambda/rendermediaonlambda#privacy) option if you are rendering into a bucket that has the ACL feature disabled.
- Picus Lambda now supports a [`downloadBehavior`](/docs/lambda/rendermediaonlambda#downloadbehavior) prop which makes it that when a output file link gets clicked in the browser, it will download instead of play in the browser.
- Adding an output filename to the [`npx picus render`](/docs/cli/render) command is not necessary anymore, it will default to `out/{composition-id}.{extension}` now.
...
- Better error message on Picus Lambda when the `s3:ListBucket` permission for the bucket you are rendering into is missing.
...
- Picus now cleans up any temporarily created files and does not pollute the hard drive.
- Executing `npx picus` commands outside of the project root now works.
- `Open in VS Code` now works if the `code` command is not installed.
- Picus Lambda now uses less memory and is less prone to crashing when using `<Video>`'s.

## Internals

- The CLI configuration code has moved from `picus` to `@picus/cli`, which makes the `picus` package 30% smaller.
