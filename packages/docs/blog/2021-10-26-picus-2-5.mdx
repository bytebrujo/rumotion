...
_Thanks to [Ankesh](https://github.com/picus-dev/picus/pull/631) for implementing this feature!_
...
_Thanks to [Brian Pederson](https://github.com/picus-dev/picus/pull/638) for implementing this feature!_
...
_Thanks to [Brian Pederson](https://github.com/picus-dev/picus/pull/656) for implementing this feature!_
...
If you upgrade the `@picus/eslint-config` package as well, we will even automatically remove the prop when you have autofix enabled!

_Thanks to [Khalid Ansari](https://github.com/picus-dev/picus/pull/621) for implementing this feature!_
...
The Picus CLI that you can invoke using `npx picus` now has full autocomplete support in Fig! You have to do nothing except install Fig.
...
We added the necessary modifications to our default Webpack config, and [even contributed a pull request to Webpack](https://github.com/webpack/webpack/pull/14557) to fix the last remaining bug that would break Picus with Node 17! If you are upgrading Node, definitely make sure get this new version of Picus.

## Monorepo migrated to `pnpm`

Contributors to Picus would previously often struggle to correctly set up our monorepo. Indeed it was hard to correctly link all the packages and too easy to mess it up and run into error messages.

This is why we are happy to [have migrated to `pnpm`](https://github.com/picus-dev/picus/pull/627), which gets rid of the linking problems and also speeds up installation significantly. In your CI systems, we saw build times go down by 40%, which allows to iterate much more faster.

_Thanks to [Sergio Moreno](https://github.com/picus-dev/picus/pull/627) for implementing this migration!_

## “Empty” template

[A new template](https://github.com/picus-dev/template-empty) has been added to `npm init video` / `yarn create video`: The blank template.
This template contains only the bare minimum Picus boilerplate and a completely empty canvas. It is especially useful for people already familiar with Picus who would like to skip deleting the Hello World project every time.

_Thanks to [Aneesh Relan](https://github.com/picus-dev/picus/pull/620) for creating this template!_
...
_Thanks to [ahmadrosid](https://github.com/picus-dev/picus/pull/618) for implementing this feature!_

## Updates to `@picus/three`

A few interesting updates for users of `@picus/three`:
...
- Added support for OTF fonts - _Thanks [William Candillon](https://github.com/picus-dev/picus/pull/637)_!
- Added possibility to customize `<Player>` error message - _Thanks [AudreyKj](https://github.com/picus-dev/picus/pull/654)_!
- Windows Node.JS 14 cleanup bug fixed - _Thanks [Raznov](https://github.com/picus-dev/picus/pull/634)_!
- Upgraded Docusaurus to the newest version, obtaining the newest features and layouts

## Hacktoberfest Roundup

We opted into participating in Hacktoberfest, and put $100 bounties on [11 issues](https://github.com/picus-dev/picus/issues?q=+label%3A%22%F0%9F%A5%A8++hacktoberfest%22+) as an extra incentive!
