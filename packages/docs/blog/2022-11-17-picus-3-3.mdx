...
Now, when you are rendering a video and don't have FFmpeg installed, Picus will download a copy for you.
Previously, installing FFmpeg required 7 steps on Windows and took several minutes when using Homebrew on macOS.

<FfmpegVideo />

When deploying Picus on a server, you can now also let Picus install FFmpeg for you using the [`ensureFfmpeg()`](/docs/renderer/ensure-ffmpeg) API or the [`npx picus install ffmpeg`](/docs/cli/install) command. Learn more about [FFmpeg auto-install](/docs/ffmpeg) here.

## New `@picus/google-fonts` package

It is now easy to import Google Fonts into Picus! [`@picus/google-fonts`](/docs/google-fonts) takes care of correct loading, and is fully type-safe!

<video src="/img/google-fonts.mp4" autoPlay loop muted playsInline />

## New `@picus/motion-blur` package
...
When starting the Picus Preview, it now shows on which URL the preview is running. The Webpack output is now also cleaner.
...
### Search the docs from the Picus Preview

Pressing <kbd>?</kbd> to reveal the keyboard shortcuts now has a secondary function: You can type in any term to search the Picus documentation!

### Shorter commands

Previously, a Picus CLI command looked like this:

```bash
npx picus render src/index.tsx my-comp output.mp4
```

We now allow you to skip the output name, in this case the render would land in `out/my-comp.mp4` by default:

```bash
npx picus render src/index.tsx my-comp
```

You can also omit the composition name and Picus will ask which composition to render:

```bash
npx picus render src/index.tsx
```
...
Finally, you can also omit the entry point and Picus will take an educated guess!

```bash
npx picus render
```

If you deviate from the defaults of our templates, you can set an [entry point](/docs/config#setentrypoint) in your config file and leave it out from Picus commands.

### Auto-reload environment variables

If you change values in your `.env` file, the Picus Preview will reload and pick them up without having to restart.

### Signal that Picus Preview disconnected

When quitting the Picus Preview using `Ctrl+C`, for example to render a video, A new popup will signalize that Fast Refresh will not work anymore.
...
The FFmpeg command that Picus executes under the hood can [now be overriden reducer-style](/docs/config#overrideffmpegcommand).

## Server-side rendering

### Resuming renders if they crash

If a render crashes due to being resource-intensive (see: [Target closed](/docs/target-closed)), Picus will now retry each failed frame once, to prevent long renders from failing on low-resource machines.
...
Using the [`inFrame`](/docs/player/player#inframe) and [`outFrame`](/docs/player/player#outframe) props, you can force the Picus Player to only play a certain section of a video. The rest of the seek bar will be greyed out.
...
In addition to the [`Preload APIs`](/docs/preload), [`prefetch()`](/docs/prefetch) presents another way of preloading an asset so it is ready to display when it is supposed to appear in the Picus Player.

```tsx twoslash title="Prefetching API"
import {prefetch} from 'picus';
...
Video and audio tags will automatically use the prefetched asset if it is available. See [`@picus/preload` vs. `prefetch()`](/docs/player/player#renderfullscreenbutton) for a comparison.

## Remix template

The Remix template is our first SaaS template! It includes the Picus Preview, the Player and Picus Lambda out of the box to jumpstart you with everything you need to create your app that offers customized video generation.
...
We try to avoid jargon, but we have also created a [Picus Terminology](/docs/terminology) page to define some commonly used terms. When using these terms, we will from now link to the terminology page for you to read about it.

## Better structure and naming in templates

The file that was previously called `src/Video.tsx` in templates is now called `src/Root.tsx`, because it did not contain a video, but a list of compositions. That component was also renamed from `PicusVideo` to `PicusRoot`. The new naming makes more sense, because that component is passed into [`registerRoot()`](/docs/register-root).
...
The temporary directory that Picus creates is now completely cleaned up after every render.
...
- [ayatko](https://github.com/ayatko) for implementing the `@picus/google-fonts` package
