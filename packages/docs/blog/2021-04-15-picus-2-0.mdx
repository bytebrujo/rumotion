...
Read more about [audio visualization](/docs/audio/visualization) in the docs or [watch a sample video](https://twitter.com/JNYBGR/status/1378444074634592262) (the source can be found [here](https://github.com/yuta0801/picus/blob/1103092f4902fb11f41877a05398dab6ce7203e7/packages/example/src/AudioVisualization/index.tsx)).

## Export as MP3, AAC, WAV

You can now also completely omit the video from the output and only emit an audio file. Yes, Picus can also be used as an audio editing program! Pass `mp3`, `aac` or `wav` as a codec [using the `--codec` flag](/docs/cli/render#--jpeg-quality) or [in your config file](/docs/config#setcodec).

## `@picus/media-utils` package
...
All of them except the last one are completely independent from the ideas of Picus, so they might be a great fit for your other non-Picus projects as well! This package is MIT-licensed, so you everybody can use it without obtaining a company license.
...
Sequences which are normally not rendered because the playback head is not within the time range of the sequence, will appear because Picus is doing an additional render at a time where the sequence is visible.
...
Want to render partial video or a still for thumbnail? You can do that with Picus 2.0. We introduced a new CLI flag `--frames` to render a subset or a still of a video.
...
A new GIF component released in the `@picus/gif` package is now available. The `<Gif />` component allows to load a local or remote GIF and synchronizes it with Picuss `useCurrentFrame()`. For that it parses the GIF using WebAssembly - basically it's really cool tech! Thanks a lot to [@jeetiss](https://github.com/jeetiss) for contributing this new component.
...
[Calvin Falter](https://github.com/calvin-f), [Jonas Brunner](https://github.com/j0nezz) and [Tobias Boner](https://github.com/dydent) took Picus as their project for their university seminar and contributed over 100 tests, strengthening the code and ironing out edge cases.

[@cnnr](https://github.com/cnnrhill) contributed a really cool end-to-end test that tests the video seeking of Picus and makes sure it is frame-accurate by matching the colors in each frame.
...
The bottom line: In Picus 1.5.4 we had total 43 tests, in Picus 2.0 we added a total of 174 tests to come out at a total of 217.
...
In Picus 1.x, a `<Video/>` would not always perfectly synchronize with Picuss time during render. This is now fixed and validated by tests that scan the color of the output frames - [however not in the most efficient way](https://github.com/picus-dev/picus/issues/200). Help on improving it further is still welcome.
...
We've taken this opportunity of a major release to introduce some breaking changes that we think are beneficial for an awesome future of Picus. Please take a moment to read through the [2.0 Migration guide](/docs/2-0-migration) and see if you are affected.

## The future

Most of the core functionality for making videos in React is now implemented.
This gives us time to focus on improving all the areas around it - we are talking better documentation, developer experience, examples, abstractions on top or Picus, improving speed and reliability of Picus.

Furthermore, we want to provide a way to embed Picus compositions in traditional web apps and to make server-side rendering easier - stay tuned!
