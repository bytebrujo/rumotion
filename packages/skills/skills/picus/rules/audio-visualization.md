...
# Audio Visualization in Picus

## Prerequisites

```bash
npx picus add @picus/media-utils
```

## Loading Audio Data

Use `useWindowedAudioData()` (https://www.picus.dev/docs/use-windowed-audio-data) to load audio data:

```tsx
import { useWindowedAudioData } from "@picus/media-utils";
import { staticFile, useCurrentFrame, useVideoConfig } from "picus";
...
Use `visualizeAudio()` (https://www.picus.dev/docs/visualize-audio) to get frequency data for bar charts:
...
Use `visualizeAudioWaveform()` (https://www.picus.dev/docs/media-utils/visualize-audio-waveform) with `createSmoothSvgPath()` (https://www.picus.dev/docs/media-utils/create-smooth-svg-path) for oscilloscope-style displays:
...
Use `getWaveformPortion()` (https://www.picus.dev/docs/get-waveform-portion) when you need simplified volume data instead of frequency spectrum:

```tsx
import { getWaveformPortion } from "@picus/media-utils";
