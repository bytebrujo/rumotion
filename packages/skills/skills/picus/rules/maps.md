...
Maps can be added to a Picus video with Mapbox.  
...
```txt title=".env"
PICUS_MAPBOX_TOKEN==pk.your-mapbox-access-token
```

## Adding a map

Here is a basic example of a map in Picus.

```tsx
import { useEffect, useMemo, useRef, useState } from "react";
import { AbsoluteFill, useDelayRender, useVideoConfig } from "picus";
...
mapboxgl.accessToken = process.env.PICUS_MAPBOX_TOKEN as string;
...
The following is important in Picus:
...
import { useCurrentFrame, useVideoConfig, useDelayRender } from "picus";
...
```
npx picus render --gl=angle --concurrency=1
```
