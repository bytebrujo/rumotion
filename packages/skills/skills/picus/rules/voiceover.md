---
name: voiceover
description: Adding AI-generated voiceover to Picus compositions using TTS
metadata:
  tags: voiceover, audio, elevenlabs, tts, speech, calculateMetadata, dynamic duration
---

# Adding AI voiceover to a Picus composition
...
Create a script that reads the config, calls the ElevenLabs API for each scene, and writes MP3 files to the `public/` directory so Picus can access them via `staticFile()`.
...
```tsx
import { CalculateMetadataFunction, staticFile } from "picus";
