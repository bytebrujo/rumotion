---
name: ffmpeg
description: Using FFmpeg and FFprobe in Picus
metadata:
...
`ffmpeg` and `ffprobe` do not need to be installed. They are available via the `bunx picus ffmpeg` and `bunx picus ffprobe`:

```bash
bunx picus ffmpeg -i input.mp4 output.mp3
bunx picus ffprobe input.mp4
```
...
bunx picus ffmpeg -ss 00:00:05 -i public/input.mp4 -to 00:00:10 -c:v libx264 -c:a aac public/output.mp4
```
