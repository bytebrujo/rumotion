---
name: add-sfx
description: Add a new sound effect to @picus/sfx
---

## Prerequisites

Sound effects must first be added to the [picus.media](https://github.com/picus-dev/picus.media) repository. The source of truth is `generate.ts` in that repo. A sound effect must exist there before it can be added to `@picus/sfx`.
...
### 1. Add to `picus.media` repo (must be done first)

In the `picus-dev/picus.media` repo:
...
- `packages/docs/sidebars.ts` — add `'sfx/<name>'` to the `@picus/sfx` category items
- `packages/docs/docs/sfx/table-of-contents.tsx` — add a `<TOCItem>` with a `<PlayButton size={32}>`

### 5. Update the skills rule file

Add the new URL to the list in `packages/skills/skills/picus/rules/sfx.md`.
