# Add a new Picus package

## Steps

1. **Create `packages/<name>/`** with these files:
   - `package.json` — copy from `@picus/light-leaks` as template; update name, description, homepage, dependencies
   ...
   - `bundle.ts` — Bun build script, externalize `react`, `picus`, `picus/no-react`, `react/jsx-runtime`, `react/jsx-dev-runtime`, `react-dom`
   - `eslint.config.mjs` — use `picusFlatConfig({react: true})` if React, `{react: false}` otherwise
   - `.npmignore` — copy from `@picus/light-leaks`
...
2. **Register in monorepo:**
   ...
   - `packages/cli/src/list-of-picus-packages.ts` — add `'@picus/<name>'`
   - `packages/create-video/src/list-of-picus-packages.ts` — add `'@picus/<name>'`
...
3. **Documentation (`packages/docs/docs/<name>/`):**
   - Add `"@picus/<name>": "workspace:*"` to `packages/docs/package.json` dependencies (needed for twoslash snippets)
...
4. **Example usage:**
   - Add `"@picus/<name>": "workspace:*"` to `packages/example/package.json`
...
For the documentation version, increment the patch version by 1 as it will only be released with the next Picus release.
