...
How to convert a hardcoded CLI flag into a proper `AnyPicusOption`, or add a brand new one.
...
export const myFlagOption = {
  ...
  docLink: 'https://www.picus.dev/docs/config#setmyflagenabled',
...
The type in `AnyPicusOption<T>` and `type: <default> as T` determines the option's value type. Use `boolean`, `string | null`, `number | null`, etc.
...
- Add to the destructured `BrowserSafeApis.options`
- Add the setter signature to the `FlatConfig` type (either in the `PicusConfigObject` global interface or the `FlatConfig` intersection)
